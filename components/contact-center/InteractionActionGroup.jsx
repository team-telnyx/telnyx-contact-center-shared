...
export function InteractionActionGroup({label,children,className}){
  return <div role="group" aria-label={label} className={cn("pointer-events-auto relative z-10 ml-auto flex shrink-0 divide-x divide-background/20 overflow-hidden rounded-md border border-foreground/20 bg-foreground text-background shadow-sm",className)}>{children}</div>;
}
...
export function InteractionActionButton({label,description,icon:Icon,busy,tone,disabled,...props}){
  return <Tooltip><TooltipTrigger asChild><span className="inline-flex" tabIndex={disabled?0:undefined}>
    <Button type="button" variant="ghost" size="icon" disabled={disabled} aria-label={label} aria-busy={busy||undefined}
      className={cn("size-8 rounded-none bg-transparent text-background hover:bg-background/15 hover:text-background aria-pressed:bg-background/20 aria-pressed:text-background focus-visible:ring-inset focus-visible:ring-background/70 focus-visible:ring-offset-0",
