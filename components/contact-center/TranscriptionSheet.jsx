...
// Speaker tones cycle for diarized turns (speaker 0, 1, 2, ...)
const SPEAKER_TONES = [
...
function speakerToneFor(speaker) {
  ...
  // Non-numeric speaker labels hash to a stable tone
  const hash = String(speaker).split("").reduce((acc, ch) => acc + ch.charCodeAt(0), 0);
  return SPEAKER_TONES[hash % SPEAKER_TONES.length];
}

function DiarizedConversation({ speakerTurns }) {
  return (
    <div className="space-y-4 px-4 pb-4" data-testid="diarized-conversation">
          ...
          >
            ...
            <div className={`flex min-w-0 max-w-[85%] flex-col ${isRight ? "items-end" : "items-start"}`}>
              <div className="mb-1 flex items-center gap-2 text-[11px] text-muted-foreground">
                <span className="font-medium">Speaker {turn.speaker}</span>
                <span className="font-mono tabular-nums">
                  {formatTimestamp(turn.start)}
                  {turn.end != null ? `–${formatTimestamp(turn.end)}` : ""}
                </span>
                {confidenceLabel && (
              ...
              <div className={`rounded-2xl px-3.5 py-2 text-sm shadow-sm ${isRight ? `${tone.bubble} rounded-br-md` : "rounded-bl-md bg-muted text-foreground"}`}>
          ...
          <Card className="mx-0 flex min-h-0 flex-1 flex-col border-border/70">
            ...
            <CardContent className="min-h-0 flex-1 p-0">
              <ScrollArea className="h-full">
                {hasSpeakerTurns ? (
                  <DiarizedConversation speakerTurns={speakerTurns} />
                ) : messages.length === 0 ? (
