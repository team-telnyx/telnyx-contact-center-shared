...
function normalizeAccordionItems(items = []) { return Array.isArray(items) ? items.map((item, index) => { const source = item && typeof item === "object" ? item : {}; return { ...source, _id: source._id || source.id || `accordion_item_${index + 1}`, title: source.title ?? `Item ${index + 1}`, content: source.content ?? source.body ?? "", defaultOpen: Boolean(source.defaultOpen || source.default_open) }; }) : []; }
...
export function FormRenderer({ form, initialValues = {}, context = {}, onSubmit, submitting = false, readOnly = false }) {
  ...
  const showNavigationButtons = normalized?.schema?.showNavigationButtons !== false;
  ...
  const renderField = (field) => {
    if (["section", "row", "columns", "grid", "flex", "spacer", "divider", "hero", "stats", "card", "richtext", "accordion", "avatar", "codeblock"].includes(field.type)) return <LayoutContainer key={field.id} field={field} byId={byId} renderField={renderField} />;
    ...
    if (field.type === "badge") return <div key={field.id} className={paddingClass(field.props?.padding)} style={paddingStyle(field.props?.padding)}><Badge variant={field.props?.variant || "secondary"} className={badgeSizeClass(field.props?.size)} style={badgeStyle(field)}>{field.props?.text || field.label || "Badge"}</Badge>{field.helpText ? <p className="mt-2 text-xs text-muted-foreground">{field.helpText}</p> : null}</div>;
    ...
    if (field.type === "image") return field.props?.src ? <div key={field.id} className="inline-block max-w-full overflow-hidden rounded-md border bg-muted"><img src={field.props.src} alt={field.label || "Form image"} className="block max-h-48 max-w-full object-contain" style={imageScaleStyle(field.props)} /></div> : null;
  ...
  return <form onSubmit={handleSubmit} className="space-y-4" style={formThemeStyle(normalized?.theme)}>
    ...
    {pages.length > 1 && showNavigationButtons ? <div className="mt-6 flex justify-end gap-2 border-t pt-4">
