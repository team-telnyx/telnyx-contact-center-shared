...
import { FORM_COMPONENT_TYPES, normalizeFormDefinition } from "@/lib/forms/form-schema";
...
export function FormBuilder({ value, onChange }) {
  ...
  function addField(type) { const id = `${type}_${Math.floor(Date.now()/1000)}`; update({ ...form, schema: { ...form.schema, fields: [...form.schema.fields, { id, type, label: `${type} field`, options: type === "select" || type === "radio" ? [{ label: "Option A", value: "a" }] : [] }] }, layout: { ...form.layout, order: [...(form.layout.order || []), id] } }); setSelectedId(id); }
  function move(id, dir) { const order = [...(form.layout.order || [])]; const i = order.indexOf(id); const j = i + dir; if (i < 0 || j < 0 || j >= order.length) return; [order[i], order[j]] = [order[j], order[i]]; update({ ...form, layout: { ...form.layout, order } }); }
  function remove(id) { update({ ...form, schema: { ...form.schema, fields: form.schema.fields.filter((f) => f.id !== id) }, layout: { ...form.layout, order: (form.layout.order || []).filter((x) => x !== id) } }); }
