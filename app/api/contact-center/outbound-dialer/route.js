...
export async function GET() {
  ...
  try {
    const [campaignsResult, listsResult, dncListsResult, formsResult, filtersRows, timeSetsRows, attemptControlsRows, settingsRows, queueRows, flowRows, assistants, inventoryNumbers] = await Promise.all([
      ...
      safeQuery(pool, `SELECT id, name, description FROM voice_flows WHERE EXISTS (SELECT 1 FROM jsonb_array_elements(nodes) AS n WHERE n->'data'->>'nodeType' = 'outbound_campaign') ORDER BY updated_at DESC LIMIT 200`),
      loadAiAssistants(),
