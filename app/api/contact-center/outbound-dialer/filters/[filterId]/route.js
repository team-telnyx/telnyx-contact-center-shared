...
export async function PUT(request, context) {
  ...
  try {
    ...
    const { rows } = await pool.query(`UPDATE outbound_contact_filters SET name=$1, description=$2, status=$3, contact_list_id=$4, conditions=$5, metadata=$6, updated_by=$7, updated_at=NOW() WHERE id=$8 AND status <> 'archived' RETURNING *`, [
      requireString(body.name, "Filter name"), optionalString(body.description), normalizeStatus(body.status), body.contact_list_id || null, JSON.stringify(safeJson(body.conditions, [])), JSON.stringify(safeJson(body.metadata, {})), usernameFor(user), filterId,
    ]);
