...
export async function POST(request) {
  ...
  try {
    ...
    const { rows } = await pool.query(`INSERT INTO outbound_contact_lists (name, description, status, source_type, standard_columns, custom_field_schema, custom_fields, created_by, updated_by) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$8) RETURNING *`, [requireString(body.name, "List name"), optionalString(body.description), ["draft", "validating", "validated"].includes(body.status) ? body.status : "draft", ["csv", "api", "crm", "manual"].includes(body.source_type) ? body.source_type : "csv", JSON.stringify(safeJson(body.standard_columns, {})), JSON.stringify(schema), JSON.stringify(safeJson(body.custom_fields, {})), username]);
