...
async function handle(request,context,authz){
  ...
  try{
    ...
    if(request.method==="GET")return NextResponse.json(await readTextDetail(pool,{workItemId:id,agentId}),{headers:{"Cache-Control":"no-store"}});
    const body=await request.json();
    if(body.action==="transfer")return NextResponse.json(await transferTextWork(pool,{...body,workItemId:id,agentId,channel:"chat"}));
    ...
    return NextResponse.json(await actOnTextWork(pool,{...body,workItemId:id,agentId,channel:"chat"}));
