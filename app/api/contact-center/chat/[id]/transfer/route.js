...
async function GET_handler(_request, context, authz){
  ...
  try{return NextResponse.json(await messagingTransferTargets(pool,{workItemId:(await context.params).id,agentId:String(user.id),channel:"chat"}),{headers:{"Cache-Control":"private, no-store"}});}
...
export const GET = withPermission("agent:self", GET_handler, { route: "/api/contact-center/chat/[id]/transfer" });
