...
async function GET_handler(request, context, authz){
  ...
  const file=(await pool.query(`SELECT f.* FROM acd_text_attachments f JOIN acd_work_items w ON w.conversation_id=f.conversation_id
    WHERE w.id=$1 AND f.id=$2 AND (EXISTS(SELECT 1 FROM acd_text_assignments a JOIN acd_segments s ON s.id=a.segment_id
...
export const GET = withPermission("agent:self", GET_handler, { route: "/api/contact-center/chat/[id]/attachments/[attachmentId]" });
