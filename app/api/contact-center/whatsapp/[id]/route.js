...
async function handle(request,context,authz){
  ...
  try{
    ...
    if(request.method==="GET")return NextResponse.json(await readTextDetail(pool,{workItemId:id,agentId,channel:"whatsapp"}),{headers:{"Cache-Control":"no-store"}});
    const body=await request.json();
    if(body.action==="transfer")return NextResponse.json(await transferTextWork(pool,{...body,workItemId:id,agentId,channel:"whatsapp"}));
    ...
    if(body.action==="send"&&body.template){
      await readTextDetail(pool,{workItemId:id,agentId,channel:"whatsapp"});
      ...
      return NextResponse.json(await actOnTextWork(pool,{action:"send",commandId:body.commandId,expectedVersion:body.expectedVersion,draftVersion:body.draftVersion,body:resolved.text,template:resolved,workItemId:id,agentId,channel:"whatsapp"},whatsappSendOptions()));
    }
    return NextResponse.json(await actOnTextWork(pool,{...body,workItemId:id,agentId,channel:"whatsapp"},whatsappSendOptions()));
