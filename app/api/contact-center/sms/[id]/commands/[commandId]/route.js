...
async function GET_handler(_request, context){
  ...
  try{const {id,commandId}=await context.params;return NextResponse.json(await readAgentChatCommand(pool,{workItemId:id,agentId:String(user.id),commandId,channel:"sms"}),{headers:{"Cache-Control":"no-store"}});}
...
export const GET = withPermission("agent:self", GET_handler, { route: "/api/contact-center/sms/[id]/commands/[commandId]" });
