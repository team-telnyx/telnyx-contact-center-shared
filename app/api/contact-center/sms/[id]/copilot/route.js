...
// SMS reuses the shared Copilot with its own channel authorization and settings fallback.
async function handle(request,context,authz){
  ...
  try{
    const {id}=await context.params,identity={workItemId:id,agentId:String(user.id),channel:"sms"};
...
export const GET = withPermission("agent:self", handle, { route: "/api/contact-center/sms/[id]/copilot" });
export const POST = withPermission("agent:self", handle, { route: "/api/contact-center/sms/[id]/copilot" });
