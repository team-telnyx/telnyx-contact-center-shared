...
export const GET=(request,context)=>withEmailUser(request,async({pool,agentId})=>{
  ...
  return readEmailDetail(pool,{workItemId:(await context.params).id,agentId,knownDraftVersions});
});
export const POST=(request,context)=>withEmailUser(request,async({pool,agentId})=>{
  const workItemId=(await context.params).id,input=await readEmailJson(request),identity={workItemId,agentId};
