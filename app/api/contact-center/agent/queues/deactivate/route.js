...
async function POST_handler(request, _context, authz) {
  try {
    ...
    const deactivated = queues.map((queue) => String(queue.id));
    queuesLogger.info("queue_memberships_deactivated", {
    ...
    queuesLogger.error("queue_memberships_deactivation_failed", contactCenterErrorPayload(error));
    return NextResponse.json({ ok: false, error: error.message || "Server error" }, { status: error.status || 500 });
  }
}

// Phase 2 migration: every export goes through the permission guard (the internal documentation).
export const POST = withPermission(["agents:queues.set","agent:self"], POST_handler, { route: "/api/contact-center/agent/queues/deactivate", elevated: "agents:queues.set" });
