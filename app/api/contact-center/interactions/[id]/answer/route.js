...
export async function POST(request, { params }) {
  try {
    ...
    const answeredAt = body?.answeredAt || new Date().toISOString();

    let updatedRoutingMetadata = interaction.routing_metadata || {};
    const timeline = updatedRoutingMetadata.timeline || [];
    const hasAlerting = timeline.some((event) => event.type === "alerting");
    if (!hasAlerting && interaction.assigned_at) {
    ...
    await PgDb.updateInteractionById(id, {
      answeredAt: interaction.answered_at || answeredAt,
      state: interaction.state === "ringing" ? "connected" : interaction.state,
      routingMetadata: updatedRoutingMetadata,
    });

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[InteractionAnswer] POST error:", err);
    return NextResponse.json(
      { ok: false, error: "Failed to mark interaction answered" },
      { status: 500 }
    );
  }
}
