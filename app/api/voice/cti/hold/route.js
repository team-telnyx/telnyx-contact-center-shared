 ...
 * Place call on hold via SIP NOTIFY
 */
export async function POST(request) {
  try {
    ...
    console.log(`[CTI Hold] Sending hold command to ${phoneConfig.ip}:${phoneConfig.port || 5060}`);

    const result = await sendPolycomNotify(phoneConfig, 'hold');
