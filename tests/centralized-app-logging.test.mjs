...
test("runtime logging config exposes centralized app logging settings with live TTL clamped to 5-30 minutes", async () => {
  ...
  const low = normalizeRuntimeLoggingConfig({ liveEnabled: true, liveTtlMinutes: 2, archiveEnabled: true, archiveProvider: "s3", archiveBucket: "cc-logs", archivePrefix: "logs/cc-ha", spoolEnabled: true, spoolDir: "/tmp/spool" });
  ...
  assert.equal(low.archiveBucket, "cc-logs");
...
test("Admin Logging settings expose live TTL and centralized archive controls", async () => {
  ...
  assert.match(page, /archiveBucket/);
