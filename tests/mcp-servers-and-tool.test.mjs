...
test("MCP Server admin page uses local Contact Center secrets and persists discovered schemas", async () => {
  ...
  assert.doesNotMatch(sheet, /Detect Authentication/, "sheet should not require a separate auth detection step before OAuth");
  assert.match(sheet, /connectServer/, "sheet should use a single Connect action for auth discovery and OAuth start");
  assert.match(sheet, /persistServer[\s\S]*oauth\/begin/, "single Connect flow should auto-save the MCP server before OAuth redirect");
  ...
  assert.doesNotMatch(sheet, /onClick=\{connectOAuth\}/, "sheet should not require a second Connect OAuth button after detection");
