...
test("payload option helper exposes HTTP, Data Action, MCP Tool, HTTP initiator, and webhook payload shapes", async () => {
  ...
  assert.match(
    source,
    /nodeType === "mcp_tool"[\s\S]*group:\s*"MCP Tool responses"[\s\S]*\[responseVariable\]: unwrappedResponsePayload[\s\S]*\`\$\{responseVariable\}_text\`[\s\S]*\`\$\{responseVariable\}_structured\`/,
