...
test("dashboard header has a right-aligned default-on running campaign toggle that filters cards", async () => {
  ...
  assert.match(pageSource, /if \(selectedCampaignId === selectedDashboardCampaign\.id\) return;[\s\S]*setSelectedCampaignId\(selectedDashboardCampaign\.id\);/, "Dashboard selection should be reconciled when the current campaign is hidden by the filter");
