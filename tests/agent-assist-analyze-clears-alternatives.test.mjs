...
test("live analyze route: completed branch clears alternatives; suggested branch persists them", async () => {
  ...
  // Five legitimate NULL clears remain: the completed branch, the
  ...
  // readBackJustConfirmed), the deterministic read-back-affirmative backstop
  // (matchesReadBackAffirmativeHints), and the deterministic agent-recitation
  // backstop (matchesAgentRecitation) — all five represent a slot/item
  // becoming genuinely completed, which never carries chips.
  const nullClears = route.match(/alternatives = NULL/g) || [];
  assert.equal(nullClears.length, 5, "completed branch + name-reconcile reset + correction auto-promotion + read-back-affirmative backstop + agent-recitation backstop clear NULL; suggested uses $7");
});

test("an earlier fix: workflow-store analyzeTranscript clears stale alternatives on merge", async () => {
