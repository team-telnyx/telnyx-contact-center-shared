...
test("outbound dialer exposes Disposition codes menu below Time Sets and CRUD wiring", async () => {
  ...
  assert.doesNotMatch(page, /Genesys-style mappings/, "Disposition codes card should not show the old top badge");
  assert.doesNotMatch(page, /Map the same wrap-up codes agents already know to outbound campaign outcomes/, "Disposition codes card should not show the long description");
  assert.match(page, /const dispositionSummaryClassifications = \[/, "Disposition codes summary should use a fixed classification list");
