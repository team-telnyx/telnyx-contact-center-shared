...
async function startServerSideWrapupForInteraction(
  ...
  try {
    ...
    console.error("[WebhookHandler] Failed to broadcast wrapup_required:", broadcastError);
  }
...
export async function handleContactCenterEvent(eventType, payload, opts = {}) {
  ...
  if (eventId) {
    try {
      ...
      console.warn("[WebhookHandler] Failed to check webhook idempotency:", idempotencyError);
    }
  }
  ...
  switch (eventType) {
    ...
    case "call.answered": {
      ...
      try {
        ...
        console.warn("[WebhookHandler] Failed to promote answered reservation to active:", {
          interactionId: interaction?.id,
          agentUsername: interaction?.agent_username,
          error: reservationError?.message || reservationError,
        });
      }
    ...
    case "call.hangup":
      ...
      if (isAgentLegNoAnswerDisconnect) {
        console.log(
          `[WebhookHandler] Agent leg disconnected before answer for interaction ${latestInteraction.id}; re-enqueueing caller instead of hanging up original leg`,
        );
        ...
        if (
          ...
          console.log(
            `[WebhookHandler] Agent leg no-answer handler found interaction ${latestInteraction.id} already in state "${latestInteraction.state}"; skipping stale hangup cleanup`,
          );
          return;
        }

        console.log(
          `[WebhookHandler] Agent leg no-answer handler did not re-enqueue interaction ${latestInteraction.id}; continuing with normal hangup cleanup`,
        );
      }
  ...
  if (Object.keys(updates).length > 0) {
    ...
    if (eventType === "call.hangup" && updates.state) {
      ...
      try {
        ...
        console.warn(
          `[WebhookHandler] Failed to release reservation for interaction ${interaction.id}:`,
