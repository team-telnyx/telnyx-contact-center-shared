...
export async function handleContactCenterEnqueue(params) {
  ...
  try {
    ...
    console.error(
      "[ContactCenter] Failed to start queue audio:",
      audioError
    );
  ...
  if (selectedAgent) {
    ...
    try {
      ...
      console.error(
        "[ContactCenter] Failed to stop queue audio:",
        audioError
      );
  ...
  if (eventType === "call.speak.ended") {
    if (callControlId) {
      try {
        ...
        if (result) {
          console.log(
            `[ContactCenter] Resumed queue media after position announcement for call ${callControlId}`
          );
        }
      } catch (audioError) {
        console.error(
          "[ContactCenter] Failed to resume queue media after speak:",
          audioError
        );
      }
    }
  ...
  switch (eventType) {
    ...
    case "call.dequeued":
      ...
      try {
        ...
        console.error(
          "[ContactCenter] Failed to stop queue audio on dequeued:",
          audioError
        );
      }
    ...
    case "call.recording.transcription.saved":
      ...
      if (transcriptionText) {
        ...
        (async () => {
          try {
            ...
            if (summary) {
              ...
              console.log(
                `[ContactCenter] Generated and saved call summary for interaction ${interaction.id}`
              );
            } else {
            ...
            console.error(
              "[ContactCenter] Error generating call summary:",
              summaryError
            );
            // Still save transcription even if summary generation fails
            const updatedMetadata = {
        ...
        updates.metadata = updatedMetadata;
        console.log(
          `[ContactCenter] Transcription saved for interaction ${interaction.id}`
        );
      }
      break;
