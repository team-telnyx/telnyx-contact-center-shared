      ...
      try {
        ...
        if (clientStateRaw) {
          ...
          if (clientStateDecoded?.azure_transcription_config?.enabled) {
            ...
            // Start Azure for the caller's call_control_id (original inbound leg)
            const callerCcId = interaction?.metadata?.original_call_control_id;
            // Start Azure for the agent's call_control_id (current answered leg)
            const agentCcId = callControlId;

            const { startAzureTranscription } = await import("@/lib/azure-speech-handler.mjs");

            if (callerCcId && callerCcId !== agentCcId) {
              startAzureTranscription(callerCcId, azureConfig, interactionId, agentUsername)
                .catch((e) => console.warn("[WebhookHandler] Azure start (caller) failed:", e.message));
            }
            startAzureTranscription(agentCcId, azureConfig, interactionId, agentUsername)
              .catch((e) => console.warn("[WebhookHandler] Azure start (agent) failed:", e.message));
          }
        }
      } catch (azureErr) {
