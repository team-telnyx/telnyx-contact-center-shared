...
export async function handleContactCenterEvent(eventType, payload) {
  ...
  switch (eventType) {
    case "call.answered": {
      ...
      // Start Azure transcription if configured in client_state (fires when agent answers)
      try {
        const clientStateRaw = payload?.client_state;
        if (clientStateRaw) {
          const clientStateDecoded = JSON.parse(
            Buffer.from(clientStateRaw, "base64").toString("utf-8")
          );
          if (clientStateDecoded?.azure_transcription_config?.enabled) {
            const azureConfig = clientStateDecoded.azure_transcription_config;
            ...
            // The Telnyx streaming WS session is registered under the caller's CCI
            // (the CCI used for streaming_start, i.e. the original inbound leg).
            // On call.answered the payload CCI is the AGENT's new leg — not the stream CCI.
            // Priority order for finding the right stream CCI:
            //   1. original_call_control_id in interaction metadata (set by webrtc-bridge)
            //   2. Scan globalThis.__azureTelnyxSessions for any active session
            //      (works even without WebRTC bridge — fallback for direct/PSTN agent legs)
            //   3. Fall back to agentCcId as last resort

            const agentCcId = callControlId;
            let streamCcId = interaction?.metadata?.original_call_control_id;

            if (!streamCcId || streamCcId === agentCcId) {
              // Fallback: find the active Telnyx stream session
              const sessions = globalThis.__azureTelnyxSessions;
              if (sessions instanceof Map && sessions.size > 0) {
                // Take the most recently added session (most likely the current call)
                streamCcId = [...sessions.keys()].pop();
                console.log(`[WebhookHandler] Azure: using stream CCI from session scan: ${streamCcId}`);
              }
            }

            const targetCcId = streamCcId || agentCcId;
            console.log(`[WebhookHandler] Azure: starting transcription (streamCcId=${targetCcId}, agentCcId=${agentCcId}, agent=${agentUsername})`);

            startAzureTranscription(targetCcId, azureConfig, interactionId, agentUsername)
