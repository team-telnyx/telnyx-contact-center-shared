...
export async function handleContactCenterEvent(eventType, payload) {
  ...
  switch (eventType) {
    ...
    case "call.hangup":
      ...
      // BUT NOT if we're in a consult state - the original call should stay parked
      const isConsultActive = interaction.metadata?.consult_state?.isActive === true;
      
      if (isAgentLegHangup && interaction.metadata?.original_call_control_id && !isConsultActive) {
      ...
      } else if (isAgentLegHangup && isConsultActive) {
        console.log(
          `[WebhookHandler] Skipping original leg hangup - consult is active for interaction ${interaction.id}`,
        );
      }
