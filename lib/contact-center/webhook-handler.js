...
import { contactCenterErrorPayload, interactionsLogger } from "./logging.mjs";
import {
  enqueueCall,
  assignCallToAgent,
  answerCall,
...
import {
  promoteReservation,
  releaseByInteraction,
  releaseReservation,
} from "./reservation-manager.js";
import { handleAgentLegNoAnswerDisconnect } from "./agent-answer-timeout.js";
import { alreadyProcessed } from "../events/idempotency.js";
...
// Share dedup + segment state with agent-assist-transcription-router.mjs.
// Both modules can process the same agent audio (once via standalone WebSocket
// STT, once via call.transcription webhook). Using separate Maps caused the
// per-leg dedup to miss the cross-module duplicate, showing every agent
// utterance twice. Sharing via globalThis ensures whichever path arrives first
// populates the cache and the second path is correctly dropped.
if (!globalThis.__agentAssistRecentTranscriptionCache) {
  globalThis.__agentAssistRecentTranscriptionCache = new Map();
}
if (!globalThis.__agentAssistActiveTranscriptionSegments) {
  globalThis.__agentAssistActiveTranscriptionSegments = new Map();
}
const recentTranscriptionCache = globalThis.__agentAssistRecentTranscriptionCache;
const activeTranscriptionSegments = globalThis.__agentAssistActiveTranscriptionSegments;
...
function normalizeForDedup(text) {
  return compactTranscriptText(text).toLowerCase().replace(/[.,!?;:]+$/, "").trim();
}
...
function isWorkflowTestingAgentTranscriptionTrack(track) {
  const normalizedTrack = String(track || "").toLowerCase();
  return !["customer", "inbound"].includes(normalizedTrack);
}
...
async function startServerSideWrapupForInteraction(
  interaction,
  { completedAt = null, reason = "call_completed" } = {},
) {
  if (!interaction?.id || !interaction.agent_username) return false;

  const answeredAt = interaction.answered_at || interaction.answeredAt;
  if (!answeredAt) return false;

  const startedAt = new Date(completedAt || Date.now()).toISOString();
  const metadata = {
    ...(interaction.metadata || {}),
    wrapup_started_at: interaction.metadata?.wrapup_started_at || startedAt,
    wrapup_started_server_side: true,
  };
  let routingMetadata = interaction.routing_metadata || interaction.routingMetadata || {};

  if (!interaction.metadata?.wrapup_started_at) {
    routingMetadata = addTimelineEvent(
      routingMetadata,
      TimelineEventTypes.WRAPUP_START,
      {
        timestamp: metadata.wrapup_started_at,
  ...
  try {
    ...
    await broadcastToKey(`contact-center:agent:${interaction.agent_username}`, {
      ...
      wrapupStartedAt: metadata.wrapup_started_at,
    });
  ...
  try {
    ...
    if (routingResult.success && routingResult.agent) {
      ...
      const updatedMetadata = {
        ...currentMetadata,
        reservationId: routingResult.reservationId || currentMetadata.reservationId,
      };
...
export async function handleCallAnswered(webhookData) {
  try {
    ...
    // Find interaction by call_control_id and include queue/agent identity so
    // monitor SSE can invalidate both queue call rows and expanded agent rows.
        ...
        for (const supervisor of supervisors.rows || []) {
          await broadcastToKey(
            `monitor:${supervisor.id}`,
            {
              ...
              agentUserId: interaction.agent_user_id
                ? String(interaction.agent_user_id)
                : null,
...
export async function handleCallEnded(webhookData) {
  try {
    ...
    const failedRinging =
      !interaction.answered_at &&
      (hangup_cause === "CALL_REJECTED" || hangup_cause === "NO_ANSWER");
    ...
    if (!wasAbandoned) {
      await startServerSideWrapupForInteraction(interaction, {
        completedAt,
        reason: "contact_center_call_ended",
      });
    }

    // Update state manager
    completeCall(interaction.id, completedAt, wasAbandoned, { failedRinging });
...
export async function handleContactCenterEnqueue(params) {
  ...
  // WS3: cross-node signal that a call is waiting (inert unless
  // ROUTING_EVENT_DRIVEN=true). The direct routeCall below stays
  // authoritative for this node; remote reactors are backstopped by
  // reservation atomicity.
  try {
    const { publishCallEnqueued } = await import("./routing-events.js");
    publishCallEnqueued({
      queueId: queue.id,
      queueName: queue.name,
      callControlId,
  ...
  if (selectedAgent) {
    ...
    const routingMetadataWithAlerting = addTimelineEvent(
      ...
      "alerting",
      {
        ...
        reservationId: routingResult.reservationId || null,
      },
    );
    ...
    try {
      ...
      if (routingResult.reservationId) {
        await releaseReservation(routingResult.reservationId);
      }
      await PgDb.updateInteractionById(interactionId, {
        agentUsername: null,
        state: "queued",
 ...
 */
export async function handleContactCenterEvent(eventType, payload, opts = {}) {
  const callControlId = payload?.call_control_id;
  const callSessionId = payload?.call_session_id;
  if (!callControlId && !callSessionId) return;

  const eventId = opts?.eventId || payload?.event_id || payload?.id || null;
  if (eventId) {
    try {
      if (await alreadyProcessed(eventId, `contact-center:${eventType}`, opts)) {
  ...
  // For call.initiated, call.answered and call.hangup, also try to find by agent_call_control_id or original_call_control_id in metadata
  // This handles cases where the webhook is for the agent's leg (WebRTC) rather than the original call leg
  // When the agent leg starts/rings/answers via WebRTC, webhook arrives with the agent's call_control_id, not the original call's
  if (
    !interaction &&
    callControlId &&
    (eventType === "call.initiated" || eventType === "call.answered" || eventType === "call.hangup")
  ) {
    try {
      const pool = getPostgresPool();
      if (pool) {
  ...
  switch (eventType) {
    case "call.initiated": {
      const isAgentLegInitiated =
        interaction.metadata?.agent_call_control_id === callControlId;
      if (isAgentLegInitiated && interaction.agent_username) {
        updates.state = "ringing";
        updates.routingMetadata = addTimelineEvent(
          updatedRoutingMetadata,
          "alerting",
          {
            timestamp: payload?.occurred_at || new Date().toISOString(),
            agentUsername: interaction.agent_username,
            source: "agent_leg_call_initiated",
          },
        );
        await handleAgentCallLifecycleStatus({
          event: "ringing",
    ...
    case "call.answered": {
      ...
      // A ringing reservation has a short lease for no-answer detection. Once the
      // agent answers, make it active so the expiry sweep cannot restore the
      // agent to Available while the call is actually connected.
      try {
        const reservationId = interaction?.metadata?.reservationId || interaction?.metadata?.reservation_id;
        if (reservationId) {
          await promoteReservation(reservationId, "active");
        }
      ...
      try {
        ...
        if (sttConfig?.enabled) {
          ...
          // If the current stream is not explicitly both_tracks, keep per-leg STT
          // separate instead of not treating it as both_tracks implicitly. Agent-leg
          // media uses inbound_track from the WebRTC leg and maps it to outbound bubbles.
    ...
    case "call.hangup":
      ...
      const isAgentLegHangup =
        interaction.metadata?.agent_call_control_id === callControlId;
      const isOriginalCustomerLegHangup =
        callControlId === interaction.call_control_id ||
        interaction.metadata?.original_call_control_id === callControlId;
      ...
      const isAgentLegNoAnswerDisconnect =
        ...
        !latestInteraction.answered_at &&
        !hasConsultState;

      if (isAgentLegNoAnswerDisconnect) {

        const wasReEnqueued = await handleAgentLegNoAnswerDisconnect(
          latestInteraction,
        );
        if (wasReEnqueued) {
          return;
        ...
        const refreshedAfterNoAnswer = await PgDb.findInteractionById(
          latestInteraction.id,
        );
        if (refreshedAfterNoAnswer) {
          latestInteraction = refreshedAfterNoAnswer;
        }
        if (
          !latestInteraction.answered_at &&
      ...
      if (
        ...
        latestInteraction.agent_username &&
        !latestInteraction.answered_at &&
        latestInteraction.state === "ringing"
      ) {
        await handleAgentCallLifecycleStatus({
          event: "customer-abandoned",
          username: latestInteraction.agent_username,
      ...
      if (
        ...
        try {
          ...
          if (apiKey) {
            ...
            interactionsLogger.info("hanging_up_original_call_leg", {
              action: "Hanging up original call leg",
          ...
          interactionsLogger.warn("original_call_leg_hangup_failed", {
            ...contactCenterErrorPayload(hangupError),
            interactionId: latestInteraction.id,
            callControlId: originalCallControlId,
          });
        }
      } else if (isAgentLegHangup && hasConsultState) {
        interactionsLogger.debug("agent_leg_hangup_consult_preserved", {
          interactionId: latestInteraction.id,
          callControlId,
          reason: "consult_call_state_present",
        });
      }
      ...
      const hasAnswered = !!latestInteraction.answered_at;
      const finalState = hasAnswered ? "completed" : "abandoned";
    ...
    case "call.dequeued":
      ...
      // Call was dequeued from Telnyx queue media/control, but that does not
      // necessarily mean an agent leg has been assigned yet. Inbound routing
      // must remain DB-routable until we have an assigned agent. Otherwise a
      // bare dequeued event can move queued -> ringing with no agent_username,
      // making the call invisible to queued-call-router re-evaluation.
      if (
  ...
  if (Object.keys(updates).length > 0) {
    ...
    if (eventType === "call.hangup" && updates.state) {
      ...
      const failedRinging =
        wasAbandoned &&
        !interaction.answered_at &&
        (payload?.hangup_cause === "CALL_REJECTED" ||
          payload?.hangup_cause === "NO_ANSWER");
      try {
        await releaseByInteraction(interaction.id, {
          reason: wasAbandoned ? "interaction_abandoned" : "interaction_completed",
        });
      } catch (reservationReleaseError) {
        // Previously swallowed silently, which let a failed release here leave
        // cc_agent_reservations stuck 'active' (and current_calls_count stale)
        // indefinitely with no signal in the logs — agent looked busy forever.
        interactionsLogger.error("reservation_release_on_hangup_failed", {
          interactionId: interaction.id,
          ...contactCenterErrorPayload(reservationReleaseError),
        });
      }
      if (!wasAbandoned) {
        await startServerSideWrapupForInteraction(interaction, {
    ...
    if (eventType === "call.hangup" && interaction.metadata?.consult_state?.isActive) {
      ...
      if (isParkedCallEnding) {
        ...
        try {
          ...
          // Set agent status to Wrapup through the central lifecycle handler
          if (interaction.agent_username) {
            await handleAgentCallLifecycleStatus({
              event: "disconnected",
              username: interaction.agent_username,
              interaction: {
                ...interaction,
                answered_at: interaction.answered_at || new Date().toISOString(),
              },
            });
          }
    ...
    if (
      ...
      try {
        ...
        const pool = getPostgresPool();
        if (pool) {
          if (!interaction.agent_user_id && interaction.agent_username) {
            const agentResult = await pool.query(
              `SELECT id FROM users WHERE username = $1 LIMIT 1`,
              [interaction.agent_username],
            );
            interaction.agent_user_id = agentResult.rows?.[0]?.id || null;
          }
...
export async function handleTranscriptionEvent(payload) {
  ...
  if (isTranscriptionSuppressed(callControlId)) {
    interactionsLogger.debug("contact_center_webhook_agentassist", { interactionId: typeof interactionId !== "undefined" ? interactionId : typeof interaction !== "undefined" ? interaction?.id : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof legId !== "undefined" ? legId : undefined, queueId: typeof queueId !== "undefined" ? queueId : undefined, agentUserId: typeof targetUserIdFinal !== "undefined" ? targetUserIdFinal : typeof userId !== "undefined" ? userId : typeof user !== "undefined" ? user?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined });
    return;
  }
  ...
  // Mirror the same logic as routeAgentAssistTranscription: require BOTH
  // speech_final=true AND is_final=true to finalize a bubble. The router and
  // webhook share activeTranscriptionMessages, so mismatched logic here causes
  // the webhook to delete the shared UUID key on the early VAD event, forcing
  // the router's later is_final event to generate a new UUID → two bubbles.
  const hasProviderFinalField = "is_final" in transcriptionData;
  const isMessageFinal = hasProviderFinalField
    ? (transcriptionData.speech_final === true && isProviderFinal)
    : (hasSpeechFinal ? transcriptionData.speech_final === true : false);
  const dedupeKey = getTranscriptionLiveKey(callControlId, track);
  const nowMs = Date.now();
  if (isMessageFinal) {
    // Per-leg dedup: same callControlId + track (catches webhook delivered twice
    // or the same event arriving via both the standalone-STT router and the
    // call.transcription webhook path, now that both share the cache).
    ...
    // Cross-leg dedup: agent audio can arrive via two different conference legs
    // (two callControlIds on the same call session). Key on session + direction +
    // text so the second arrival within the window is dropped.
    //
    // IMPORTANT: seed the cross-leg key even when isPerLegDup is true. If the
    // standalone-STT router processed this audio first it populated the per-leg
    // key but never touched the session key. Without seeding here, a subsequent
    // arrival on a different conference leg (different callControlId, different
    // per-leg key) finds no cross-leg entry and slips through as a duplicate.
  ...
  // Deepgram WebSocket STT (router path) is the single source of truth when active.
  // Skip this webhook path entirely so the two STT systems never produce duplicate bubbles.
  // Check both callControlId (same leg) and interactionId (cross-leg, e.g. streamCcId webhook
  // when agentCcId has the active WebSocket session). Use String() to avoid type mismatches.
  if (globalThis.__telnyxSttActiveSessions instanceof Map) {
    const idStr = String(interaction.id);
    const hasActiveSession =
      globalThis.__telnyxSttActiveSessions.has(callControlId) ||
      [...globalThis.__telnyxSttActiveSessions.values()].some(
        a => a.interactionId != null && String(a.interactionId) === idStr
      );
    if (hasActiveSession) return;
  }

  // Interaction-level dedup: mirrors the same check in routeAgentAssistTranscription.
  // The Telnyx built-in STT (this webhook path) and the standalone Deepgram STT
  // (routeSTT path) can both fire for the same agent utterance. They reach
  // different broadcast functions so the router-side dedup does not catch this.
  // Use the full accumulated turn text (same merge logic as buildDisplayTranscript)
  // so different chunk boundaries still produce the same key.
  if (isMessageFinal && transcriptionData.transcript) {
    ...
    const itrDedup = normalizeForDedup(itrText);
    const ownLeg = normalizeTranscriptionLeg(track);
    const otherLeg = ownLeg === "inbound" ? "outbound" : "inbound";
    // Cross-path tail suppression: the router (standalone STT) uses agentCcId while
    // call.transcription webhooks carry callerCcId — different per-callControlId
    // rfinal keys, same shared cache. If the router already broadcast the full
    // utterance (e.g. "Sara Thompson.") and Telnyx STT now sends a suffix-only
    // final (e.g. "Thompson."), suppress it here before the itr-r text-exact check.
    const rfinalItrKey = `rfinal-itr:${interaction.id}:${ownLeg}`;
    const rfinalItr = recentTranscriptionCache.get(rfinalItrKey);
    if (rfinalItr && nowMs - rfinalItr.ts < 5000) {
      const rfDedup = normalizeForDedup(rfinalItr.text);
      if (rfDedup === itrDedup || rfDedup.endsWith(" " + itrDedup) || rfDedup.endsWith(itrDedup)) {
    ...
    // Source-tagged keys: itr-w (this webhook path) vs itr-r (standalone-STT router).
    // Keys use normalizeForDedup so utterance_end_ms punctuation refinements don't bypass dedup.
    const itrKeyWOwn  = `itr-w:${interaction.id}:${ownLeg}:${itrDedup}`;
    const itrKeyROOwn = `itr-r:${interaction.id}:${ownLeg}:${itrDedup}`;
    const itrKeyROther = `itr-r:${interaction.id}:${otherLeg}:${itrDedup}`;
    ...
    const isDup =
      ...
      (seenROther && nowMs - seenROther.ts < 2000);
    if (isDup) {
      activeTranscriptionSegments.delete(itrLiveKey);
      getTranscriptionMessageKey(callControlId, track, true);
      clearOpposingTranscriptionSegments(callControlId, track);
    ...
    recentTranscriptionCache.set(itrKeyWOwn, { ts: nowMs });
    recentTranscriptionCache.set(`rfinal-itr:${interaction.id}:${ownLeg}`, { text: itrText, ts: nowMs });
  }

  // Phase-1 source diagnostic: log every final that reaches broadcast.
  if (isMessageFinal && transcriptionData?.transcript) {
    interactionsLogger.info("transcription_source_webhook", {
      ccid: callControlId,
      track,
      interactionId: interaction.id,
      text: transcriptionData.transcript.slice(0, 80),
    });
  }
  ...
  // The router (WebSocket/Deepgram) is the sole source of live-update bubbles.
  // Broadcasting non-finals from the webhook path creates orphaned live bubbles:
  // the router's final lands with a different key, the dedup suppresses the
  // webhook's final, and the webhook's live bubble is never closed.
  // Only broadcast finals from the webhook path so it serves as a fallback
  // without interfering with the router's live-update stream.
  if (!isMessageFinal) {
    return;
  }
  ...
  processFinalTranscriptionEnhancements({
    interaction,
    callControlId,
    transcriptionData: routedTranscriptionData,
    transcriptionKey,
    assistConfig,
  }).catch((error) => {
  ...
  if (isWorkflowTestingAgentTranscriptionTrack(track)) {
    import("@/lib/call-generator/workflow-testing.mjs")
      .then(({ handleWorkflowTestingFinalTranscription }) => handleWorkflowTestingFinalTranscription({
      ...
      .catch((error) => {
        interactionsLogger.warn("contact_center_webhook_agentassist", {
          ...contactCenterErrorPayload(error),
          interactionId: interaction.id,
          callControlId,
          reason: "workflow_testing_transcription_reply_failed",
        });
      });
  }
}
