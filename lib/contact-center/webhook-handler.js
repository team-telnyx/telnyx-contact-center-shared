...
export async function handleTranscriptionEvent(payload) {
  ...
  // AI Agent voice tests can use plain Call Control transcription without an
  // Agent Assist/contact-center interaction. Correlate those final assistant
  // utterances through the voice-test ledger before the non-contact-center guard
  // below, and pass the same accumulated transcript Agent Assist would display.
  if (isMessageFinal && isWorkflowTestingAgentTranscriptionTrack(track)) {
    try {
      const { resolveAiAgentVoiceTestAssist } = await import("@/lib/workflows/ai-agent-voice-test.mjs");
      const { handleWorkflowTestingFinalTranscription } = await import("@/lib/call-generator/workflow-testing.mjs");
      const pool = getPostgresPool();
      const aiTest = await resolveAiAgentVoiceTestAssist({ pool, interaction, payload });
      if (aiTest?.assistConfig) {
        handleWorkflowTestingFinalTranscription({
          pool,
          interaction: interaction || { from_number: payload?.from || null, metadata: {} },
          payload,
          transcriptionData: {
            ...transcriptionData,
      ...
      interactionsLogger.warn("contact_center_webhook_agentassist", {
        ...
        interactionId: interaction?.id,
        callControlId,
        reason: "ai_agent_voice_test_route_failed",
      });
    }
  }
