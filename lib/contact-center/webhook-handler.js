...
export async function handleContactCenterEvent(eventType, payload) {
  ...
  switch (eventType) {
    case "call.answered": {
      ...
      // Start Azure transcription if configured
      // Strategy: azure_transcription_config lives on the CALLER's leg client_state
      // (injected by streaming_start node). call.answered fires for the AGENT's leg
      // which has a DIFFERENT client_state — so we CANNOT read azure config from payload.
      // Instead: scan globalThis.__azureTelnyxSessions for active stream sessions,
      // which store the full caller client_state (including azure_transcription_config).
      try {
        const { startAzureTranscription } = await import("@/lib/azure-speech-handler.mjs");
        const interactionId = interaction?.id;
        const agentUsername = interaction?.agent_username;
        const agentCcId = callControlId;

        console.log(`[WebhookHandler] call.answered: agentCcId=${agentCcId}, agent=${agentUsername}, azureSessions=${globalThis.__azureTelnyxSessions?.size ?? 0}`);

        // Find the active Telnyx stream session (registered under caller's CCI)
        const sessions = globalThis.__azureTelnyxSessions;
        let streamCcId = null;
        let azureConfig = null;

        if (sessions instanceof Map && sessions.size > 0) {
          // Check original_call_control_id first (set by webrtc-bridge)
          const callerCcId = interaction?.metadata?.original_call_control_id;
          if (callerCcId && sessions.has(callerCcId)) {
            streamCcId = callerCcId;
            azureConfig = sessions.get(callerCcId)?.clientState?.azure_transcription_config;
            console.log(`[WebhookHandler] Azure: found stream session via original_call_control_id (${streamCcId})`);
          }

          if (!streamCcId) {
            // Fallback: scan sessions — but only match the one belonging to THIS interaction
            // to avoid cross-call transcript mixing when multiple calls are active concurrently.
            for (const [cid, sess] of sessions) {
              const cfg = sess?.clientState?.azure_transcription_config;
              if (!cfg?.enabled) continue;
              // Verify the session belongs to this interaction by interactionId
              if (sess?.interactionId && interactionId && sess.interactionId !== interactionId) continue;
              streamCcId = cid;
              azureConfig = cfg;
              console.log(`[WebhookHandler] Azure: found stream session via scan (${streamCcId}, interactionId=${sess?.interactionId})`);
              break;
            }
          }
        }

        // Also check payload client_state as fallback (for direct calls where same CCI is used)
        if (!azureConfig) {
          try {
            const csRaw = payload?.client_state;
            if (csRaw) {
              const cs = JSON.parse(Buffer.from(csRaw, "base64").toString("utf-8"));
              if (cs?.azure_transcription_config?.enabled) {
                azureConfig = cs.azure_transcription_config;
                streamCcId = streamCcId || agentCcId;
                console.log(`[WebhookHandler] Azure: found config in payload client_state`);
              }
            }
          } catch (_) {}
        }
        ...
        if (azureConfig?.enabled) {
          console.log(`[WebhookHandler] Azure: starting (streamCcId=${streamCcId}, agent=${agentUsername}, translationEnabled=${azureConfig.translationEnabled})`);
          startAzureTranscription(streamCcId, azureConfig, interactionId, agentUsername)
            .catch((e) => console.warn("[WebhookHandler] Azure start failed:", e.message));
        } else {
          console.log(`[WebhookHandler] Azure: not configured or no active stream session — skipping`);
        }
      } catch (azureErr) {
        console.warn("[WebhookHandler] Azure transcription check failed:", azureErr.message);
      }

      // Start Telnyx standalone STT WebSocket transcription if configured.
      // It mirrors the Azure strategy: stream config is on the caller leg client_state,
      // while call.answered usually arrives for the agent leg.
      try {
        const { startTelnyxSttTranscription } = await import("@/lib/telnyx-stt-handler.mjs");
        const interactionId = interaction?.id;
        const agentUsername = interaction?.agent_username;
        const agentCcId = callControlId;
        ...
        if (sttConfig?.enabled) {
          console.log(`[WebhookHandler] TelnyxSTT: starting (streamCcId=${streamCcId}, agent=${agentUsername}, engine=${sttConfig.transcription_engine}, model=${sttConfig.model})`);
          startTelnyxSttTranscription(streamCcId, sttConfig, interactionId, agentUsername)
            .catch((e) => console.warn("[WebhookHandler] TelnyxSTT start failed:", e.message));
        } else {
    ...
    case "call.hangup":
      ...
      // Stop Azure transcription sessions if active
      try {
        const { stopAzureTranscription } = await import("@/lib/azure-speech-handler.mjs");
        stopAzureTranscription(callControlId).catch(() => {});
      } catch (_azureErr) {
        // Azure cleanup — not critical
      }
    ...
    try {
      ...
      if (!callerLanguage && transcriptionData.transcription_track === "inbound") {
        if (detectedLanguage) {
          ...
          console.log("[AgentAssist] No language in webhook — detecting via Google Translate");
