...
async function broadcastAgentCallsChanged({
  ...
  try {
    ...
    console.error("[AgentAnswerTimeout] Failed to broadcast agent call removal:", error);
  }
}
...
async function handleTimeoutForInteraction(interaction) {
  ...
  if (!["ringing", "queued"].includes(currentState)) {
    console.log(
      `[AgentAnswerTimeout] Interaction ${interactionId} is in state "${currentState}" (not ringing/queued pre-answer), skipping timeout`,
    );
    return false;
  }
...
export async function resetAgentNotAnsweringStatus(userId) {
  console.log(
    `[AgentAnswerTimeout] Ignoring legacy Agent Not Answering reset for agent ${userId}; use manual dropdown or lifecycle handler`,
  );
