...
async function handleTimeoutForInteraction(interaction) {
  ...
  // Get queue configuration for enqueue API call
  const queueResult = await pool.query(
    `SELECT name, max_wait_time_secs, max_size FROM cc_queues WHERE id = $1`,
    [queueId],
  );
  ...
  // Get original call priority and required skills from metadata
  const originalPriority = interactionMetadata?.priority || null;
  const originalRequiredSkills = interactionMetadata?.required_skills || null;
  ...
  if (originalPriority || originalRequiredSkills) {
    ...
    if (originalPriority) {
      clientStateObj.priority = originalPriority;
    }
...
async function hangupAgentLeg(interaction) {
  const { metadata, call_control_id: originalCallControlId } = interaction;
  const agentCallControlId =
    metadata?.agent_call_control_id || metadata?.agentCallControlId;

  // If we have the agent's call control ID, hangup that leg
  ...
  // Fallback: Try to hangup the original call control ID
  // This will stop the entire call, which is acceptable if we can't find the agent leg
  if (originalCallControlId) {
    try {
      await hangupCallLeg(originalCallControlId);
      console.log(
        `[AgentAnswerTimeout] Hung up original call leg ${originalCallControlId}`,
      );
    } catch (error) {
      console.error(
        `[AgentAnswerTimeout] Failed to hangup original call leg ${originalCallControlId}:`,
        error,
      );
      throw error;
    }
 ...
 * @param {number|null} maxWaitTimeSecs - Max wait time in seconds
 * @param {number|null} maxSize - Max queue size
 */
async function reEnqueueCallViaTelnyx(
  callControlId,
  queueName,
  clientState = null,
  maxWaitTimeSecs = null,
  maxSize = null,
) {
  const apiKey = process.env.TELNYX_API_KEY;
  if (!apiKey) {
  ...
  if (maxWaitTimeSecs !== null && maxWaitTimeSecs !== undefined) {
    body.max_wait_time_secs = maxWaitTimeSecs;
  }
