...
async function reEvaluateRoutingForAvailableAgents() {
  try {
    console.log("[StateManager][RoutingDiagnostics] periodic re-eval tick", {
      timestamp: new Date().toISOString(),
    });
    const pool = getPostgresPool();
    if (!pool) {
      console.log("[StateManager][RoutingDiagnostics] periodic re-eval skipped", {
    ...
    console.log("[StateManager][RoutingDiagnostics] eligible agents loaded", {
      count: agentsResult.rows?.length || 0,
      userIds: (agentsResult.rows || []).map((row) => String(row.user_id)),
      timestamp: new Date().toISOString(),
    });
    ...
    const reEvalPromises = agentsResult.rows.map(async (row) => {
      try {
        ...
        console.log("[StateManager][RoutingDiagnostics] agent offer result", {
        ...
        console.error("[StateManager][RoutingDiagnostics] agent offer result", {
          userId: String(row.user_id),
