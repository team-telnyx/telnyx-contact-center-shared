...
export function initializeStateManager() {
  ...
  console.log("[StateManager] Initialized with sync interval:", SYNC_INTERVAL);
}
...
async function loadStateFromDatabase() {
  try {
    ...
    console.log(
      `[StateManager] Loaded ${stateCache.queues.size} queues and ${stateCache.agents.size} agents`
    );
  } catch (error) {
  ...
  while (retryCount < maxRetries) {
    ...
    try {
      ...
      if (error.code === "40P01" && retryCount < maxRetries - 1) {
        ...
        console.warn(
          `[StateManager] Deadlock detected, retrying (${retryCount}/${maxRetries}) after ${delay.toFixed(
            0
          )}ms...`
        );
      ...
      // For other errors or max retries reached, log and exit
      console.error("[StateManager] Error syncing state to database:", error);
      if (error.code === "23514") {
        console.error(
          "[StateManager] Check constraint violation - ensure cc_agent_state_agent_status_check constraint is dropped"
        );
      }
 ...
 */
export function enqueueCall(queueId, interactionId, enqueuedAt) {
...
export async function refreshQueueState(queueId) {
  try {
    ...
    console.error(
      `[StateManager] Error refreshing queue state for ${queueId}:`,
      error
    );
  }
}
