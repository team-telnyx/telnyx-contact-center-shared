...
import { agentPayload, contactCenterErrorPayload, routingLogger } from "./logging.mjs";
...
async function reEvaluateRoutingForAvailableAgents() {
  try {
    routingLogger.debug("state_manager_periodic_reeval_tick", {
      reason: "skill_relaxation_periodic_check",
    });

    const pool = getPostgresPool();
    if (!pool) {
      routingLogger.warn("state_manager_periodic_reeval_skipped", {
    ...
    routingLogger.debug("state_manager_periodic_reeval_eligible_agents_loaded", {
      eligibleAgentCount: agentsResult.rows?.length || 0,
    });
    ...
    const reEvalPromises = agentsResult.rows.map(async (row) => {
      try {
        const result = await offerQueuedCallForAgent({ userId: String(row.user_id) });
        routingLogger.debug("state_manager_periodic_reeval_agent_offer_result", {
          ...agentPayload({ agentUserId: row.user_id }),
        ...
        routingLogger.error("state_manager_periodic_reeval_agent_offer_failed", {
          ...agentPayload({ agentUserId: row.user_id }),
    ...
    routingLogger.error("state_manager_periodic_reeval_failed", contactCenterErrorPayload(error));
  }
}
    ...
    for (const row of result.rows || []) {
      ...
      if (!agentState) {
        ...
        const userResult = await pool.query(
          `SELECT u.id, u.username, ast.agent_status AS current_agent_status, u.max_concurrent_calls, u.available_for_routing
             FROM users u
        ...
        agentState = {
          ...
          agentStatus: user.current_agent_status || "Available",
          currentCallsCount: 0,
...
async function loadStateFromDatabase() {
  try {
    ...
    const queueStates = await pool.query(`
      SELECT 
        qs.*,
        COUNT(DISTINCT i.id) FILTER (WHERE i.state IN ('queued', 'ringing', 'answered', 'connected', 'active') AND i.completed_at IS NULL AND i.abandoned_at IS NULL) as current_size,
        MIN(i.enqueued_at) FILTER (WHERE i.state IN ('queued', 'ringing') AND i.completed_at IS NULL AND i.abandoned_at IS NULL) as oldest_call_enqueued_at
      FROM cc_queue_state qs
      INNER JOIN cc_queues q ON qs.queue_id = q.id
      LEFT JOIN cc_interactions i ON i.queue_id = qs.queue_id
    ...
    const agentStates = await pool.query(`
      ...
      INNER JOIN users u ON ast.user_id = u.id
      LEFT JOIN cc_interactions i ON i.agent_username = ast.username AND i.state IN ('ringing', 'answered', 'connected', 'active')
      WHERE ast.agent_status != 'Offline'
...
async function syncStateToDatabase() {
  ...
  while (retryCount < maxRetries) {
    ...
    try {
      ...
      // Agent status/state is DB-authoritative for multi-node HA. Do not write
      // node-local agent cache snapshots back to cc_agent_state here; that can
      // overwrite a fresher status written by another node. Agent transitions
      // must go through the centralized DB writer/lifecycle helpers.
...
export function completeCall(
  ...
  const failedRinging = Boolean(options.failedRinging);
...
/**
 * Get queued interactions for the provided queues from Postgres.
 *
 * The queued-call router must not depend on process-local stateCache here: in
 * HA/multi-instance deployments the webhook that enqueues a call and the
 * coordinator that re-evaluates routing may run in different processes. DB is
 * the source of truth for offer decisions; the cache is only a realtime/UI
 * optimization.
 */
export async function getQueuedInteractionsForQueuesFromDatabase(queueIds, opts = {}) {
  const pool = opts.pool || getPostgresPool();
  if (!pool) return [];

  const targetIds = Array.isArray(queueIds)
  ...
  const result = await pool.query(
    `SELECT
     ...
     WHERE state = 'queued'
       ...
       AND queue_id = ANY($1::text[])
       AND COALESCE(metadata->>'is_consult_call', 'false') <> 'true'
     ORDER BY COALESCE(priority, 3) DESC, enqueued_at ASC NULLS LAST, created_at ASC`,
    [targetIds],
  );
...
if (typeof window === "undefined") {
  ...
  // WS3-T2: start the singleton coordinator (advisory-lock leader election).
  // Inert unless COORDINATOR_SINGLETON=true; existing lease-gated intervals
  // above remain the safety backstop either way.
  import("./coordinator.js")
    .then(({ startCoordinator }) => startCoordinator())
    .catch(() => {});
}
