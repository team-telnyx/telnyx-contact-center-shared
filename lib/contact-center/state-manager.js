...
let routingReEvalInterval = null;
  ...
  try {
    ...
    const agentsResult = await pool.query(
      `SELECT DISTINCT ast.user_id
       ...
       WHERE ast.agent_status IN ('Available', 'Busy')
...
export function initializeStateManager() {
  ...
  routingReEvalInterval = setInterval(async () => {
    try {
      await reEvaluateRoutingForAvailableAgents();
    } catch (error) {
      console.error(
  ...
  // Start periodic agent answer timeout checking (every 5 seconds)
  ...
  if (typeof global !== "undefined") {
    global.agentAnswerTimeoutInterval = timeoutCheckInterval;
  }
}
...
export function stopStateManager() {
  ...
  if (routingReEvalInterval) {
    clearInterval(routingReEvalInterval);
...
export function updateAgentStatus(userId, status, username) {
  ...
  if (!agentState) {
    ...
    // Set available_since when status changes to "Available" and agent has no calls
    if (
      status === "Available" &&
