...
export async function bridgeCallToAgent(
  ...
  try {
    ...
    // Update the interaction record with the agent's call control ID
    // IMPORTANT: Update the interaction's call_control_id to the agent's call_control_id
    // This ensures that subsequent webhooks (call.bridged, call.hangup) can find the interaction
    // The agent's call_control_id is the one that will be used in future webhooks
    // Store the original call_control_id in metadata for issuing commands
    if (interaction) {
      try {
        const metadata = interaction.metadata || {};
        metadata.original_call_control_id = queuedCallControlId; // Store original for reference

        // If we have the agent's call_control_id, update the interaction's call_control_id
        if (agentCallControlId) {
          metadata.agent_call_control_id = agentCallControlId;
          await PgDb.updateInteractionById(interaction.id, {
            callControlId: agentCallControlId, // Update to agent's call_control_id
            metadata,
          });
          console.log(
            `[WebRTCTransfer] Updated interaction call_control_id: ${queuedCallControlId} → ${agentCallControlId}`
          );
        } else {
          // Store original in metadata even if agent call_control_id not yet available
          await PgDb.updateInteractionById(interaction.id, {
            metadata,
          });
          console.log(
            `[WebRTCTransfer] Stored original_call_control_id in metadata: ${queuedCallControlId}`
          );
        }
      } catch (err) {
        console.error(
