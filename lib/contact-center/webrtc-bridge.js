      ...
      try {
        ...
        if (sttConfig?.enabled) {
          ...
          prewarmTelnyxSttAgentLeg(
            ...
            console.warn("[WebRTCBridge] Telnyx STT agent-leg prewarm failed:", err.message);
          });
        }
      } catch (err) {
        console.warn("[WebRTCBridge] Telnyx STT agent-leg prewarm check failed:", err.message);
      }
    }
