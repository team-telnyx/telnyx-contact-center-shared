...
export async function cleanupGhostCalls() {
  ...
  try {
        ...
        batch.map(async (interaction) => {
          try {
            ...
            if (ageHours > MAX_ACTIVE_AGE_HOURS) {
              // Mark as abandoned if never answered, completed if answered
              shouldClean = true;
              finalState = interaction.answered_at ? "completed" : "abandoned";
              cleanupReason = interaction.call_control_id ? "stale" : "noCallControlId";
              console.log(
            ...
            if (shouldClean) {
              ...
              if (finalState === "completed") {
                ...
                if (interaction.answered_at) {
                  ...
                  const talkTimeSeconds = Math.floor(
              ...
              try {
                const { completeCall } = await import(
    ...
    if (cleaned > 0) {
      ...
      summaryLines.push(`[GhostCallCleanup] Records cleaned up by final state:`);
