...
export async function getSkillsMapping() {
  ...
  try {
    ...
    routingLogger.error("skills_mapping_load_failed", contactCenterErrorPayload(error));
    return new Map();
  }
}
...
export function getAdjustedSkillRequirements(
  ...
  routingLogger.debug("skill_relaxation_evaluated", {
    waitTimeSeconds,
    ...queuePayload({ queueId: queue.id, queueName: queue.name }),
    skillCount: Object.keys(requiredSkills || {}).length,
    skillRelaxationEnabled: queue.skill_relaxation_enabled,
    skillRelaxationAfterSeconds: queue.skill_relaxation_after_seconds,
    skillRelaxationStrategy: queue.skill_relaxation_strategy,
  });

  if (!queue.skill_relaxation_enabled) {
    routingLogger.debug("skill_relaxation_disabled", queuePayload({ queueId: queue.id, queueName: queue.name }));
    return requiredSkills;
  }

  if (waitTimeSeconds < queue.skill_relaxation_after_seconds) {
    routingLogger.debug("skill_relaxation_not_due", {
      ...queuePayload({ queueId: queue.id, queueName: queue.name }),
      waitTimeSeconds,
      thresholdSeconds: queue.skill_relaxation_after_seconds,
    });
  ...
  if (strategy === "fallback") {
    ...
    routingLogger.debug("skill_relaxation_applied", {
      ...queuePayload({ queueId: queue.id, queueName: queue.name }),
      strategy: "fallback",
      originalSkillCount: Object.keys(requiredSkills || {}).length,
      adjustedSkillCount: Object.keys(adjusted || {}).length,
    });
  } else {
    ...
    routingLogger.debug("skill_relaxation_progressive_calculated", {
      ...queuePayload({ queueId: queue.id, queueName: queue.name }),
      waitTimeSeconds,
      thresholdSeconds: queue.skill_relaxation_after_seconds,
      elapsedAfterThresholdSeconds: waitTimeSeconds - queue.skill_relaxation_after_seconds,
      relaxationAmount,
    });
...
async function routeSkillsBased(queue, callData) {
  ...
  if (callData.enqueued_at) {
    ...
    routingLogger.debug("skills_based_relaxation_started", {
      ...queuePayload({ queueId: queue.id, queueName: queue.name }),
      ...callPayload(callData),
      enqueuedAt: callData.enqueued_at,
      evaluatedAt: new Date(now).toISOString(),
      waitTimeSeconds,
      requiredSkillCount: Object.keys(requiredSkills || {}).length,
      skillRelaxationEnabled: queue.skill_relaxation_enabled,
      skillRelaxationAfterSeconds: queue.skill_relaxation_after_seconds,
      skillRelaxationStrategy: queue.skill_relaxation_strategy,
    });
    ...
    routingLogger.debug("skills_based_relaxation_completed", {
      ...queuePayload({ queueId: queue.id, queueName: queue.name }),
      ...callPayload(callData),
      requiredSkillCount: Object.keys(requiredSkills || {}).length,
    });
  } else {
    routingLogger.debug("skills_based_relaxation_skipped", {
      ...queuePayload({ queueId: queue.id, queueName: queue.name }),
      ...callPayload(callData),
      reason: "missing_enqueued_at",
    });
  }
...
export async function calculateExpectedWaitTime(queueId) {
  try {
    ...
    routingLogger.error("expected_wait_time_failed", contactCenterErrorPayload(error));
    return null;
  }
}
