...
async function routeFIFO(queue, callData) {
  ...
  if (availableAgents.length === 0) {
    ...
    if (busyAgents.length > 0) {
      // Sort by available_since (LAA) if available, otherwise by call count
      busyAgents.sort((a, b) => {
        const aTime = a.available_since
          ? new Date(a.available_since).getTime()
          : 0;
        const bTime = b.available_since
...
export async function routeCall(queueId, callData = {}) {
  try {
    ...
    console.log(
      `[RoutingEngine] routeCall for queue ${queue.name} (${queueId}):`,
      {
        routingStrategy,
        ...
        callData: {
          hasEnqueuedAt: !!callData.enqueued_at,
          enqueuedAt: callData.enqueued_at,
          hasRequiredSkills: !!(
            callData.required_skills &&
            Object.keys(callData.required_skills).length > 0
          ),
          requiredSkills: callData.required_skills,
          priority: callData.priority,
          allCallDataKeys: Object.keys(callData),
        },
      },
    );
