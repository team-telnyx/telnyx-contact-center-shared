...
import {
  markAgentBusyForRinging,
  restoreAgentAvailableAfterFailedRinging,
} from "./agent-status-transition.js";

const ROUTING_DIAG_PREFIX = "[QueuedCallRouter][RoutingDiagnostics]";

function routingDiag(message, details = {}) {
  try {
    console.log(`${ROUTING_DIAG_PREFIX} ${message}`, {
      ...details,
...
function routingDiagError(message, details = {}) {
  try {
    console.error(`${ROUTING_DIAG_PREFIX} ${message}`, {
      ...details,
      timestamp: new Date().toISOString(),
    });
  } catch (_) {}
...
async function assignQueuedInteractionToAgent(
  ...
  await markAgentBusyForRinging({ userId: agent.id, username: agent.username });
  ...
  try {
    ...
    await restoreAgentAvailableAfterFailedRinging({
      userId: agent.id,
      username: agent.username,
    });
    await pool.query(
