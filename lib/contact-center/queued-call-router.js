...
export async function offerQueuedCallForAgent({
  ...
  const queueConfigs = new Map(
    queueResult.rows.map((row) => {
      ...
      console.log(`[QueuedCallRouter] Queue config for ${config.name}:`, {
        id: config.id,
        routingStrategy: config.routingStrategy,
        skillRelaxationEnabled: config.skillRelaxationEnabled,
        skillRelaxationAfterSeconds: config.skillRelaxationAfterSeconds,
        skillRelaxationStrategy: config.skillRelaxationStrategy,
        raw_enabled: row.skill_relaxation_enabled,
        raw_after_seconds: row.skill_relaxation_after_seconds,
        raw_strategy: row.skill_relaxation_strategy,
      });
  ...
  for (const interaction of validInteractions) {
    try {
      ...
      console.log(
        `[QueuedCallRouter] Processing interaction ${interaction.id} for queue ${queueConfig.name}:`,
        {
          routingStrategy,
          hasRequiredSkills: Object.keys(requiredSkills).length > 0,
          requiredSkills,
          enqueuedAt: interaction.enqueued_at,
          skillRelaxationEnabled: queueConfig.skillRelaxationEnabled,
        },
      );
      ...
      if (routingStrategy === "Skill-based") {
        if (requiredSkills && Object.keys(requiredSkills).length > 0) {
          ...
          if (interaction.enqueued_at && queueConfig.skillRelaxationEnabled) {
            ...
            console.log(
              `[QueuedCallRouter] Calculating skill relaxation for interaction ${interaction.id}:`,
              {
                enqueuedAt: interaction.enqueued_at,
                now: new Date(now).toISOString(),
                waitTimeSeconds,
                threshold: queueConfig.skillRelaxationAfterSeconds,
                strategy: queueConfig.skillRelaxationStrategy,
            ...
            console.log(
              `[QueuedCallRouter] Skill relaxation result for interaction ${interaction.id}:`,
              {
                originalSkills: requiredSkills,
                relaxedSkills: skillsToCheck,
                wasRelaxed:
                  JSON.stringify(requiredSkills) !==
                  JSON.stringify(skillsToCheck),
              },
            );
          } else {
            console.log(
              `[QueuedCallRouter] Skill relaxation skipped for interaction ${interaction.id}:`,
              {
                hasEnqueuedAt: !!interaction.enqueued_at,
                skillRelaxationEnabled: queueConfig.skillRelaxationEnabled,
              },
            );
          }
          ...
          if (
            ...
            console.log(
              `[QueuedCallRouter] Agent ${agent.username} does not match skills for interaction ${interaction.id}:`,
              {
          ...
          console.log(
            `[QueuedCallRouter] Agent ${agent.username} matches skills for interaction ${interaction.id}`,
          );
        }
      }
      ...
      console.error(
        `[QueuedCallRouter] Error checking interaction ${interaction?.id}:`,
        error,
      );
