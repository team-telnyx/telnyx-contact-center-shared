...
async function getTranscriptionConfigForCall(callControlId) {
  ...
  try {
    ...
    console.warn(
      "[SpeakQueue] Failed to resolve transcription config:",
      err?.message || err,
    );
  }
  return null;
...
async function getTranscriptionCallLegs(callControlId) {
  ...
  try {
    ...
    console.warn(
      "[SpeakQueue] Failed to resolve call legs for transcription:",
      err?.message || err,
    );
  }
  ...
  try {
    ...
    await Promise.all(
      legIds.map(async (legId) => {
        ...
        console.log("[SpeakQueue] Transcription stopped for", legId);
      })
    );
  } catch (err) {
...
async function resumeTranscription(callControlId) {
  try {
    ...
    if (!transcriptionConfig?.transcription_engine) {
      console.warn(
        "[SpeakQueue] Missing transcription config; cannot resume transcription for",
        callControlId,
      );
      return;
    }
    ...
    await Promise.all(
      legIds.map(async (legId) => {
        ...
        console.log("[SpeakQueue] Transcription resumed for", legId);
      })
    );
  } catch (err) {
