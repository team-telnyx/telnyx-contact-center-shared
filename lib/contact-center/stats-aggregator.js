  ...
  try {
    ...
    const stats = await Promise.all(
      queues.map(async (queue) => {
        ...
        // Get real-time metrics from interactions
        const realtimeQuery = `
          SELECT 
            (SELECT COUNT(*) FROM cc_interactions WHERE queue_id = $1 AND state IN ('queued', 'ringing')) as waiting_calls,
            (SELECT COUNT(*) FROM cc_interactions WHERE queue_id = $1 AND state IN ('answered', 'connected', 'active')) as active_calls,
            (SELECT COUNT(*) FROM cc_interactions WHERE queue_id = $1 AND state = 'queued') as queued_calls,
            (SELECT COUNT(*) FROM cc_interactions WHERE queue_id = $1 AND state = 'ringing') as ringing_calls,
            COALESCE((
              SELECT MIN(EXTRACT(EPOCH FROM (NOW() - enqueued_at)))::INTEGER
              FROM cc_interactions
              WHERE queue_id = $1 AND state IN ('queued', 'ringing') AND enqueued_at IS NOT NULL
            ), 0) as longest_wait_seconds,
            COALESCE((
              SELECT AVG(EXTRACT(EPOCH FROM (NOW() - enqueued_at)))::NUMERIC(10,2)
              FROM cc_interactions
              WHERE queue_id = $1 AND state IN ('queued', 'ringing') AND enqueued_at IS NOT NULL
            ), 0) as avg_wait_seconds
        `;

        const realtimeResult = await pool.query(realtimeQuery, [qId]);
        ...
        return {
          ...
          realtime: {
            currentSize: parseInt(realtime.waiting_calls) || 0,
            waitingCalls: parseInt(realtime.waiting_calls) || 0,
            activeCalls: parseInt(realtime.active_calls) || 0,
            queuedCalls: parseInt(realtime.queued_calls) || 0,
            ringingCalls: parseInt(realtime.ringing_calls) || 0,
            longestWaitSeconds: parseInt(realtime.longest_wait_seconds) || 0,
            avgWaitSeconds: parseFloat(realtime.avg_wait_seconds) || 0,
          },
          today: {
  ...
  try {
    ...
    if (userId) {
      const agentResult = await pool.query(
        `SELECT u.id, u.username, u.first_name, u.last_name, u.agent_status, u.max_concurrent_calls, u.available_for_routing
         FROM users u
    ...
    const stats = await Promise.all(
      agents.map(async (agent) => {
        ...
        const activeQuery = `
          ...
          FROM cc_interactions
          WHERE agent_username = $1
            AND state IN ('ringing', 'answered', 'connected', 'active')
        `;

        const activeResult = await pool.query(activeQuery, [username]);
        const activeCalls = parseInt(activeResult.rows[0]?.active_calls) || 0;
  ...
  try {
    ...
    const callsQuery = `
      SELECT 
        ...
        COUNT(*) FILTER (WHERE state IN ('queued', 'ringing', 'answered', 'connected', 'active')) as active_calls,
      ...
      WHERE DATE(created_at) = CURRENT_DATE
        OR state IN ('queued', 'ringing', 'answered', 'connected', 'active')
    `;
    ...
    const agentsQuery = `
      SELECT 
        COUNT(DISTINCT u.id) FILTER (WHERE u.agent_status = 'Available' AND EXISTS (
        ...
        COUNT(DISTINCT u.id) FILTER (WHERE u.agent_status = 'Busy' AND EXISTS (
          SELECT 1 FROM cc_queue_user_assignments qa 
          WHERE qa.user_id = u.id 
            AND qa.enabled = true 
            AND qa.activated_at IS NOT NULL 
        ...
        COUNT(DISTINCT u.id) FILTER (WHERE u.agent_status IN ('Available', 'Busy') AND EXISTS (
          SELECT 1 FROM cc_queue_user_assignments qa 
          WHERE qa.user_id = u.id 
            AND qa.enabled = true 
            AND qa.activated_at IS NOT NULL 
    ...
    const queuesQuery = `
      SELECT 
        ...
        COALESCE(SUM(
          (SELECT COUNT(*) FROM cc_interactions i 
           WHERE i.queue_id = q.id 
             AND i.state IN ('queued', 'ringing'))
        ), 0) as total_waiting_calls
      FROM cc_queues q
    `;
    ...
    return {
      calls: {
        ...
        active: parseInt(calls.active_calls) || 0,
      ...
      queues: {
        ...
        totalWaitingCalls: parseInt(queues.total_waiting_calls) || 0,
      },
      lastUpdated: new Date().toISOString(),
    };
  } catch (error) {
