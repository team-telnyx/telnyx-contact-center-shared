  ...
  try {
    ...
    const stats = await Promise.all(
      agents.map(async (agent) => {
        ...
        // Get current active calls from the same DB-authoritative live-call
        // model used by expanded agent rows. Do not mix cc_agent_state status
        // with node-local stateCache counts, or Supervisor can show impossible
        // states like Available + 1/1 while the DB row says no active calls.
        const activeCallsResult = await pool.query(
          `SELECT COUNT(DISTINCT i.id)::int AS active_calls
        ...
        const activeCalls = Number(activeCallsResult.rows?.[0]?.active_calls || 0);
        ...
        try {
          ...
          if (error?.code !== "42P01") contactCenterRuntimeLogger.error("runtime_error", { ...runtimePayload({ error: typeof error !== "undefined" ? error : typeof err !== "undefined" ? err : undefined, status: typeof status !== "undefined" ? status : undefined }) });
        }
...
export async function updateHourlyStatistics() {
  try {
    ...
    contactCenterRuntimeLogger.info("runtime_diagnostic", { ...runtimePayload({ error: typeof error !== "undefined" ? error : typeof err !== "undefined" ? err : undefined, status: typeof status !== "undefined" ? status : undefined }) });
  } catch (error) {
    contactCenterRuntimeLogger.error("runtime_error", { ...runtimePayload({ error: typeof error !== "undefined" ? error : typeof err !== "undefined" ? err : undefined, status: typeof status !== "undefined" ? status : undefined }) });
  }
}
