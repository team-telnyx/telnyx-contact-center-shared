...
async function withClient(poolOrClient, fn) {
  if (typeof poolOrClient.query === "function" && typeof poolOrClient.connect !== "function") {
    ...
    try {
      ...
      const result = await db.query(
        `WITH locked_agent AS (
         ...
         SELECT $1, locked_agent.id, $3, $4, $5, $6, $7, 'reserved', now(), now() + ($8::text || ' milliseconds')::interval
         FROM locked_agent
         WHERE (
             SELECT COUNT(*)
             FROM cc_agent_reservations r
             WHERE r.agent_id = locked_agent.id
               AND r.state IN ('reserved', 'ringing', 'active')
               AND r.lease_expires_at > now()
           ) < locked_agent.max_concurrent_calls
         RETURNING id`,
        [id, agentId, channel, interactionId, attemptId, queueId, campaignId, leaseMs],
      );
