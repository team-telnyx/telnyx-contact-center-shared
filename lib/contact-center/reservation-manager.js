...
async function restoreAgentAfterExpiredReservation(agentId) {
  try {
    const { restoreAgentAvailableAfterFailedRinging } = await import("./agent-status-transition.js");
    await restoreAgentAvailableAfterFailedRinging({ userId: agentId });
  } catch (error) {
    console.error("[ReservationManager] Failed to restore agent status after reservation expiry:", {
      agentId,
  ...
  return withClient(client || pool, async (db) => {
    ...
    for (const agentId of agentIds) {
      ...
      if (activeReservations === 0) {
        await restoreAgentAfterExpiredReservation(agentId);
      }
    }
    return agentIds;
  });
