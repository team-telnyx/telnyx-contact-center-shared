      ...
      `INSERT INTO cc_agent_state (user_id, username, agent_status, last_status_change, last_activity, available_since)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         available_since = CASE
           WHEN EXCLUDED.agent_status = 'Available' THEN COALESCE(cc_agent_state.available_since, NOW())
           ELSE NULL
         END`,
...
export async function setUserStatus({
  ...
  if (pool) {
    ...
    // Rules:
    // 1. Preserve when transitioning TO break statuses (type = 'break'), "Agent Not Answering", or "Offline"
    ...
    // 4. For all other transitions to "Available", preserve existing available_since (or set if null)
    ...
    await pool.query(
      `INSERT INTO cc_agent_state (user_id, username, agent_status, last_status_change, last_activity, available_since)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         last_activity = NOW(),
         available_since = CASE 
           -- Preserve available_since when transitioning TO break statuses (type = 'break'), "Agent Not Answering", or "Offline"
           ...
           -- Preserve available_since when transitioning from break statuses, "Agent Not Answering", or "Offline" to "Available"
