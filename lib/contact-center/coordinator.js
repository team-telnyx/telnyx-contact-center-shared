...
import { allowsWorkerRole, getProcessRole } from "../runtime/process-role.mjs";
...
export function isCoordinatorEnabled() {
  return (
    ...
    && allowsWorkerRole(getProcessRole())
  );
}

async function runLeaderLoop({ signal }) {
  const { sweepExpiredReservations } = await import("./reservation-manager.js");
  const { sweepExpiredSessionPresence } = await import("./session-presence.js");
  const { pruneProcessedEvents } = await import("../events/idempotency.js");
  const { startRoutingReactor, stopRoutingReactor } = await import(
    "./routing-reactor.js"
  );
  // WS5-T3: Power pacing loop (no-op unless OUTBOUND_POWER_PACING=true).
  const { startPowerPacingLoop, stopPowerPacingLoop } = await import(
    "../outbound-dialer/pacing-power.js"
  );
  // WS6: Predictive pacing loop (no-op unless OUTBOUND_PREDICTIVE_PACING=true).
  const { startPredictivePacingLoop, stopPredictivePacingLoop } = await import(
    "../outbound-dialer/pacing-predictive.js"
  );

  await startRoutingReactor();
  await startPowerPacingLoop({ signal });
  await startPredictivePacingLoop({ signal });
  ...
  while (!signal.aborted) {
    ...
    try {
      const expiredPresence = await sweepExpiredSessionPresence();
      if (expiredPresence?.offlineCount > 0 || expiredPresence?.deletedCount > 0) {
        coordinatorLogger.info("coordinator_session_presence_sweep", expiredPresence);
      }
    } catch (err) {
      coordinatorLogger.warn("coordinator_session_presence_sweep_failed", {
        error: err?.message || String(err),
      });
    }
  ...
  await stopPredictivePacingLoop();
  await stopPowerPacingLoop();
