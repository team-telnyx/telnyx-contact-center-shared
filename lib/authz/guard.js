...
export function withPermission(permission, handler, options = {}) {
  return async (request, context) => {
    try {
      ...
      let response;
      try {
        if (options.voiceControl && !authz.apiKey) {
          const { withVoiceControl } = await import("@/lib/acd/voice-endpoints.mjs");
          const { getPostgresPool } = await import("@/lib/postgres.mjs");
          response = await withVoiceControl(getPostgresPool(), authz.user, handlerRequest, () => handler(handlerRequest, context, authz));
        } else response = await handler(handlerRequest, context, authz);
      }
      ...
      if (!authz.apiKey && response?.headers?.get("content-type")?.includes("text/event-stream") && response.body) {
        ...
        return guardEventStream(response, {
          ...
          authorize: async () => {
            ...
            if (authz.user.authSessionId) {
              const { hasAuthSession } = await import("@/lib/auth-refresh-sessions.mjs");
              if (request.headers.get('authorization') && !hasAuthSession(user,authz.user.authSessionId)) return false;
            }
            const access = await effectiveAccess(user);
