...
export async function createTelephonyCredential(apiKey, options) {
  ...
  try {
    ...
    let data;
    console.log("Telephony credentials response", JSON.parse(responseText));
    try {
    ...
    console.error("[Telnyx] Failed to create telephony credential:", {
      error: error.message,
      name,
      connectionId,
    });
...
export async function createUserTelephonyCredentials(userInfo) {
  ...
  if (!apiKey) {
    console.warn(
      "[Telnyx] TELNYX_API_KEY not configured, skipping credential creation"
    );
    return null;
  }
  ...
  if (process.env.TELNYX_AUTO_CREATE_CREDENTIALS === "false") {
    console.log("[Telnyx] Auto credential creation disabled");
    return null;
  }
  ...
  try {
    ...
    console.log("[Telnyx] Created telephony credential for user:", {
      email,
    ...
    console.error(
      "[Telnyx] Failed to create user telephony credentials:",
      error.message
    );
