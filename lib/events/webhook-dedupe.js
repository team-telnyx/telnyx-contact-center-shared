 ...
 *   L2 — DB-backed idempotency via voice_webhook_dedupe (cross-node /
...
async function dbClaim(key, kind) {
  ...
  const result = await pool.query(
    `INSERT INTO voice_webhook_dedupe (event_id, kind)
     VALUES ($1, $2)
...
async function dbHas(key) {
  ...
  if (!pool) return null;
  const result = await pool.query(
    `SELECT 1 FROM voice_webhook_dedupe WHERE event_id = $1 LIMIT 1`,
    [key],
  );
...
function maybePruneDb() {
  ...
  pool
    .query(
      `DELETE FROM voice_webhook_dedupe
