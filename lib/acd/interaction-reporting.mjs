...
export async function readInteractionReport(db, scope, report = "overview") {
  ...
  const results = await Promise.all([
    db.query(
      ...
      SUM(handling_seconds) AS handling_seconds,COUNT(handling_seconds)::int AS handling_count,
  ...
  const series = channels.map((channel) => {
    ...
    return {
      ...
      handlingSeconds: number(row.handling_seconds),
      handlingCount: row.handling_count || 0,
      totalTalkSeconds: definition.capabilities.recordings ? number(row.talk_seconds) : null,
  ...
  // Aggregate measured handling durations, not averages weighted by all closed work:
  // unanswered/abandoned interactions may have no agent segment at all.
  const handlingCount = series.reduce((sum, row) => sum + row.handlingCount, 0);
  totals.avgHandlingSeconds = handlingCount > 0
    ? series.reduce((sum, row) => sum + (row.handlingSeconds || 0), 0) / handlingCount
    : null;
  totals.totalTalkSeconds = series.some(row => channelDefinition(row.channel).capabilities.recordings)
    ? series.reduce((sum, row) => sum + (row.totalTalkSeconds || 0), 0)
    : null;
  return {
    version: 1,
