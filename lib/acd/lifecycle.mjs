...
export async function openSegment(db, segment) {
  ...
  if (kind === "agent" && agentId) {
    // Freeze the media destination at the handling segment, never infer it from
    // the later Calls On preference when the agent files the disposition.
    const owned = await db.query(`UPDATE acd_segments s SET media_endpoint_id=p.endpoint_id,
      wrapup_endpoint_id=p.endpoint_id FROM cc_agent_voice_preferences p, acd_work_items w
      WHERE s.id=$1 AND p.agent_id=s.agent_id AND w.id=s.work_item_id AND w.channel IN ('voice','video')
      RETURNING s.*`, [result.rows[0].id]);
    if (owned.rows[0]) result.rows[0] = owned.rows[0];
  }
  if (kind === "queue_wait") {
