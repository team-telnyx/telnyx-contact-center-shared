...
export async function tryReserveCapacity(db, {
  ...
  // All occupancy, workflow and live voice-session checks below still apply.
  ...
  const live = (await db.query(`SELECT COALESCE(SUM(weight), 0)::numeric AS weight, COUNT(*)::int AS count,
    COUNT(*) FILTER (WHERE channel = $2)::int AS channel_count, BOOL_OR(channel = 'voice') AS voice
  ...
  if (profile.exclusive ? live.count > 0 || agent.workflow_state !== "idle" : live.voice || !["idle", "offered", "handling"].includes(agent.workflow_state)) return null;
  const budget = profile.exclusive ? Number(agent.capacity) : Math.min(Number(agent.capacity), 1);
  if (Number(live.weight) + amount > budget + 0.00001 || live.channel_count >= policy.maxConcurrent) return null;
