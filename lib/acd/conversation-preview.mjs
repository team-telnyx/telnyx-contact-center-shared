...
export async function readConversationPreview(
  ...
  const drafts = email && work.can_preview_drafts && !work.terminal_at && work.state === "active"
    ? (await db.query(`SELECT d.agent_id,d.draft_id,d.draft_scope,d.version::text,d.updated_at,
      ...
      WHERE d.work_item_id=$1 AND jsonb_typeof(d.content)='object'
        ...
        .map(draft => ({...draft,id:`draft:${work.id}:${draft.agent_id}${draft.draft_scope==='legacy'?'':`:${draft.draft_scope}`}${draft.draft_id==='legacy'?'':`:${draft.draft_id}`}`,sender_role:'agent',status:'draft',
          attachments:draft.attachments.map((file,index)=>({...file,
            url:`/api/contact-center/interactions/${work.id}/conversation/email-drafts/${encodeURIComponent(draft.agent_id)}/attachments/${index}?version=${draft.version}${draft.draft_scope==='legacy'?'':`&draftScope=${encodeURIComponent(draft.draft_scope)}`}${draft.draft_id==='legacy'?'':`&draftId=${encodeURIComponent(draft.draft_id)}`}`}))}))
    : [];
