import { ensureMobileAlertSchema } from "../mobile/alerts.mjs";
// ACD Core schema — the internal documentation §3.
// Idempotent Core-only schema, safe to run on every boot.

import { ensureVoiceEndpointSchema } from "./voice-endpoints.mjs";
...
export async function ensureAcdSchema(client) {
  ...
  await ensureVoiceEndpointSchema(client);
  await ensureMobileAlertSchema(client);
