...
const artifactLogger = createDiagnosticLogger("platform.db");

// Module tables that carried a pre-ACD-Core `interaction_id` link and are now
// keyed on acd_work_items. Each is attached to Core by the FK named here.
const WORK_ITEM_LINKED_TABLES = Object.freeze([
  { table: "form_submissions", constraint: "form_submissions_work_item_fk" },
  { table: "aa_workflow_sessions", constraint: "aa_workflow_sessions_work_item_fk" },
  { table: "aa_ai_handoff_events", constraint: "aa_ai_handoff_events_work_item_fk" },
  { table: "quality_evaluations", constraint: "quality_evaluations_work_item_fk" },
  { table: "quality_ai_jobs", constraint: "quality_ai_jobs_work_item_fk" },
]);

/**
 * Rows written before the ACD Core cutover point at the retired voice-runtime
 * table Core does not have. They can satisfy neither the work-item FK nor the
 * NOT NULL that follows it, so the link could never be attached and the
 * bootstrap would fail on every boot. Drop them on the single pass that
 * attaches the constraint, and record how many went so the logs show what the
 * upgrade discarded.
 */
async function dropRowsWithoutWorkItem(db) {
  for (const { table, constraint } of WORK_ITEM_LINKED_TABLES) {
    const state = await db.query(
      `SELECT to_regclass($1) IS NOT NULL AS present,
              EXISTS (SELECT 1 FROM pg_constraint WHERE conname = $2) AS linked`,
      [`public.${table}`, constraint],
    );
    const { present, linked } = state.rows[0] || {};
    if (!present || linked) continue;
    const removed = await db.query(
      `DELETE FROM ${table} legacy
        WHERE legacy.work_item_id IS NULL
           OR NOT EXISTS (SELECT 1 FROM acd_work_items w WHERE w.id = legacy.work_item_id)`,
    );
    if (removed.rowCount > 0) {
      artifactLogger.warn("acd_legacy_rows_dropped", { table, rows: removed.rowCount });
    }
  }
}
...
export async function ensureAcdArtifactSchema(db) {
  ...
  await dropRowsWithoutWorkItem(db);
