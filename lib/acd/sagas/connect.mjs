...
async function startQueueAudioAfterRequeue(tx, ctx) {
  ...
  const [queueResult, positionResult] = [
    await (tx.query(`SELECT * FROM cc_queues WHERE id = $1`, [ctx.data.queueId])),
    await (tx.query(
  ...
  steps: {
    ...
    prepare_requeue: {
      run: async (tx, ctx) => {
        ...
        if (!rejected && ctx.data.voiceEndpointId) {
          // A failed push offer is evidence that the phone is no longer reachable.
          const invalidated = await tx.query(`UPDATE cc_voice_endpoints SET push_ready=false,expires_at=now()
            WHERE id=$1 AND push_ready=true RETURNING id`,[ctx.data.voiceEndpointId]);
          if (invalidated.rowCount) await tx.query(`UPDATE acd_agent_sessions SET capabilities=jsonb_set(capabilities,'{voice}','false'),expires_at=now() WHERE id=$1`,[ctx.data.voiceEndpointId]);
        }
...
export async function startConnectSaga(db, params) {
  ...
  const voiceEndpoint=(await db.query(`SELECT e.id,e.sip_username,p.generation FROM cc_agent_voice_preferences p
    JOIN cc_voice_endpoints e ON e.id=p.endpoint_id WHERE p.agent_id=$1`,[routeResult.agentId])).rows[0];
  return startSaga(db, {
    type: "connect",
    ...
    data: {
      ...
      customerProviderCallId,
      agentSipUri: voiceEndpoint ? `sip:${voiceEndpoint.sip_username}@sip.telnyx.com` : agentSipUri,
      voiceEndpointId: voiceEndpoint?.id || null,
      voiceGeneration: voiceEndpoint?.generation == null ? null : String(voiceEndpoint.generation),
      agentDisplayName,
      answerTimeoutMs,
      wrapupDeadlineMs,
      maxCallDurationMs,
