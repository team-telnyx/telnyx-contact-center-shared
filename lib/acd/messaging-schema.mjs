...
import { ensureCobrowseSchema } from "../cobrowse/schema.mjs";
...
export async function ensureMessagingSchema(db) {
  await db.query(`
    ...
    ALTER TABLE acd_text_drafts ADD COLUMN IF NOT EXISTS draft_scope TEXT NOT NULL DEFAULT 'legacy';
    DO $$ BEGIN
      IF (SELECT COUNT(*) FROM information_schema.key_column_usage WHERE table_schema=current_schema()
          AND table_name='acd_text_drafts' AND constraint_name='acd_text_drafts_pkey')=2 THEN
        ALTER TABLE acd_text_drafts DROP CONSTRAINT acd_text_drafts_pkey;
        ALTER TABLE acd_text_drafts ADD PRIMARY KEY(work_item_id,agent_id,draft_scope);
      END IF;
    END $$;
