import { apnsAlertConfiguration, drainMobileAlerts } from "../mobile/alerts.mjs";
...
export function startAcdWorker(pool, {
  ...
  const alertConfig = apnsAlertConfiguration();
  const alertTick = setInterval(async () => {
    if (stopped || alertsRunning || !alertConfig) return;
    alertsRunning = true;
    try { await drainMobileAlerts(pool); }
    catch (error) { onDrain?.([{ channel:'mobile_alerts', error:String(error.message).slice(0,150) }]); }
