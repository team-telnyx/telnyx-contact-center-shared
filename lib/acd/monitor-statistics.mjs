...
export async function readMonitorStatistics(pool, options = {}) {
  ...
  const [
    ...
    readLiveWorkload(pool, null, restriction),
    pool.query(
      `SELECT user_id,queue_id,enabled,activated_at,deactivated_at FROM cc_queue_user_assignments WHERE enabled=true`,
    ),
  ]);
  const [agentPolicies, queuePolicies, channelMembership] = await Promise.all([
    pool.query("SELECT * FROM cc_agent_channel_policies"),
    pool.query("SELECT * FROM cc_queue_channels"),
    pool.query("SELECT * FROM cc_agent_queue_channels WHERE enabled=false"),
  ]);
