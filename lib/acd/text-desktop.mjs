...
export async function readTextInteractions(db,agentId){
  ...
  return {interactions:rows.map(row=>({...row,interaction_type:row.channel,channel:row.channel,from_name:row.customer_name,
    from_number:row.customer_address,customer_address:row.customer_address,core_state:row.state,state:row.offer_id?"ringing":row.assignment_state==="wrapup"?"wrapup":"active"})),
    utilization:await readUtilization(db,"agent",agentId)};
}

export async function readTextDetail(db,{workItemId,agentId,channel="chat",draftScope="legacy"}){
  ...
  // Claim a pre-upgrade shared draft once. It remains on one device rather
  // than disappearing or being copied into two sendable composers.
  if(draftScope!=="legacy")await db.query(`UPDATE acd_text_drafts d SET draft_scope=$3
    WHERE d.work_item_id=$1 AND d.agent_id=$2 AND d.draft_scope='legacy'
      AND NOT EXISTS(SELECT 1 FROM acd_text_drafts own WHERE own.work_item_id=d.work_item_id
        AND own.agent_id=d.agent_id AND own.draft_scope=$3)`,[workItemId,agentId,draftScope]);
  const draft=(await db.query("SELECT body,version FROM acd_text_drafts WHERE work_item_id=$1 AND agent_id=$2 AND draft_scope=$3",[workItemId,agentId,draftScope])).rows[0]||{body:"",version:"0"};
  ...
  const codes=(await db.query(`SELECT id,name,to_jsonb(c)->>'icon' AS icon,to_jsonb(c)->>'color' AS color FROM cc_wrapup_codes c WHERE is_active=true AND
  ...
  const offer=(await db.query("SELECT id,deadline_at FROM acd_offers WHERE work_item_id=$1 AND agent_id=$2 AND state IN ('created','ringing') ORDER BY generation DESC LIMIT 1",[workItemId,agentId])).rows[0]||null;
  const assignment=(await db.query("SELECT state FROM acd_text_assignments WHERE work_item_id=$1 AND agent_id=$2 AND state<>'completed' ORDER BY (state='active') DESC LIMIT 1",[workItemId,agentId])).rows[0]||null;
  return {work,offer,assignment,messages,draft:{...draft,scope:draftScope},customerName:customer?.customer_name||(sms?sms.customer_address:whatsapp?whatsapp.customer_name||whatsapp.customer_address:"Website visitor"),agent,customerTyping:customer?.typing===true,wrapupCodes:codes,
...
export async function saveTextDraft(db,{workItemId,agentId,body,expectedVersion,draftScope="legacy"}){
  ...
  const result=await db.query(`INSERT INTO acd_text_drafts(work_item_id,agent_id,body,draft_scope) SELECT $1,$2,$3,$5 WHERE $4::bigint=0
    ON CONFLICT(work_item_id,agent_id,draft_scope) DO NOTHING RETURNING body,version`,[workItemId,agentId,body,expectedVersion,draftScope]);
  if(result.rowCount)return result.rows[0];
  const updated=await db.query(`UPDATE acd_text_drafts SET body=$3,version=version+1,updated_at=now()
    WHERE work_item_id=$1 AND agent_id=$2 AND version=$4 AND draft_scope=$5 RETURNING body,version`,[workItemId,agentId,body,expectedVersion,draftScope]);
