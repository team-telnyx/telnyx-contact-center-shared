...
export const PgDb = {
  async upsertUserByUsername(username, fields) {
    ...
    // Handle roles - support both single role (legacy) and roles array
    const roles = fields.roles
      ...
      ? [fields.role]
      : ["agent"];
    const primaryRole = roles[0] || "agent"; // Keep role field for backward compatibility

    const q = `
      INSERT INTO users (
        id, username, first_name, last_name, nick, language, status, theme, mobile,
        sms_number, voice_number, role, roles, verified, auth_strategy,
    ...
    if (activityType === "login") {
      ...
      if (duration > 0) {
        if (status === "Available") {
          ...
          updates.push(
            "status_on_queue_seconds = status_on_queue_seconds + $" + paramIndex
          );
          values.push(duration);
          paramIndex++;
