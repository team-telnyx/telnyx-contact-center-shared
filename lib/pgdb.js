...
export const PgDb = {
  async upsertUserByUsername(username, fields) {
    ...
    const q = `
      INSERT INTO users (
        id, username, first_name, last_name, nick, language, status, theme, mobile,
        sms_number, voice_number, role, verified, auth_strategy,
  ...
  async findQueueByName(name) {
    const pool = getPostgresPool();
    if (!pool) return null;
    const r = await pool.query("SELECT * FROM cc_queues WHERE name = $1", [
      name,
    ]);
    return r.rows?.[0] || null;
  },
  ...
  async insertQueue(doc) {
    ...
    const q = `
      INSERT INTO cc_queues (
        id, name, display_name, description, queue_type, owner_username,
        routing_strategy, max_wait_time_secs, max_size, priority, enabled,
    ...
    const vals = [
      id,
      doc.name,
      doc.displayName || doc.display_name,
      doc.description || null,
      doc.queueType || doc.queue_type || "shared",
      doc.ownerUsername || doc.owner_username || null,
      doc.routingStrategy || doc.routing_strategy || "acd",
  ...
  async findAgentQueueAssignment(agentUsername, queueId) {
    ...
    if (!pool) return null;
    const r = await pool.query(
      "SELECT * FROM cc_agent_queue_assignments WHERE agent_username = $1 AND queue_id = $2",
      [agentUsername, queueId]
    );
    return r.rows?.[0] || null;
  },
  ...
  async insertAgentQueueAssignment(doc) {
    ...
    const id = doc.id || randomUUID();
    const q = `
      INSERT INTO cc_agent_queue_assignments (
        id, agent_username, queue_id, priority, max_concurrent_interactions,
        enabled, activated_at, created_at, updated_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
      ON CONFLICT (agent_username, queue_id) 
      DO UPDATE SET 
        enabled = EXCLUDED.enabled,
        activated_at = CASE WHEN EXCLUDED.enabled THEN NOW() ELSE cc_agent_queue_assignments.activated_at END,
        deactivated_at = CASE WHEN NOT EXCLUDED.enabled THEN NOW() ELSE cc_agent_queue_assignments.deactivated_at END,
        updated_at = NOW()
      RETURNING id
    `;
    const vals = [
      ...
      doc.priority || 0,
      doc.maxConcurrentInteractions || doc.max_concurrent_interactions || 1,
      doc.enabled !== undefined ? doc.enabled : true,
  ...
  async listAgentQueueAssignments(agentUsername) {
    ...
    if (!pool) return [];
    const r = await pool.query(
      `SELECT aqa.*, q.name as queue_name, q.display_name, q.queue_type
       FROM cc_agent_queue_assignments aqa
       JOIN cc_queues q ON aqa.queue_id = q.id
       WHERE aqa.agent_username = $1
  ...
  async findAgentsInQueue(queueName, status = null) {
    ...
    if (!pool) return [];
    const q = `
      SELECT DISTINCT u.*, aqa.priority as queue_priority
      FROM users u
      JOIN cc_agent_queue_assignments aqa ON u.username = aqa.agent_username
      JOIN cc_queues q ON aqa.queue_id = q.id
      WHERE q.name = $1 AND aqa.enabled = true
  ...
  async findCustomerByPhoneNumber(phoneNumber) {
    ...
    // Normalize phone number: remove spaces, dashes, parentheses, and ensure consistent format
    const normalizePhone = (num) => {
      if (!num) return null;
      // Remove all non-digit characters except +
      let normalized = String(num).replace(/[^\d+]/g, "");
      // If it doesn't start with +, try to add it for international numbers
    ...
    // Search across all phone number fields in demo_customers table
    // Try both normalized and original format
    const r = await pool.query(
      `SELECT * FROM demo_customers 
       WHERE phone = $1 OR phone = $2
          OR mobile = $1 OR mobile = $2
          OR business_phone_1 = $1 OR business_phone_1 = $2
          OR business_phone_2 = $1 OR business_phone_2 = $2
          OR home_phone_1 = $1 OR home_phone_1 = $2
          OR home_phone_2 = $1 OR home_phone_2 = $2
       LIMIT 1`,
  ...
  async listAgentInteractions(agentUsernameOrFilter, filter = {}) {
    ...
    if (typeof agentUsernameOrFilter === "string") {
      ...
      console.error(
        "[PgDb] listAgentInteractions: invalid agentUsername",
        agentUsernameOrFilter
      );
      return [];
    }
    ...
    if (!agentUsername || typeof agentUsername !== "string") {
      console.error(
        "[PgDb] listAgentInteractions: agentUsername is not a string",
        agentUsername
      );
      return [];
    }
    ...
    // Exclude transfer legs (is_transfer_leg in metadata) and non-contact-center interactions
