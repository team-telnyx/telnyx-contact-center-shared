...
export const PgDb = {
  ...
  async insertCall(doc) {
    ...
    try {
      ...
      if (err.code === "42P01") {
        console.debug("[PgDb] Calls table not found, skipping call insert");
        return null;
      }
      throw err;
    }
  ...
  async updateCallByCallControlId(callControlId, set) {
    ...
    try {
      ...
      // Handle case where calls table doesn't exist (non-critical)
      if (err.code === "42P01") {
        console.debug("[PgDb] Calls table not found, skipping call update");
        return;
      }
      throw err;
    }
  ...
  async findAgentsInQueue(queueName, status = null) {
    ...
    if (!pool) return [];
    const q = `
      SELECT DISTINCT u.*, qa.priority as queue_priority
      FROM users u
  ...
  async findContactByPhoneNumber(phoneNumber) {
    ...
    if (!pool) {
      console.log(
        "[PgDb] findContactByPhoneNumber: No postgres pool available"
      );
      return null;
    }
    if (!phoneNumber) {
      console.log("[PgDb] findContactByPhoneNumber: No phone number provided");
      return null;
    }
    ...
    console.log(
      "[PgDb] findContactByPhoneNumber: Searching for contact with phone:",
      {
    ...
    if (contact) {
      console.log("[PgDb] findContactByPhoneNumber: ✅ Found contact:", {
        id: contact.id,
        ...
        business_phone_1: contact.business_phone_1,
        business_phone_2: contact.business_phone_2,
        home_phone_1: contact.home_phone_1,
        home_phone_2: contact.home_phone_2,
        matchedNumber: normalizedPhone,
      });
    } else {
      console.log(
        "[PgDb] findContactByPhoneNumber: ❌ No contact found for phone:",
        normalizedPhone
      );
    }
