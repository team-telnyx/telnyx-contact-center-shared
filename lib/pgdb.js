...
import { ensureAgentState } from "./acd/agent-state.mjs";
...
export const PgDb = {
  async upsertUserByUsername(username, fields) {
    ...
    await ensureAgentState(pool, String(userId));
    return userId;
  },
  ...
  async updateUserById(id, set, db = null) {
  ...
  async insertQueue(doc, db = null) {
    ...
    const q = `
      INSERT INTO cc_queues (
        ...
        agent_answer_timeout_secs, overflow_queue_id, overflow_action, priority, enabled,
    ...
    const vals = [
      ...
      doc.agentAnswerTimeoutSecs ?? doc.agent_answer_timeout_secs ?? null,
  ...
  async logUserActivity({
    ...
    const q = `
      INSERT INTO cc_user_activity_log (
        ...
        metadata, queue_id, started_at, ended_at,
