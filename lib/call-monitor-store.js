...
/**
 * Add a Telnyx Standalone STT WebSocket transcription event to the monitor.
 * These are intentionally NOT stored as `call.transcription` webhooks because
 * they originate from the direct Telnyx STT WebSocket bridge, not Voice API
 * in-call transcription webhooks.
 */
export function addTelnyxStandaloneSttEvent({
  callControlId,
  flowId = null,
  interactionId = null,
  transcript = "",
  ...
  const event = {
    id: `telnyx_stt_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    event_type: "telnyx_standalone_stt.transcription",
  ...
  events.push(event);
  // STT WebSocket interim results can be high-volume. Keep the monitor bounded
  // without affecting regular webhook/command history retention.
...
const cleanupOldCallsInterval = setInterval(cleanupOldCalls, 10 * 60 * 1000);
cleanupOldCallsInterval.unref?.();
