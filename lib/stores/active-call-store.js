    ...
    (set, get) => ({
      ...
      updateTranscriptionAnalysis: (transcriptionId, analysis) => {
        ...
        for (let i = state.transcriptions.length - 1; i >= 0; i -= 1) {
          if (
            state.transcriptions[i].originalTranscriptionKey === transcriptionId
          ) {
            targetIndex = i;
            break;
          }
        ...
        if (targetIndex < 0) {
          targetIndex = state.transcriptions.findIndex(
        ...
        if (targetIndex < 0) {
          for (let i = state.transcriptions.length - 1; i >= 0; i -= 1) {
            if (state.transcriptions[i].transcriptionKey === transcriptionId) {
              targetIndex = i;
              break;
            }
          }
