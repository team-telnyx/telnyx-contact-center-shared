  ...
  devtools(
    (set, get) => ({
      ...
      updateStatus: (status) => {
        ...
        if (shouldSyncMetrics) {
          get()
            ...
            .catch((err) => {
              console.error(
                "[ActiveCallStore] Failed to sync metrics after status update:",
                err
              );
            });
        }
      ...
      recordTransfer: ({ to, type = "external", callControlId = null }) => {
        ...
        if (!state.call) {
          console.warn(
            "[ActiveCallStore] recordTransfer called without active call"
          );
          return;
        }
        ...
        console.log(
          "[ActiveCallStore] Transfer recorded:",
          transferEntry,
          "Total transfers:",
      ...
      syncCallMetricsToDb: async () => {
        const state = get();
        if (!state.call) {
          console.log("[ActiveCallStore] No active call to sync metrics");
          return;
        }
          ...
          if (callControlId) {
            try {
              ...
              if (data.ok && data.interaction?.id) {
                ...
                console.log(
                  "[ActiveCallStore] Resolved interactionId via call_control_id:",
                  interactionId
                );
              }
            } catch (err) {
              console.error(
                "[ActiveCallStore] Failed to resolve interactionId:",
                err
              );
            }
          }
        ...
        if (!interactionId) {
          console.log(
            "[ActiveCallStore] Not a contact center call, skipping metrics sync"
          );
          return;
        }
        ...
        try {
          ...
          console.log("[ActiveCallStore] Syncing call metrics to DB:", metrics);
          ...
          if (!res.ok) {
            ...
            console.error("[ActiveCallStore] Failed to sync metrics:", error);
          } else {
            console.log(
              "[ActiveCallStore] ✅ Successfully synced call metrics to DB"
            );
          }
        } catch (err) {
          console.error("[ActiveCallStore] Error syncing call metrics:", err);
        }
      },
      ...
      syncAgentAssistToDb: async () => {
        const state = get();
        if (!state.call) {
          console.log("[ActiveCallStore] No active call to sync transcripts");
          return;
        }
        ...
        if (!interactionId) {
          ...
          if (callControlId) {
            try {
              ...
              console.error(
                "[ActiveCallStore] Failed to resolve interactionId for transcripts:",
                err
              );
            }
          }
        ...
        try {
          ...
          if (!res.ok) {
            const error = await res
              ...
              .catch(() => ({ error: "Unknown error" }));
            console.error(
              "[ActiveCallStore] Failed to sync transcripts:",
              error
            );
          } else {
            console.log(
              "[ActiveCallStore] ✅ Successfully synced transcripts to DB"
            );
          }
        } catch (err) {
          console.error("[ActiveCallStore] Error syncing transcripts:", err);
        }
      },
      ...
      clearActiveCall: () => {
        ...
        get()
          .syncCallMetricsToDb()
          .catch((err) => {
            console.error("[ActiveCallStore] Error syncing metrics:", err);
          });
        get()
          .syncAgentAssistToDb()
          .catch((err) => {
            console.error(
              "[ActiveCallStore] Error syncing transcription data:",
              err
            );
          });
