...
function mergeTranscriptionText(currentText = "", incomingText = "") {
  ...
  return `${currentText}${incomingText.startsWith(" ") ? "" : " "}${incomingText}`;
}
        ...
        if (existingIndex >= 0) {
          ...
          const updated = {
            ...
            transcript: isSpeechFinal
              ? incomingTranscript || existing.transcript
              : mergeTranscriptionText(existing.transcript, incomingTranscript),
            isFinal,
            track,
            callControlId,
