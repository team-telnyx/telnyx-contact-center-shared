  ...
  devtools(
    (set, get) => ({
      ...
      addTranscription: (transcriptionData) => {
        ...
        // Dedup a repeated FINAL. Some STT streams deliver the same final twice
        // (an is_final segment-end AND a speech_final end-of-speech carry the same
        // accumulated text, or a message is re-delivered). The open-bubble match
        // above only catches non-final bubbles, so the second final would append a
        // duplicate row. Collapse it when the immediately preceding entry on the
        // SAME leg is already a final with identical text — a legit repeat (e.g.
        // "No" ... "No" separated by the agent's turn) won't match, since the last
        // entry would then belong to the other leg.
        if (isFinal) {
          ...
          if (
            last &&
            last.isFinal &&
            legOf(last.track) === incomingLeg &&
            incomingText &&
            norm(last.transcript) === incomingText
          ) {
