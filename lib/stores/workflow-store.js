        ...
        try {
          const response = await fetch("/api/agent-assist/workflow/analyze", {
            ...
            body: JSON.stringify({
              ...
              recentTranscripts: options.recentTranscripts,
            }),
          });
          ...
          // Update local state with any completed or suggested items.
          // The analyze endpoint returns "completed" (high LLM confidence) or
          // "suggested" (0.60-0.85). Both should surface their extracted value
          // in the UI so the agent sees the captured/proposed slot data.
          if (data.updates) {
            for (const update of data.updates) {
              if (
                update.status === "completed" ||
                (update.status === "suggested" && update.extracted_value)
              ) {
                get().updateItemStatus(update.item_id, {
                  status: update.status,
                  completed_by: update.completed_by,
                  llm_confidence: update.llm_confidence ?? update.confidence,
                  confidence_score: update.llm_confidence ?? update.confidence,
                  extracted_value: update.extracted_value,
                  below_threshold: update.below_threshold,
                  stt_below_threshold: update.stt_below_threshold,
                  threshold: update.threshold,
                  stt_threshold: update.stt_threshold,
