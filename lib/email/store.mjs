...
export async function readEmailDetail(db, identity) {
  ...
  if(identity.draftScope && identity.draftScope!=='legacy')await db.query(`UPDATE cc_email_drafts d SET draft_scope=$3
    WHERE d.work_item_id=$1 AND d.agent_id=$2 AND d.draft_scope='legacy'
      AND NOT EXISTS(SELECT 1 FROM cc_email_drafts own WHERE own.work_item_id=d.work_item_id
        AND own.agent_id=d.agent_id AND own.draft_id=d.draft_id AND own.draft_scope=$3)`,[work.id,identity.agentId,identity.draftScope]);
  const drafts=(await db.query(`SELECT d.draft_id AS id,
    ...
    WHERE d.work_item_id=$1 AND d.agent_id=$2 AND d.draft_scope=$4 ORDER BY d.created_at,d.draft_id`,[work.id,identity.agentId,JSON.stringify(identity.knownDraftVersions||{}),identity.draftScope||"legacy"])).rows;
  ...
  const offer=(await db.query("SELECT id,deadline_at FROM acd_offers WHERE work_item_id=$1 AND agent_id=$2 AND state IN ('created','ringing') ORDER BY generation DESC LIMIT 1",[work.id,identity.agentId])).rows[0]||null;
  const assignment=(await db.query("SELECT state FROM acd_text_assignments WHERE work_item_id=$1 AND agent_id=$2 AND state<>'completed' ORDER BY (state='active') DESC LIMIT 1",[work.id,identity.agentId])).rows[0]||null;
  const wrapupCodes=(await db.query(`SELECT id,name,to_jsonb(c)->>'icon' AS icon,to_jsonb(c)->>'color' AS color FROM cc_wrapup_codes c WHERE is_active=true AND
    (NOT EXISTS(SELECT 1 FROM cc_queue_wrapup_codes WHERE queue_id=$1) OR EXISTS(SELECT 1 FROM cc_queue_wrapup_codes WHERE queue_id=$1 AND wrapup_code_id=c.id)) ORDER BY name`,[work.queue_id])).rows;
  return {work,offer,assignment,wrapupCodes,mailbox,messages,draft:project(draft),drafts:drafts.filter(d=>d.hasContent).map(project),selfAddresses,preview:await loadEmailPreviewSettings(db)};
}
...
export async function saveEmailDraft(pool,{workItemId,agentId,content,expectedVersion,draftId='legacy',draftScope='legacy',discard=false}) {
  ...
  try {
    ...
    const existing=(await db.query(`SELECT d.*,e.status AS send_status FROM cc_email_drafts d
      ...
      WHERE work_item_id=$1 AND agent_id=$2 AND draft_id=$3 AND draft_scope=$4`,[workItemId,agentId,draftId,draftScope])).rows[0];
    ...
    if(String(expectedVersion)==='0'&&!existing){
      const count=(await db.query(`SELECT count(*)::int AS count FROM cc_email_drafts WHERE work_item_id=$1 AND agent_id=$2 AND content<>'null'::jsonb AND draft_scope=$3`,[workItemId,agentId,draftScope])).rows[0].count;
      if(count>=20)throw emailError('At most 20 draft tabs are allowed',409);
    }
    let result;
    if(String(expectedVersion)==='0')result=await db.query(`INSERT INTO cc_email_drafts(work_item_id,agent_id,draft_id,content,draft_scope) VALUES($1,$2,$3,$4::jsonb,$5)
      ON CONFLICT DO NOTHING RETURNING content,version`,[workItemId,agentId,draftId,JSON.stringify(parsed),draftScope]);
      ...
      WHERE work_item_id=$1 AND agent_id=$2 AND draft_id=$3 AND version=$5 AND draft_scope=$6 RETURNING content,version`,[workItemId,agentId,draftId,JSON.stringify(parsed),expectedVersion,draftScope]);
...
export async function sendAgentEmail(pool,{workItemId,agentId,commandId,expectedVersion,content,draftVersion,draftId='legacy',draftScope='legacy',retainDraft=false}, {provider=emailProvider(),journalHash}={}) {
  validateDraftId(draftId);
  ...
  const hash=journalHash??createHash('sha256').update(JSON.stringify({content,expectedVersion,draftVersion,...(draftScope==='legacy'?{}:{draftScope}),...(draftId==='legacy'?{}:{draftId}),...(retainDraft?{retainDraft:true}:{})})).digest('hex');
  ...
  try{
    ...
    if(draftVersion!=null){
      const draft=(await db.query('SELECT version,submitted_message_id FROM cc_email_drafts WHERE work_item_id=$1 AND agent_id=$2 AND draft_id=$3 AND draft_scope=$4 FOR UPDATE',[workItemId,agentId,draftId,draftScope])).rows[0];
    ...
    if(draftVersion!=null)await db.query(`UPDATE cc_email_drafts SET
      ...
      version=version+1,updated_at=now() WHERE work_item_id=$1 AND agent_id=$2 AND draft_id=$3 AND version=$4 AND draft_scope=$8`,
      [workItemId,agentId,draftId,draftVersion,messageId,!keepDraft||Boolean(draft.scheduledAt),keepDraft,draftScope]);
