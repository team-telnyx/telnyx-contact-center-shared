...
export async function ensureEmailSchema(db) {
  await db.query(`
    ...
    DO $$ BEGIN
      ...
      END IF;
    END $$;
    ALTER TABLE cc_email_drafts ADD COLUMN IF NOT EXISTS draft_scope TEXT NOT NULL DEFAULT 'legacy';
    DO $$ BEGIN
      IF (SELECT COUNT(*) FROM information_schema.key_column_usage WHERE table_schema=current_schema()
          AND table_name='cc_email_drafts' AND constraint_name='cc_email_drafts_pkey')=3 THEN
        ALTER TABLE cc_email_drafts DROP CONSTRAINT cc_email_drafts_pkey;
        ALTER TABLE cc_email_drafts ADD PRIMARY KEY(work_item_id,agent_id,draft_id,draft_scope);
      END IF;
    END $$;
