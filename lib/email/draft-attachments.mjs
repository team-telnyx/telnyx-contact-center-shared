...
export async function readSupervisorDraftAttachment(db, work, { agentId, index, version, draftId='legacy' }) {
  ...
  const row = (await db.query(`SELECT d.version::text,d.content->'attachments'->$3::int AS file
    ...
    WHERE d.work_item_id=$1 AND d.agent_id=$2 AND d.draft_id=$4 AND w.channel='email' AND w.state='active' AND w.terminal_at IS NULL
      AND EXISTS(SELECT 1 FROM acd_text_assignments a WHERE a.work_item_id=d.work_item_id
        AND a.agent_id=d.agent_id AND a.state='active')`, [work.id, agentId, Number(index), draftId])).rows[0];
