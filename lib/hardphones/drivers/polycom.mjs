...
export const polycomDriver = {
  ...
  async hangup(phone) {
    const call = await activeCallRef(phone);
    if (!call) return { ok: false, reason: "no_active_call" };
    const result = await polyRequest(phone, "POST", "/api/v1/callctrl/endCall", { data: { Ref: call.ref } });
    if (!result.ok && isHeldCall(call)) {
      await polyRequest(phone, "POST", "/api/v1/callctrl/resumeCall", { data: { Ref: call.ref } });
  ...
  async mute(phone, state = true) {
    ...
    if (isHeldCall(call)) {
      await polyRequest(phone, "POST", "/api/v1/callctrl/resumeCall", { data: { Ref: call.ref } });
      const result = await polyRequest(phone, "POST", "/api/v1/callctrl/mute", { data: { state: state ? "1" : "0" } });
      await polyRequest(phone, "POST", "/api/v1/callctrl/holdCall", { data: { Ref: call.ref } });
      return result;
    }
