...
// (AudioCodes 4xxHD, NAT-ed/remote phones). For AudioCodes hardphones the
// Telnyx Credential Connection/assigned caller ID represents the phone identity:
//   dial(number)  → originate a direct outbound Telnyx call to the typed
//                   destination using the hardphone assigned number as CLI.
//   hangup/hold/resume/mute → Telnyx Call Control commands on the active leg
//                   (tracked via hp_cti_sessions, or the phone's current
//                   cc_interaction).
...
export function createTelnyxFallbackDriver({ pool }) {
  return {
    vendor: "telnyx-fallback",

    // Direct outbound dial: the typed UI number is the Telnyx `to` target.
    ...
    // Direct Telnyx-managed call; explicit answer is a no-op success for parity.
    async answer() {
      return { ok: true, note: "telnyx-managed outbound leg" };
    },
