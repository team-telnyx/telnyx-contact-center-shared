...
function ctiDtmfCallControlId(session) {
  return session?.target_call_control_id || session?.pstn_call_control_id || session?.call_control_id || null;
}
...
function fallbackConnectionId() {
  return String(process.env.TELNYX_CALL_CONTROL_ID || "").trim();
}
...
export function createTelnyxFallbackDriver({ pool, baseUrl = "" } = {}) {
  ...
  return {
    ...
    async dial(phone, number) {
      ...
      const connectionId = fallbackConnectionId();
    ...
    async sendDtmf(phone, digits) {
      const session = pool ? await activeCtiSession(pool, phone.id, phone) : null;
      if (!session) return { ok: false, reason: "no_active_call" };
      const callControlId = ctiDtmfCallControlId(session);
      if (!callControlId) return { ok: false, reason: "no_active_call" };
      return telnyxCall(`/calls/${encodeURIComponent(callControlId)}/actions/send_dtmf`, { digits: String(digits) });
    },
