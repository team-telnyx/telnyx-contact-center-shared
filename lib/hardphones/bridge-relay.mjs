...
function normalizedRegistrationStatus(value) {
  const raw = String(value || "").trim().toLowerCase().replace(/[\s-]+/g, "_");
  if (!raw) return null;
  if (["registered", "reg", "ok", "online", "up"].includes(raw)) return "registered";
  if (["not_registered", "unregistered", "notregister", "failed", "down", "offline"].includes(raw)) return "not_registered";
  if (["registering", "in_progress", "progress"].includes(raw)) return "registering";
  return raw;
}

function registrationStatusFromMessage(message = {}) {
  const candidates = [
    message.registration,
    message.registration_status,
    message.sip_registration_status,
    message.RegistrationStatus,
    message.LineState,
    message.SipStatus,
    message.result?.registration,
    message.result?.registration_status,
    message.result?.line,
    message.result?.line?.RegistrationStatus,
    message.result?.line?.LineState,
  ];
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) {
      for (const entry of candidate) {
        const nested = registrationStatusFromMessage(entry);
        if (nested) return nested;
      }
      continue;
    }
    if (candidate && typeof candidate === "object") {
      const nested = registrationStatusFromMessage(candidate);
      if (nested) return nested;
      continue;
    }
    const normalized = normalizedRegistrationStatus(candidate);
    if (normalized) return normalized;
  }
  return null;
}

async function bridgePhoneRegistry(bridge_id) {
  const pool = await postgresPool();
  if (!pool || !bridge_id) return [];
  try {
    const { rows } = await pool.query(
      `SELECT id AS phone_id, mac, vendor, COALESCE(NULLIF(ip_address, ''), NULLIF(last_ip, '')) AS ip, admin_password
       FROM hp_phones
       WHERE local_bridge_id = $1
         AND COALESCE(NULLIF(ip_address, ''), NULLIF(last_ip, '')) IS NOT NULL
    ...
    return rows.map((row) => ({
      ...
      vendor: row.vendor,
      ip: row.ip,
      admin_password: row.admin_password || undefined,
    }));
  } catch (err) {
    relayLogger.debug?.("hardphone_bridge_registry_failed", { bridgeId: bridge_id, error: err?.message || String(err) });
    return [];
  }
}
...
async function recordObservedPhone(bridge_id, message = {}) {
  ...
  try {
    ...
    const registrationStatus = registrationStatusFromMessage(message);
    await pool.query(
      `UPDATE hp_phones
       SET last_ip = $2,
           ip_address = COALESCE(NULLIF(ip_address, ''), $2),
           local_bridge_id = COALESCE(local_bridge_id, $3),
           sip_registration_status = COALESCE($4, sip_registration_status),
           sip_registration_status_at = CASE WHEN $4 IS NULL THEN sip_registration_status_at ELSE NOW() END,
           last_seen_at = NOW(),
           updated_at = NOW()
       WHERE ${where}`,
      [key, ip, bridge_id, registrationStatus],
    );
    await pool.query(
      `INSERT INTO hp_provisioning_events (phone_id, mac, event_type, detail)
       SELECT id, mac, CASE WHEN $4::text IS NULL THEN 'bridge_phone_observed' ELSE 'registration_status_event' END, $3::jsonb
       FROM hp_phones WHERE ${where}`,
      [key, ip, JSON.stringify({ bridge_id, detected_ip: ip, source: message.source || "bridge", registration_status: registrationStatus }), registrationStatus],
    );
  } catch (err) {
...
export async function registerBridgeConnection(ws, request) {
  ...
  ws.on("message", (raw) => {
    ...
    if (message.type === "phone_registry_request") {
      bridgePhoneRegistry(bridge_id).then((phones) => safeSend(ws, { type: "phone_registry", bridge_id, phones, timestamp: new Date().toISOString() }));
      return;
    }
    ...
    if (message.type === "command_result") {
      ...
      if (command?.ws === ws) {
        ...
        if (discoveredIp && command.phone_id) {
          recordObservedPhone(bridge_id, { phone_id: command.phone_id, ip: discoveredIp, result, source: "bridge_command_result" });
        }
...
export async function sendBridgeCommand({ bridge_id, phone_id = null, vendor, host, action, payload = {}, timeoutMs = DEFAULT_COMMAND_TIMEOUT_MS }) {
  ...
  return new Promise((resolve) => {
    ...
    timer.unref?.();
    pendingCommands.set(command_id, { bridge_id, phone_id, ws: bridge.ws, resolve: finish, timer });
