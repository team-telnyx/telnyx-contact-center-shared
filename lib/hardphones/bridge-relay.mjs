...
async function recordObservedPhone(bridge_id, message = {}) {
  ...
  if (!pool || mac.length !== 12 || !ip) return;
  try {
    await pool.query(
      `UPDATE hp_phones
       SET last_ip = $2,
           ...
           last_seen_at = NOW(),
           updated_at = NOW()
       WHERE replace(replace(lower(mac), ':', ''), '-', '') = $1`,
      [mac, ip, bridge_id],
    );
    await pool.query(
      `INSERT INTO hp_provisioning_events (phone_id, mac, event_type, detail)
       SELECT id, mac, 'bridge_phone_observed', $3::jsonb
       FROM hp_phones WHERE replace(replace(lower(mac), ':', ''), '-', '') = $1`,
      [mac, ip, JSON.stringify({ bridge_id, detected_ip: ip, source: message.source || "bridge" })],
    );
  } catch (err) {
