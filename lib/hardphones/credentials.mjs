...
export async function getPhoneSipConnection(connectionId) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(`Telnyx SIP connection fetch failed: ${errorDetail(data, `HTTP ${response.status}`)}`);
  return data?.data || null;
}

export function normalizeTelnyxRegistrationStatus(value) {
  const raw = String(value || "").trim().toLowerCase().replace(/[\s-]+/g, "_");
  if (!raw) return null;
  if (["registered", "reg", "ok", "online", "up"].includes(raw)) return "registered";
  if (["unregistered", "not_registered", "notregister", "registration_failed", "failed", "offline", "down", "expired", "inactive", "non_active"].includes(raw)) return "not_registered";
  if (["registering", "in_progress", "progress"].includes(raw)) return "registering";
  return raw;
}

export async function checkPhoneSipRegistrationStatus(connectionId) {
  const apiKey = process.env.TELNYX_API_KEY;
  if (!apiKey || !connectionId) return null;
  const response = await fetch(buildTelnyxV2Url(`/credential_connections/${encodeURIComponent(connectionId)}/actions/check_registration_status`), {
    method: "POST",
    headers: telnyxHeaders(apiKey),
    cache: "no-store",
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(`Telnyx SIP registration status check failed: ${errorDetail(data, `HTTP ${response.status}`)}`);
  const payload = data?.data || null;
  return payload ? { ...payload, registration_status: normalizeTelnyxRegistrationStatus(payload.status || payload.registration_status) } : null;
}
