...
export async function listTelnyxPhoneNumbers({ limit = 250, pages = 5 } = {}) {
...
function outboundCallerIdPatch(phoneNumber) {
  const ani = normalizeAssignedPhoneNumber(phoneNumber);
  return ani ? { ani_override: ani, ani_override_type: "always" } : {};
}
...
export async function updatePhoneSipConnectionCallerId({ connectionId, phoneNumber }) {
  const apiKey = process.env.TELNYX_API_KEY;
  const outboundPatch = outboundCallerIdPatch(phoneNumber);
  if (!apiKey || !connectionId || !outboundPatch.ani_override) return null;
  const response = await fetch(buildTelnyxV2Url(`/credential_connections/${encodeURIComponent(connectionId)}`), {
    method: "PATCH",
    headers: telnyxHeaders(apiKey),
