...
export async function createPhoneSipConnection({ label, mac, vendor, model, password }) {
  ...
  const body = {
    ...
    tags,
    inbound: {
      // Telnyx otherwise sends the called DID as the Request-URI user on
      // inbound PSTN calls. AudioCodes 4xxHD matches incoming INVITEs against
      // its registered line id and rejects DID Request-URIs with 404 Not Found.
      // Send the SIP credential username instead; the assigned DID remains in
      // CC metadata and Telnyx number assignment.
