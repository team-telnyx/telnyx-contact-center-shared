...
// registers to Telnyx SIP using its own telephony credential injected here.
//
...
// registration file using the [PHONE_MAC_ADDRESS] macro.
export function polycomMasterConfig() {
  return `<?xml version="1.0" standalone="yes"?>
<APPLICATION APP_FILE_PATH="" CONFIG_FILES="[PHONE_MAC_ADDRESS]-reg.cfg" MISC_FILES="" LOG_FILE_DIRECTORY="" OVERRIDES_DIRECTORY="" CONTACTS_DIRECTORY="" LICENSE_DIRECTORY=""/>
`;
}
...
export function polycomRegistrationConfig(phone, { baseUrl = "" } = {}) {
  ...
  const label = phone.label || phone.sip_username || "Telnyx";
  const lines = [
    ...
    `  <prov prov.polling.enabled="1" prov.polling.mode="rel" prov.polling.period="3600"/>`,
    `  <httpd httpd.enabled="1" apps.restapi.enabled="1"/>`,
  ];
  if (baseUrl) {
    lines.push(
      `  <apps apps.telNotification.URL="${xmlEscape(`${baseUrl}/api/provisioning/events/polycom?mac=[PHONE_MAC_ADDRESS]`)}" apps.telNotification.incomingEvent="1" apps.telNotification.outgoingEvent="1" apps.telNotification.callStateChangeEvent="1" apps.telNotification.offhookEvent="1" apps.telNotification.onhookEvent="1"/>`,
    );
  }
...
export function yealinkPhoneConfig(phone, { baseUrl = "" } = {}) {
  ...
  if (baseUrl) {
    const ev = (name) => `${baseUrl}/api/provisioning/events/yealink?mac=$mac&event=${name}&call_id=$call_id&remote=$remote&local=$local`;
    lines.push(
      ``,
...
export function audiocodesPhoneConfig(phone, { baseUrl = "" } = {}) {
  ...
  const lines = [
    ...
    `provisioning/check_sync/enabled=1`,
    ``,
...
export function buildConfigForPhone(phone, kind, { baseUrl = "" } = {}) {
  ...
  if (vendor === "polycom") {
    if (kind === "registration") return { contentType: "text/xml", body: polycomRegistrationConfig(phone, { baseUrl }) };
