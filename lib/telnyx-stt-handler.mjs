...
function shouldSwapTelnyxL16Payload(config = {}) {
  return String(config.input_format || "").toLowerCase() === "linear16";
}

export function convertTelnyxL16PayloadForLinear16(audioBuffer) {
  if (!Buffer.isBuffer(audioBuffer)) return audioBuffer;
  const converted = Buffer.from(audioBuffer);
  for (let offset = 0; offset + 1 < converted.length; offset += 2) {
    const highByte = converted[offset];
    converted[offset] = converted[offset + 1];
    converted[offset + 1] = highByte;
  }
  return converted;
}
...
class TelnyxSttRealtimeSession {
  ...
  sendAudio(payloadBase64) {
    ...
    try {
      ...
      const providerAudioBuffer = shouldSwapTelnyxL16Payload(this.config)
        ? convertTelnyxL16PayloadForLinear16(audioBuffer)
        : audioBuffer;
      this.ws.send(providerAudioBuffer);
      this.sentChunks++;
      this.sentBytes += providerAudioBuffer.length;
      if (this.sentChunks <= 3 || this.sentChunks % 500 === 0) {
        logSttDebug("provider_socket_audio_sent", {
          ...
          sentChunks: this.sentChunks,
          sentBytes: this.sentBytes,
          lastChunkBytes: providerAudioBuffer.length,
        });
      }
    } catch (e) {
...
export async function startTelnyxSttMediaStream(callControlId, config, interactionId, agentUsername, outputTrack = "outbound") {
  ...
  logSttInfo("starting_telnyx_stt_media_stream", {
    ...
    streamBidirectionalMode: body.stream_bidirectional_mode,
    streamBidirectionalCodec: body.stream_bidirectional_codec,
    streamBidirectionalSamplingRate: body.stream_bidirectional_sampling_rate,
    config: summarizeConfig(config),
  });
