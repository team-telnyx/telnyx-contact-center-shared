...
export async function prewarmTelnyxSttAgentLeg(callControlId, config, interactionId, agentUsername) {
  ...
  if (agentUsername) {
    try {
      ...
      console.warn("[TelnyxSTT] Failed to resolve agent profile language:", error.message);
    }
  }
  const outboundConfig = { ...config, transcription_tracks: "outbound", language: agentLanguage };
  console.log(`[TelnyxSTT] Prewarming agent leg callControlId=${callControlId}, interactionId=${interactionId}, agent=${agentUsername}, engine=${outboundConfig.transcription_engine}, model=${outboundConfig.model}, language=${outboundConfig.language}`);
