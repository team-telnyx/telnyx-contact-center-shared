...
// Observed failure mode (two near-simultaneous session starts, e.g. inbound +
// outbound legs answering within milliseconds of each other): one provider
// socket opens, accepts audio, but never delivers a single message, then the
// provider itself force-closes it cleanly (code 1000, no reason) after ~6s.
// Without a reconnect, that track stays silent for the rest of the call. Cap
// automatic reconnects so a persistent problem (bad credentials, provider
// outage) surfaces as an error instead of retrying forever.
const STT_DEAD_SOCKET_MAX_RECONNECTS = 3;
...
class TelnyxSttRealtimeSession {
  ...
  connect() {
    ...
    this.ws.on("close", (code, reason) => {
      ...
      // that's how an intentional close is told apart from the provider
      // closing on us unexpectedly.
      const wasIntentional = this.closed;
      this.connected = false;
      this.closed = true;

      // Zero messages received for this socket's entire lifetime, closed by
      // the provider (not by us) — the dead-socket pattern described above.
      const deadSocket = !wasIntentional && this.providerMessages === 0;
      const willReconnect = deadSocket && this.reconnectAttempts < STT_DEAD_SOCKET_MAX_RECONNECTS;
      ...
      if (!deadSocket) return;

      if (!willReconnect) {
        logSttError("provider_socket_dead_reconnect_exhausted", {
          callControlId: logIdentifier(this.callControlId),
          interactionId: logIdentifier(this.interactionId),
          track: this.track,
          reconnectAttempts: this.reconnectAttempts,
      ...
      this.providerFrames = 0;
      this.nonTranscriptFrames = 0;
      logSttWarn("provider_socket_dead_reconnecting", {
        callControlId: logIdentifier(this.callControlId),
        interactionId: logIdentifier(this.interactionId),
        track: this.track,
        reconnectAttempts: this.reconnectAttempts,
        ...
        case "media": {
          ...
          if (active) {
            ...
            if (target) target.sendAudio(payload);
              ...
              // Debug-level: expected during prewarm when both_tracks stream
              // carries outbound audio before B2 session is created.
              logSttDebug("media_ws_no_target_for_track", {
                callControlId: logIdentifier(callControlId),
                track,
                sttTrack,
        ...
        case "stop": {
          ...
          if (callControlId) {
            ...
            if (currentStreamSession === session) {
              // Remove the stream session from the map — it's done.
              // Do NOT call stopTelnyxSttTranscription here: call.hangup owns that
              // cleanup. Calling it here races with a new streaming_start that opens
              // a fresh WebSocket; if the old stop fires before the new start registers
              // in __telnyxSttStreamSessions, the new STT sessions get deleted and the
              // Telnyx built-in STT bypass fails, producing duplicate bubbles.
...
export async function startTelnyxSttMediaStream(callControlId, config, interactionId, agentUsername, outputTrack = "outbound", callSessionId = null) {
  if (!callControlId || !config?.enabled) return;
  const apiKey = process.env.TELNYX_API_KEY;
  if (!apiKey) {
  ...
  const body = {
    ...
    stream_track: outputTrack === "both" ? "both_tracks" : "inbound_track",
    stream_codec: "PCMU",
...
export async function prewarmTelnyxSttAgentLeg(callControlId, config, interactionId, agentUsername) {
  ...
  // Use "both" so startTelnyxSttMediaStream sends both_tracks from the start.
  // The TARGET block (call.answered) will skip the second streaming_start because
  // the commandId already matches, avoiding a ~10-second media stream restart.
  // Outbound audio (hold music) arrives but has no session target until B2 is
  // created — it is silently dropped by the media router.
  await startTelnyxSttMediaStream(callControlId, outboundConfig, interactionId, agentUsername, "both");
}
...
// startTelnyxSttMediaStream("both") call recognises it as already sent and
