...
const sttLogger = createDiagnosticLogger("telnyx.standalone_stt");

function telnyxSttDebugEnabled() {
  return envFlagEnabled("DEBUG_TELNYX_STT");
}

function shortId(value) {
  if (!value) return null;
  const str = String(value);
  return str.length <= 12 ? str : `${str.slice(0, 4)}…${str.slice(-8)}`;
}

function logSttDebug(message, payload = {}) {
  if (telnyxSttDebugEnabled()) sttLogger.debug(message, payload);
}

function logSttInfo(message, payload = {}) {
  if (telnyxSttDebugEnabled()) sttLogger.info(message, payload);
}

function logSttWarn(message, payload = {}) {
  if (telnyxSttDebugEnabled()) sttLogger.warn(message, payload);
}

function logSttError(message, payload = {}) {
  if (telnyxSttDebugEnabled()) sttLogger.error(message, payload);
}
...
async function routeTranscriptionThroughAgentAssist(payload) {
  try {
    ...
    logSttError("agent_assist_route_failed", {
      callControlId: shortId(payload.callControlId),
      interactionId: shortId(payload.interactionId),
      source: payload.source,
  ...
  connect() {
    ...
    if (!apiKey) {
      logSttError("provider_socket_missing_api_key", {
        callControlId: shortId(this.callControlId),
        interactionId: shortId(this.interactionId),
        track: this.track,
      });
      return;
    }
      ...
      switch (msg.event) {
        case "start": {
          ...
          logSttInfo("media_ws_start", {
            callControlId: shortId(callControlId),
            streamId: shortId(startData.stream_id || startData.streamId),
            ...
            interactionId: shortId(clientState.interaction_id),
...
export async function startTelnyxSttTranscription(callControlId, config, interactionId, agentUsername, options = {}) {
  ...
  if (globalThis.__telnyxSttActiveSessions.has(callControlId)) {
    logSttInfo("STT session already active", {
      callControlId: shortId(callControlId),
      interactionId: shortId(interactionId),
    });
    return;
  }
