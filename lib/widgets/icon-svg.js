...
export function widgetIconSvgMarkup(name) {
  ...
  // Lucide changed this internal shape from iconNode to icon.node. Keep both
  // supported because Next route handlers cannot import react-dom/server.
  const iconNode = props.iconNode || props.icon?.node;
  if (!Array.isArray(iconNode)) throw new Error("Unsupported widget launcher icon format");
