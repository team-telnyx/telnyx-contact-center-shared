...
export function createS3Driver() {
  ...
  return {
    ...
    /** Fragment obiektu przez naglowek Range. */
    async getRange(filename, start, end) {
      const s3 = await client();
      const { GetObjectCommand } = _lib;
      try {
        const out = await s3.send(new GetObjectCommand({ Bucket: c.bucket, Key: keyFor(filename), Range: `bytes=${start}-${end}` }));
        return { body: await streamToBuffer(out.Body) };
      } catch (err) {
        if (err?.name === "NoSuchKey" || err?.$metadata?.httpStatusCode === 404) return null;
        throw err;
      }
    ...
    async list() {
      ...
      do {
        ...
        for (const obj of out.Contents || []) {
          const base = String(obj.Key).slice(prefix.length);
