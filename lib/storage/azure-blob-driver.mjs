...
export function createAzureBlobDriver() {
  ...
  return {
    ...
    /** Fragment bloba (offset, count). */
    async getRange(filename, start, end) {
      const container = await containerClient();
      const blockBlobClient = container.getBlockBlobClient(keyFor(filename));
      try {
        const download = await blockBlobClient.download(start, end - start + 1);
        return { body: await streamToBuffer(download.readableStreamBody) };
      } catch (err) {
