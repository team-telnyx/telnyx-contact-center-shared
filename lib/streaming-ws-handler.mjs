...
function registerProcessShutdownHandlers() {
  ...
  const shutdownForSignal = (signal) => {
    shutdownStreamingWSServer({ reason: `process_${signal}` })
      .catch((err) => {
        console.warn("[Streaming WS] Shutdown error:", err?.message || err);
      })
      .finally(() => {
        process.exit(0);
      });
...
export function initStreamingWSServer(options = {}) {
  ...
  if (state.closing) {
    console.log("[Streaming WS] Shutdown already in progress — skipping init");
    return state;
  }
  ...
  if (state.httpServer && state.wss && !state.closing) {
    if (isHttpServerListening(state.httpServer)) {
      ...
      console.log(`[Streaming WS] Startup already in progress on port ${state.port || "pending"} — skipping init`);
    } else {
...
function startServer(port, options = {}) {
  ...
  wss.on("error", (err) => {
    console.warn("[Streaming WS] WebSocket server error:", err?.message || err);
  });
  ...
  server.on("listening", () => {
    ...
    console.log(`[Streaming WS] ✅ Listening on port ${state.port}${secret}`);
    console.log(`[Streaming WS] HTTP health: /api/health | Routes: /streaming/google | /streaming/openai | /streaming/telnyx-stt`);
  });

  server.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      console.warn(`[Streaming WS] Port ${port} already in use — could not start sidecar`);
    } else {
...
export async function shutdownStreamingWSServer(options = {}) {
  ...
  state.shutdownPromise = (async () => {
    ...
    clearStreamingStateIfCurrent(state, server, wss);
    console.log("[Streaming WS] Shutdown complete");
