...
const streamingLogger = createDiagnosticLogger("platform.app");
...
function startServer(port, options = {}) {
  ...
  server.on("listening", () => {
    ...
    state.starting = false;
    syncLegacyGlobals(state);
    streamingLogger.info("streaming_ws_listening", {
      port: state.port,
      message: `Streaming WS listening on port ${state.port}`,
    });
    streamingLogger.info("streaming_ws_routes_ready", {
      port: state.port,
      message: `Streaming WS routes ready on port ${state.port}`,
    });
  });
