    ...
    try {
      ...
      await client.query(`
        ...
        VALUES 
          ...
          ('busy', 'Busy', 'active', true, true, 'x', '#dc2626', 2, 'Agent is busy handling calls'),
          ('away', 'Away', 'active', true, true, 'circle-off', '#f97316', 3, 'Agent is away from desk'),
          ('offline', 'Offline', 'active', true, true, 'circle-off', '#64748b', 4, 'Agent is offline'),
      ...
      await client.query(`
        INSERT INTO cc_wrapup_codes (id, name, is_active, is_default, display_order, description)
        VALUES ('default', 'Default', true, true, 0, 'Default wrapup code when no selection is made')
        ON CONFLICT (id) DO NOTHING;
      `);
