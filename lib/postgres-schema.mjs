    ...
    try {
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS aa_workflows (
          ...
          is_active BOOLEAN DEFAULT true,
          stt_confidence_threshold NUMERIC(3,2) DEFAULT 0.95 CHECK (stt_confidence_threshold >= 0 AND stt_confidence_threshold <= 1),
          created_by TEXT REFERENCES users(id) ON DELETE SET NULL,
          created_at TIMESTAMPTZ DEFAULT NOW(),
          updated_at TIMESTAMPTZ DEFAULT NOW()
        );
      ...
      // Add stt_confidence_threshold column to aa_workflows if it doesn't exist (migration for existing databases)
      await client.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'aa_workflows' AND column_name = 'stt_confidence_threshold'
          ) THEN
            ALTER TABLE aa_workflows ADD COLUMN stt_confidence_threshold NUMERIC(3,2) DEFAULT 0.95;
          END IF;
        END $$;
      `);
