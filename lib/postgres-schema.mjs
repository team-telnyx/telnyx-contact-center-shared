    ...
    try {
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS outbound_contact_lists (
          ...
          standard_columns JSONB NOT NULL DEFAULT '{}'::jsonb,
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS outbound_campaigns (
          ...
          name TEXT NOT NULL,
          description TEXT,
          status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'ready', 'paused', 'running', 'completed', 'archived')),
      ...
      await client.query(`
        ...
        CREATE INDEX IF NOT EXISTS idx_outbound_contact_lists_custom_fields ON outbound_contact_lists USING GIN (custom_fields);
        ...
        CREATE INDEX IF NOT EXISTS idx_outbound_contact_records_phone_number ON outbound_contact_records (phone_number);
        CREATE INDEX IF NOT EXISTS idx_outbound_contact_records_validation_status ON outbound_contact_records (validation_status);
        CREATE INDEX IF NOT EXISTS idx_outbound_contact_records_standard_fields ON outbound_contact_records USING GIN (standard_fields);
        CREATE INDEX IF NOT EXISTS idx_outbound_contact_records_custom_fields ON outbound_contact_records USING GIN (custom_fields);
      ...
      try {
        ...
        console.warn("[Postgres Schema] form template seeding skipped:", err.message);
      }
      ...
      try {
        ...
        console.log("[Postgres] aa_ai_handoff_events table ready");
      } catch (handoffErr) {
        console.error("[Postgres] Failed to create aa_ai_handoff_events:", handoffErr.message);
      }
      ...
      try {
        ...
        // Log but don't fail schema creation if seeding fails
        console.warn(
          "[Postgres] Warning: Failed to seed email domains:",
          domainSeedError.message
        );
      }
      ...
      try {
        ...
        // Log but don't fail schema creation if seeding fails
        console.warn(
          "[Postgres] Warning: Failed to seed default owner:",
          ownerSeedError.message
        );
      }
      ...
      try {
        ...
        // Log but don't fail schema creation if seeding fails
        console.warn(
          "[Postgres] Warning: Failed to seed sample workflows:",
          workflowSeedError.message
        );
      }
