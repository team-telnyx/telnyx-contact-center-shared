...
export async function ensurePostgresSchema() {
  try {
    ...
    try {
      ...
      // Create aa_ai_handoff_events table for storing AI handoff webhook events
      // Note: interaction_id is TEXT to match cc_interactions.id type
      await client.query(`
      ...
      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_aa_ai_handoff_events_insight_group_id 
        ON aa_ai_handoff_events (insight_group_id);
      `);

      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_aa_ai_handoff_events_status 
        ON aa_ai_handoff_events (status);
      `);
