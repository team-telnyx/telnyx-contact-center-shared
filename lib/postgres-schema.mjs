...
export async function ensurePostgresSchema() {
  try {
    ...
    try {
      ...
      // HA session presence table - active SSE session streams across app nodes.
      // Additive and TTL-based: crashed nodes stop heartbeating and expire.
      await client.query(`
        CREATE TABLE IF NOT EXISTS cc_session_presence (
          ...
          created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          CONSTRAINT fk_session_presence_user FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
        );
      `);

      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_cc_session_presence_user_expires ON cc_session_presence (user_id, expires_at);
        CREATE INDEX IF NOT EXISTS idx_cc_session_presence_node ON cc_session_presence (node_id);
      `);
      ...
      // FDE-535: persist alternative candidate values for low-confidence slots.
      // Idempotent ADD COLUMN IF NOT EXISTS (matches the simpler idempotent
      // pattern used for hp_phones / cc_agent_state columns elsewhere in this
      // schema). Stored as JSONB since alternatives is an array of
