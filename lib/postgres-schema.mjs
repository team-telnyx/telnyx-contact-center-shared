import { OUTBOUND_DIAL_STATE_UPGRADE } from "./outbound-dialer/dial-state-schema.mjs";
import { OUTBOUND_MESSAGING_UPGRADE } from "./outbound-dialer/messaging/schema.mjs";
...
export async function ensurePostgresSchema() {
  try {
    ...
    try {
      ...
      // Pre-ACD-Core databases carry these tables with the old
      // `interaction_id` link. ensureAcdSchema() (below, after COMMIT) moves
      // them onto `work_item_id`, but indexes in this transaction already
      // reference that column — so add it here, or the whole bootstrap
      // transaction rolls back and the migration can never run.
      await client.query(`
        ALTER TABLE IF EXISTS form_submissions ADD COLUMN IF NOT EXISTS work_item_id UUID;
        ALTER TABLE IF EXISTS aa_workflow_sessions ADD COLUMN IF NOT EXISTS work_item_id UUID;
        ALTER TABLE IF EXISTS aa_ai_handoff_events ADD COLUMN IF NOT EXISTS work_item_id UUID;
        ALTER TABLE IF EXISTS quality_evaluations ADD COLUMN IF NOT EXISTS work_item_id UUID;
        ALTER TABLE IF EXISTS quality_ai_jobs ADD COLUMN IF NOT EXISTS work_item_id UUID;
      `);
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS users (
          ...
          -- Enables in-progress UI and runtime features for this account.
      ...
      // Remove the obsolete users.status column; ACD Core owns agent state.
      await client.query(`
        DO $$
        BEGIN
          IF EXISTS (
            ...
            RAISE NOTICE 'Dropped obsolete users.status column; ACD Core is authoritative';
          END IF;
        END $$;
      `);
      ...
      // Add the per-user experimental feature gate for existing databases.
      await client.query(`
        ALTER TABLE users
        ADD COLUMN IF NOT EXISTS experimental_features BOOLEAN NOT NULL DEFAULT false;
      `);
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS app_settings (
          ...
          cc_settings JSONB NOT NULL DEFAULT '{}'::jsonb,
          updated_by TEXT,
          updated_at TIMESTAMPTZ DEFAULT NOW(),
          created_at TIMESTAMPTZ DEFAULT NOW()
        );
      ...
      await client.query(`
        ...
        ADD COLUMN IF NOT EXISTS cc_settings JSONB NOT NULL DEFAULT '{}'::jsonb;
      `);

      // Per-user interaction sound overrides (NULL = the system settings).
      await client.query(`
        ALTER TABLE users
        ADD COLUMN IF NOT EXISTS notification_sounds JSONB;
      `);
      ...
      // Add brand_name column if it doesn't exist (migration for existing databases).
      // Used as the company/brand for agent-assist suggested-response greetings
      // ("Thanks for calling <brand_name>").
      await client.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
      ...
      await client.query(`
        ...
        BEGIN
          ...
          IF NOT EXISTS (
            ...
            ALTER TABLE cc_queues ADD COLUMN agent_answer_timeout_secs INTEGER;
          END IF;
        END $$;
      `);

      // NULL means the queue inherits System -> Settings -> Agent lifecycle.
      await client.query(`
        ALTER TABLE cc_queues ALTER COLUMN agent_answer_timeout_secs DROP DEFAULT;
      `);
      ...
      await client.query(`
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_user_sessions_session_token_unique
          ON cc_user_sessions (session_token)
      ...
      // Canonical closed status intervals written by ACD Core.
      await client.query(`
        CREATE TABLE IF NOT EXISTS cc_agent_status_intervals (
          id TEXT PRIMARY KEY,
          idempotency_key TEXT UNIQUE NOT NULL,
          user_id TEXT NOT NULL,
          agent_username TEXT,
          status TEXT NOT NULL,
          status_type TEXT NOT NULL CHECK (status_type IN ('active', 'break')),
          ...
          created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          CONSTRAINT fk_agent_status_intervals_user
            FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,
          CONSTRAINT chk_agent_status_interval_bounds CHECK (ended_at >= started_at)
        );
      `);

      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_agent_status_intervals_user_started
          ON cc_agent_status_intervals (user_id, started_at);
        CREATE INDEX IF NOT EXISTS idx_agent_status_intervals_user_ended
          ON cc_agent_status_intervals (user_id, ended_at);
        CREATE INDEX IF NOT EXISTS idx_agent_status_intervals_status_type
          ON cc_agent_status_intervals (status_type, started_at);
        CREATE INDEX IF NOT EXISTS idx_agent_status_intervals_work_item
          ON cc_agent_status_intervals (work_item_id)
      ...
      await client.query(`
        ...
        CREATE INDEX IF NOT EXISTS idx_form_submissions_work_item_id ON form_submissions (work_item_id);
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS outbound_campaigns (
          ...
          mode TEXT NOT NULL DEFAULT 'preview' CHECK (mode IN ('preview', 'progressive', 'agentless_ai', 'agentless_flow', 'power', 'predictive', 'broadcast')),
      ...
      await client.query(`ALTER TABLE outbound_attempt_ledger ADD COLUMN IF NOT EXISTS next_retry_at TIMESTAMPTZ`);

      await client.query(OUTBOUND_DIAL_STATE_UPGRADE);
      await client.query(OUTBOUND_MESSAGING_UPGRADE);
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS aa_workflows (
          ...
          is_active BOOLEAN DEFAULT true,
          data_action_buttons JSONB DEFAULT '[]'::jsonb,
          llm_confidence_threshold FLOAT DEFAULT 0.95 CHECK (llm_confidence_threshold >= 0 AND llm_confidence_threshold <= 1),
          llm_reasoning_enabled BOOLEAN NOT NULL DEFAULT false,
          llm_max_output_tokens INTEGER NOT NULL DEFAULT 1200 CHECK (llm_max_output_tokens BETWEEN 512 AND 1200),
          llm_fallback_model VARCHAR(100) NOT NULL DEFAULT 'openai/gpt-5.6-luna',
          created_by TEXT REFERENCES users(id) ON DELETE SET NULL,
          created_at TIMESTAMPTZ DEFAULT NOW(),
          updated_at TIMESTAMPTZ DEFAULT NOW()
        );
      ...
      // Per-slot MCP tool binding: lets a slot invoke an MCP tool once it is
      // filled, for mid-call enrichment such as lookup_addresses.
      await client.query(`
        DO $$ 
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'aa_workflow_items' AND column_name = 'mcp_binding'
          ) THEN
            ALTER TABLE aa_workflow_items ADD COLUMN mcp_binding JSONB;
          END IF;
        END $$;
      `);

      // Per-workflow fallback for real-time slot extraction. Keep Luna as the
      // current default, but persist the choice so operators can move to a new
      // low-latency model without changing application code.
      await client.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'aa_workflows' AND column_name = 'llm_fallback_model'
          ) THEN
            ALTER TABLE aa_workflows
              ADD COLUMN llm_fallback_model VARCHAR(100) NOT NULL DEFAULT 'openai/gpt-5.6-luna';
          END IF;

          UPDATE aa_workflows
          SET llm_fallback_model = 'openai/gpt-5.6-luna'
          WHERE llm_fallback_model IS NULL OR btrim(llm_fallback_model) = '';

          ALTER TABLE aa_workflows
            ALTER COLUMN llm_fallback_model SET DEFAULT 'openai/gpt-5.6-luna',
            ALTER COLUMN llm_fallback_model SET NOT NULL;
        END $$;
      `);
      ...
      // Add data_action_buttons column to aa_workflows for agent desktop workflow buttons
      await client.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'aa_workflows' AND column_name = 'data_action_buttons'
          ) THEN
            ALTER TABLE aa_workflows ADD COLUMN data_action_buttons JSONB DEFAULT '[]'::jsonb;
          END IF;

          UPDATE aa_workflows
          SET data_action_buttons = '[]'::jsonb
          WHERE data_action_buttons IS NULL;

          ALTER TABLE aa_workflows
            ALTER COLUMN data_action_buttons SET DEFAULT '[]'::jsonb;
        END $$;
      `);
      ...
      // Reasoning is opt-in for workflow slot extraction. Some hosted models
      // enable it by default when the parameter is omitted, which adds large
      // and highly variable latency to a deterministic JSON extraction task.
      await client.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'aa_workflows' AND column_name = 'llm_reasoning_enabled'
          ) THEN
            ALTER TABLE aa_workflows
              ADD COLUMN llm_reasoning_enabled BOOLEAN NOT NULL DEFAULT false;
          END IF;

          UPDATE aa_workflows
          SET llm_reasoning_enabled = false
          WHERE llm_reasoning_enabled IS NULL;

          ALTER TABLE aa_workflows
            ALTER COLUMN llm_reasoning_enabled SET DEFAULT false,
            ALTER COLUMN llm_reasoning_enabled SET NOT NULL;
        END $$;
      `);

      // Per-workflow hard ceiling for real-time slot-extraction output. The
      // analyzer still scales down for small prompts; this prevents a model
      // from expanding into a long response beyond the configured ceiling.
      await client.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'aa_workflows' AND column_name = 'llm_max_output_tokens'
          ) THEN
            ALTER TABLE aa_workflows
          ...
          UPDATE aa_workflows
          SET llm_max_output_tokens = 1200
          WHERE llm_max_output_tokens IS NULL
             OR llm_max_output_tokens < 512
             OR llm_max_output_tokens > 1200;

          ALTER TABLE aa_workflows
            ALTER COLUMN llm_max_output_tokens SET DEFAULT 1200,
            ALTER COLUMN llm_max_output_tokens SET NOT NULL;

          IF NOT EXISTS (
            SELECT 1 FROM pg_constraint
            WHERE conname = 'aa_workflows_llm_max_output_tokens_check'
          ) THEN
            ALTER TABLE aa_workflows
              ADD CONSTRAINT aa_workflows_llm_max_output_tokens_check
              CHECK (llm_max_output_tokens BETWEEN 512 AND 1200);
          END IF;
        END $$;
      `);
      ...
      // Active workflow sessions - tracks workflow progress per Core work item
      await client.query(`
        CREATE TABLE IF NOT EXISTS aa_workflow_sessions (
          ...
          data_action_statuses JSONB DEFAULT '{}'::jsonb,
      ...
      await client.query(`
        ALTER TABLE aa_workflow_sessions ADD COLUMN IF NOT EXISTS data_action_statuses JSONB DEFAULT '{}'::jsonb;
        ALTER TABLE aa_workflow_sessions ADD COLUMN IF NOT EXISTS mcp_results JSONB DEFAULT '{}'::jsonb;
        -- mcp_runs maps binding key -> latest fingerprint. It must be an object:
        -- jsonb '||' concatenates into an array but merges into an object, so an
        -- array-shaped value would append instead of replacing the fingerprint.
        ALTER TABLE aa_workflow_sessions ADD COLUMN IF NOT EXISTS mcp_runs JSONB DEFAULT '{}'::jsonb;
        -- Multi-match lookups park their candidates here until the agent picks
        -- one. Deliberately separate from mcp_results: an unresolved lookup must
        -- not satisfy a binding chained off it.
        ALTER TABLE aa_workflow_sessions ADD COLUMN IF NOT EXISTS mcp_candidates JSONB DEFAULT '{}'::jsonb;
        -- Bumped on every slots_filled write. An MCP pass records the version it
        -- resolved its arguments against, so a request that stalled and resumed
        -- cannot claim a binding using a snapshot the session has moved past.
        ALTER TABLE aa_workflow_sessions ADD COLUMN IF NOT EXISTS slots_version INTEGER NOT NULL DEFAULT 0;
      `);

      // An earlier revision of mcp_runs defaulted to an array. Normalize any rows
      // created under it so the merge semantics above hold.
      await client.query(`
        UPDATE aa_workflow_sessions
           SET mcp_runs = '{}'::jsonb
         WHERE mcp_runs IS NOT NULL AND jsonb_typeof(mcp_runs) <> 'object';
        ALTER TABLE aa_workflow_sessions ALTER COLUMN mcp_runs SET DEFAULT '{}'::jsonb;
      `);

      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_aa_workflow_sessions_work_item_id ON aa_workflow_sessions (work_item_id);
      ...
      // an earlier fix: persist alternative candidate values for low-confidence slots.
      // Idempotent ADD COLUMN IF NOT EXISTS. Stored as JSONB since alternatives is an array of
      // {value, confidence} objects.
      await client.query(`
        ALTER TABLE aa_workflow_item_status ADD COLUMN IF NOT EXISTS alternatives JSONB;
      `);

      // an earlier fix follow-up: an unambiguous marker for "the agent typed this
      // value by hand" (via the manual complete/slot routes), set ONLY by
      // those two routes going forward. Replaces trying to infer manual-
      // edit-ness from completed_by='agent' + confidence_score=1 — that
      // pair is NOT unique to a manual edit: the analyze route's auto-
      // complete branch can ALSO write completed_by='agent' (when the AGENT
      // SPOKE the value) with an LLM confidence of exactly 1.0 for an
      // unambiguous utterance, which is indistinguishable from a manual
      // edit under the old heuristic and would wrongly block that spoken
      // value from ever being corrected again.
      await client.query(`
        ALTER TABLE aa_workflow_item_status ADD COLUMN IF NOT EXISTS is_manual_edit BOOLEAN NOT NULL DEFAULT FALSE;
      `);

      // Codex review (PR #1387, P1): marks a completion as copied from
      // another slot (currently only the the reference workflow pickup_facility <-
      // caller_facility "same as requesting facility" guard in
      // analyze/route.js) so a later change to that source premise can find
      // and resync/reopen the copy instead of leaving a stale value in
      // place - a wrong pickup facility silently feeding MCP lookup and
      // dispatch. Nullable, no backfill needed: NULL is correct for every
      // pre-existing row, since none of them were ever derived.
      await client.query(`
        ALTER TABLE aa_workflow_item_status ADD COLUMN IF NOT EXISTS derived_from_slot TEXT;
      `);

      // Read-back correction handling (PR #1281): tags a workflow item's
      // status row as a correction candidate so generate-suggestion can gate
      // suggestion targeting on it (route.js checks itemStatus?.is_correction)
      // and workflow-store.js writes it on every status upsert. Shipped in
      // application code without a matching migration here, so any
      // environment whose schema predates this column throws
      // 'column "is_correction" ... does not exist' on every write that
      // touches this table - not just correction-tagged writes, since the
      // column is included unconditionally in the upsert's column list.
      // Idempotent, same simple pattern as the alternatives column above; no
      // backfill needed since FALSE is the correct value for every
      // pre-existing row (none of them were ever a live correction candidate).
      await client.query(`
        ALTER TABLE aa_workflow_item_status ADD COLUMN IF NOT EXISTS is_correction BOOLEAN NOT NULL DEFAULT FALSE;
      `);
      ...
      await client.query(`
        ...
        CREATE INDEX IF NOT EXISTS idx_quality_evaluations_work_item_id ON quality_evaluations (work_item_id);
      ...
      try {
        ...
        `);
        await client.query(`CREATE INDEX IF NOT EXISTS idx_aa_ai_handoff_events_work_item_id ON aa_ai_handoff_events (work_item_id);`);
      ...
      // Seed the "Sales" queue (+ wrapup-code assignments) BEFORE the
      // Default Call Flow below — see lib/seed-default-queue.mjs's header
      // for why this ordering is defensive-but-not-strictly-required (the
      // flow's enqueue node resolves queue_name at CALL time, long after
      // both seeds have run either way). Also BEFORE the owner seed below
      // now, since seedDefaultOwner() assigns the owner into this queue
      // (cc_queue_user_assignments.queue_id has an FK to cc_queues.id) —
      // the queue row must exist first.
      try {
        const { seedDefaultQueue } = await import("./seed-default-queue.mjs");
        await seedDefaultQueue();
      } catch (queueSeedError) {
        // Log but don't fail schema creation if seeding fails
        schemaLogger.warn("seed_queue_failed");



      }

      // Seed default owner user from environment variables — runs AFTER
      // the queue seed above so the owner's "Sales" queue assignment
      // (added so the owner shows up as an active agent in that queue,
      // not just an account with the `owner` role) has a queue row to
      // point at.
      try {
      ...
      // Seed the Default Call Flow (+ its dedicated Telnyx voice app's
      // phone-number assignment, when known) from env vars written by the
      // deploy wizard's Telnyx provisioning step. Runs on every boot for
      // BOTH local and cloud targets — see lib/seed-default-call-flow.mjs's
      // header comment for why this replaced the old wizard-side,
      // Local-target-only seeding path.
      try {
        const { seedDefaultCallFlow } = await import("./seed-default-call-flow.mjs");
        await seedDefaultCallFlow();
      } catch (callFlowSeedError) {
        // Log but don't fail schema creation if seeding fails
        schemaLogger.warn("seed_call_flow_failed");
      ...
      try {
        const { ensureWorkflowTestingAction } = await import("./call-generator/workflow-testing.mjs");
        await ensureWorkflowTestingAction(client);
      } catch (workflowTestingSeedError) {
        schemaLogger.warn("seed_workflow_testing_action_failed");
      }
      ...
      const { ensureGeneratorRuntimeSchema } = await import("./call-generator/schema.mjs");
      await ensureGeneratorRuntimeSchema(client);

      // Hard phones provisioning inventory (Polycom / Yealink / AudioCodes).
      // Each phone carries its own Telnyx SIP credential connection (sip_username /
      ...
      // CTI sessions for the Telnyx fallback driver — one row per hardphone
      // bridge session. The phone leg is controlled by phone_call_control_id;
      // the target leg is tracked for webhook lifecycle/diagnostics.
      await client.query(`
        CREATE TABLE IF NOT EXISTS hp_cti_sessions (
          id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
          phone_id UUID NOT NULL,
          call_control_id TEXT,
          phone_call_control_id TEXT,
      ...
      `);
      await client.query(`ALTER TABLE hp_cti_sessions ADD COLUMN IF NOT EXISTS phone_call_control_id TEXT`);
      await client.query(`ALTER TABLE hp_cti_sessions ADD COLUMN IF NOT EXISTS target_call_control_id TEXT`);
      await client.query(`ALTER TABLE hp_cti_sessions ADD COLUMN IF NOT EXISTS call_session_id TEXT`);
      await client.query(`UPDATE hp_cti_sessions SET phone_call_control_id = call_control_id WHERE phone_call_control_id IS NULL AND call_control_id IS NOT NULL`);
      ...
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_cti_sessions_phone_ccid ON hp_cti_sessions(phone_call_control_id)`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_cti_sessions_target_ccid ON hp_cti_sessions(target_call_control_id)`);

      // Outbound local bridge registry. The bridge token is shown once at
      // enrollment time; only its SHA-256 hash is persisted for audit/admin UX.
      await client.query(`
        CREATE TABLE IF NOT EXISTS hp_local_bridges (
          id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      `);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_local_bridges_bridge_id ON hp_local_bridges(bridge_id)`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_local_bridges_status ON hp_local_bridges(status)`);

      await client.query(`
        CREATE TABLE IF NOT EXISTS hp_local_bridge_commands (
          id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      `);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_local_bridge_commands_bridge ON hp_local_bridge_commands(bridge_id, created_at DESC)`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_local_bridge_commands_phone ON hp_local_bridge_commands(phone_id, created_at DESC)`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS local_bridge_id TEXT`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_phones_local_bridge ON hp_phones(local_bridge_id)`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS phone_name TEXT`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS telnyx_connection_id TEXT`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS telnyx_connection_name TEXT`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS assigned_phone_number_id TEXT`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS assigned_phone_number TEXT`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS sip_registration_status TEXT`);
      await client.query(`ALTER TABLE hp_phones ADD COLUMN IF NOT EXISTS sip_registration_status_at TIMESTAMPTZ`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_hp_phones_telnyx_connection ON hp_phones(telnyx_connection_id)`);

      // Roles and permissions (the internal documentation).
      // cc_roles stores role -> permission keys and scopes; the catalogue of
      // keys lives in lib/authz/permissions.mjs. cc_authz_audit records who
      // changed which role or assignment, with before/after snapshots.
      await client.query(`
        CREATE TABLE IF NOT EXISTS cc_roles (
          ...
          origin TEXT NOT NULL DEFAULT 'custom' CHECK (origin IN ('system', 'preset', 'custom')),
        ...
        CREATE INDEX IF NOT EXISTS idx_cc_roles_permissions ON cc_roles USING GIN (permissions);
        CREATE TABLE IF NOT EXISTS cc_authz_audit (
          id TEXT PRIMARY KEY,
          actor_id TEXT,
          action TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_cc_authz_audit_target ON cc_authz_audit (target_type, target_id, created_at DESC);
      `);
      // System roles are rewritten from the catalogue on every start; shipped
      // position roles are inserted once and never overwritten (decision D-19).
      try {
        const { seedRoles } = await import("./seed-roles.mjs");
        await seedRoles();
      } catch (rolesSeedError) {
        schemaLogger.warn("seed_roles_failed", { error: rolesSeedError?.message || String(rolesSeedError) });
      }
      ...
      // ACD Core tables (the internal documentation §3).
      const { ensureAcdSchema } = await import("./acd/schema.mjs");
