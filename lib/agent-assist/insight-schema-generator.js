...
export function generateSlotsSchema(workflowSlots) {
  ...
  for (const slot of workflowSlots) {
    ...
    // Build slot-level description with context
    let slotDesc = slot.description || slot.label;
    if (slot.hints?.length > 0) {
      slotDesc += `. Detection hints: ${slot.hints.join(", ")}`;
    }
    ...
    slotProperties[slot.slot_name] = {
      ...
      properties: {
        ...
        confidence: {
          ...
          minimum: 0,
          maximum: 1,
          description: "1.0=explicitly stated, 0.8-0.9=clearly inferred, <0.6=omit slot entirely",
          nullable: true,
        },
        source_utterance: {
          type: "string",
          description: "Exact verbatim quote from caller where this value was mentioned",
          nullable: true,
        },
      },
...
export function generateSlotsInstructions(workflow, stages) {
  ...
  instructions += `### Critical: Handling Missing Information\n`;
  instructions += `- **OMIT slots entirely** if information was not mentioned in the conversation\n`;
  instructions += `- **NEVER** set value to "null", "None", "N/A", "unknown", or empty string ""\n`;
  instructions += `- **NEVER** guess or infer information that wasn't stated\n`;
  instructions += `- Only include slots where the caller **explicitly provided** the information\n\n`;
  ...
  instructions += `- **< 0.8** = OMIT the slot - do not include uncertain extractions\n\n`;
  instructions += `### Source Utterance\n`;
  instructions += `- Must be the **exact verbatim quote** from the conversation\n`;
  instructions += `- Copy the caller's words exactly as spoken\n\n`;
  ...
  instructions += `- Booleans: true/false only (not "yes"/"no" strings)\n`;
  instructions += `\n## Example Output\n`;
  ...
  instructions += `    }\n`;
  instructions += `  },\n`;
  instructions += `  "completed_stages": []\n`;
  instructions += `}\n`;
  instructions += `\`\`\`\n`;
  instructions += `Notice: transport_reason is NOT included because it wasn't found.\n`;
