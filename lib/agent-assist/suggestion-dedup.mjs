...
/**
 * Key a suggestion by its target: the workflow item plus the target mode
 * (e.g. "confirm_slot" vs "collect_missing_slot"). One entry exists per target.
 *
 * @param {{itemId?: string, targetMode?: string}|null|undefined} s
 * @returns {string}
 */
export function suggestionTargetKey(s) {
  return `${s?.itemId ?? "none"}:${s?.targetMode ?? "default"}`;
}
