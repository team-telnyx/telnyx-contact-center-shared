 ...
 * @param {string} [options.model] - AI model to use (e.g. from workflow llm_model), defaults to openai/gpt-4o
...
export async function analyzeWorkflowTranscript({
  ...
  try {
    ...
    if (!content) {
      console.error("[WorkflowAnalyzer] No content in response");
      return getDefaultResult();
    }
    ...
    try {
      ...
      console.error("[WorkflowAnalyzer] JSON parse error:", parseError);
      return getDefaultResult();
    }
...
function normalizeAnalysisResult(analysis, pendingItems) {
  ...
  // Normalize sentiment
  let sentiment = (analysis.sentiment || "neutral").toLowerCase();
  ...
  return {
    ...
    detected_intent: analysis.detected_intent || "unknown",
    sentiment,
