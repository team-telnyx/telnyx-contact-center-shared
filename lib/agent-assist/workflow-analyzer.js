...
export async function analyzeWorkflowTranscript({
  ...
  try {
    ...
    // live route can send up to 12 items. Too small a cap truncates the JSON, and
    // the parser then falls back to an empty result — losing ALL captures for the
    // utterance, not just the chips.
    //
    // Each low-confidence slot needs 2-3 alternatives (~80 tokens each), so the
    // per-item budget must accommodate the worst case where every item is a slot
    // with alternatives. 250 tokens/item gives room for the slot value + 3
    // alternatives + overhead.
