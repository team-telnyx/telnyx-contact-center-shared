...
export async function analyzeWorkflowTranscriptBatch({
  ...
  try {
    ...
    if (!rawContent) {
      console.error("[WorkflowAnalyzer] No batch content in response", JSON.stringify(data?.choices?.[0]?.message));
      return getDefaultResult({ includeIntent, includeSentiment });
    }
    ...
    try {
      ...
      const jsonMatch = content.match(/\{[\s\S]*\}/);
      if (!jsonMatch) {
        console.error("[WorkflowAnalyzer] Batch JSON parse error:", parseError, "Content:", content.slice(0, 200));
        return getDefaultResult({ includeIntent, includeSentiment });
      }
      try {
        analysis = JSON.parse(jsonMatch[0]);
