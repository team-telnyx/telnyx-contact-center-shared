 ...
 *
 * Uses Telnyx AI chat completion with GPT-4o for dynamic intent and sentiment analysis
 */
...
export async function analyzeTranscription(transcript) {
  ...
  try {
    // Build the prompt for GPT-4o to analyze the transcription
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  "tags": ["tag1", "tag2", "tag3"]
    ...
    // Call Telnyx AI chat completion with GPT-4o
    ...
    if (!content) {
      console.error("[AgentAssist] No content in response:", data);
      return {
        intent: "general inquiry",
        sentiment: "neutral",
        sentimentScore: 50,
    ...
    try {
      ...
      console.error("[AgentAssist] Failed to parse AI response:", content);
      return {
        intent: "general inquiry",
        sentiment: "neutral",
        sentimentScore: 50,
    ...
    if (
      ...
      console.error("[AgentAssist] Invalid analysis structure:", analysis);
      return {
        intent: "general inquiry",
        sentiment: "neutral",
        sentimentScore: 50,
    ...
    console.error("[AgentAssist] Error analyzing transcription:", error);
    // Fallback to default values on error
    return {
      intent: "general inquiry",
      sentiment: "neutral",
 ...
 * Since intents are now dynamically generated by GPT-4o, we just capitalize them properly
