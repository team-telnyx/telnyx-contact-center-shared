...
export async function detectLanguage(text) {
  if (!GOOGLE_TRANSLATE_API_KEY || !text) {
    if (!GOOGLE_TRANSLATE_API_KEY) {
      console.warn("[TranslationService] GOOGLE_TRANSLATE_API_KEY is missing");
    }
    return null;
  }

  console.log("[TranslationService] Detect language via Google Translate");
  ...
  try {
    ...
    if (!detection?.language) {
      console.warn("[TranslationService] Detect language returned empty result");
      return null;
    }

    console.log("[TranslationService] Detected language:", detection.language, "confidence:", detection.confidence);
...
export async function translateText({ text, sourceLanguage, targetLanguage }) {
  if (!GOOGLE_TRANSLATE_API_KEY || !text || !targetLanguage) {
    if (!GOOGLE_TRANSLATE_API_KEY) {
      console.warn("[TranslationService] GOOGLE_TRANSLATE_API_KEY is missing");
    }
    return null;
  ...
  console.log("[TranslationService] Translate text via Google Translate", {
  ...
  try {
    ...
    if (!translation?.translatedText) {
      console.warn("[TranslationService] Translate returned empty result");
      return null;
    }

    console.log("[TranslationService] Translate ok", {
