...
function bestConversationStage(stages, conversationText) {
  let best = null;
  ...
  for (const stage of sortedStages(stages)) {
    ...
    if (!best || score > best.score) {
      best = { stage, matchedItem: bestItem.item, score, itemScore: bestItem.score };
    }
  }
...
function result({ stage, item, mode, reason, blockedItem = null, matchedItem = null, score = 0 }) {
...
export function resolveSuggestedResponseTarget({
  ...
  const matchedStage = bestConversationStage(stages, conversationText);
