...
export function buildWorkflowAnalysisSystemPrompt({
  ...
  const itemsList = pendingItems.map((item, index) => {
   ...
   Stage: ${item.stage_name}`;
    ...
    // Only items re-included after already completing (accumulating notes, or
    // read-back-stage correction candidates) carry current_status "completed"
    // in this list — a plain pending item never does. Flag those explicitly so
    // the model doesn't treat a value it already has as still-blank.
    if (item.current_status === "completed" && item.completed_value !== undefined) {
      itemDesc += `\n   Currently collected value: ${JSON.stringify(item.completed_value)} (ALREADY CONFIRMED — do not resurface just because it's mentioned again; see correction rules below if this is a correction candidate)`;
    }
  ...
  const currentFocusStr = focusLabel
    ? `\n\n## Current Collection Focus\n\nThe agent is currently asking the customer for: **${currentTarget.label || currentTarget.slotName}**${currentTarget.slotName ? ` (slot: \`${currentTarget.slotName}\`)` : ""}.\n\n- Treat the customer's direct answer as the value for THIS slot when it plausibly fits, before matching it to any other pending slot.\n- The surface form can mislead, especially with speech-to-text errors: a facility/organization name may sound like a person's name (e.g. a destination hospital heard as "John Meyer" / "John Miller" is really "John Mabry Hospital") — assign it to the facility/location slot, NOT a patient-name or caller-name slot. A name given while the focus is a facility/location slot is almost always that facility.\n- If the focus slot is a yes/no (boolean) slot, a bare affirmative or negative from the customer ("yes" / "yeah" / "correct" / "no" / "nope" / "none" / "not that we know of") is the answer to THIS slot — return it as the boolean value for the focus slot (yes -> true, no -> false), not for a different yes/no slot elsewhere. A "no" is a real captured value (false), not a missing answer.\n- This focus is a tie-breaker for ambiguous answers, not a hard filter: still capture a value the customer CLEARLY volunteers for a different, explicitly-named slot (e.g. "the patient's date of birth is ...").`
    : "";

  // Bleed guard: a short numeric/ordinal fragment (e.g. "eight") arriving right
  // after a date or multi-part slot was filled can bleed into the next pending
  // slot (e.g. weight = 8). The prompt tells the model to recognise these stray
  // tails and return empty results instead of assigning them to a different slot.
  const bleedGuardStr = bleedGuardSlot
    ? `\n\n## Stray Fragment Guard\n\nThe slot \`${bleedGuardSlot.slotName}\` (type: ${bleedGuardSlot.slotType || "text"}) was completed moments ago with value \`${bleedGuardSlot.value}\`.\n\nIf the current transcript is 1–3 words AND it is a number, ordinal, or short numeric expression (e.g. "eight", "sixty-eight", "the eighth", "12"), check whether it could be the trailing fragment of the caller's answer for \`${bleedGuardSlot.slotName}\` rather than an independent answer to the current focus slot:\n\n- If the fragment plausibly completes or extends the just-filled value (e.g. "eight" after a DOB stored as a year ending in "60" → the caller said "nineteen sixty-eight"), return an **empty** completed_items array — do NOT assign it to any pending slot.\n- If the fragment is clearly a self-contained answer to the current focus slot (e.g. "No", "Yes", a hospital name, or a number that cannot possibly extend the just-filled value), extract normally.`
    : "";

  // Stage boundary: when the LLM receives items from multiple stages it can
  // mistakenly assign the same value to structurally similar slots in different
  // stages (e.g. pickup_department="ICU" bleeds into destination_department="ICU"
  // even though the agent never asked about the destination). Tell the model the
  // currently active stage so it can honour the stage boundary.
  const stageBoundaryStr = currentStageName
    ? `\n\n## Stage Boundary Rule\n\nThe **currently active stage** is: **${currentStageName}**.\n\nItems in the pending list may belong to later stages. Apply these rules:\n- A value the customer provides in the context of the CURRENT STAGE fills ONLY current-stage slots — do NOT copy it to a structurally similar slot in a future stage (e.g. a pickup department/room/bed must not populate a destination department/room/bed).\n- Fill a future-stage slot ONLY when the transcript EXPLICITLY provides information that clearly belongs to that future stage — for example, when the agent has asked about the destination and the customer answers about the destination.\n- When in doubt, omit the future-stage slot rather than guessing.`
    : "";

  // Read-back/confirmation step: some items in the pending list are slots that
  // were already collected and confirmed earlier in the call, re-included here
  // ONLY as correction candidates (marked "ALREADY CONFIRMED" above) — not
  // because they still need collecting. This is the one moment a correction to
  // an earlier answer should be captured at all; outside of it, an already-
  // completed slot is never sent to the analyzer, so nothing else in the call
  // can accidentally overwrite a confirmed value.
  const correctionContinuationStr = correctionInProgress
    ? `\n\n- **A correction is already in progress**: the caller or agent flagged moments ago that a value is wrong. Speech-to-text often mangles the corrected value differently on each retry (e.g. "Twin Hill" / "Stonemere" / "John Miller" said in place of "John Mabry Hospital"), so the speaker may just restate a plain new value on this turn WITHOUT repeating "that's wrong" again. Treat that bare restatement as continuing the same correction and return the new value — but only when it plausibly matches what that slot holds (e.g. a facility name for a facility slot), not for an unrelated slot.`
    : "";
  // Emitted whenever correction candidates are present: the whole read-back
  // stage, or any other stage once someone explicitly said a value is wrong
  // (an earlier fix). Defaults to the read-back stage so older callers are unchanged.
  const correctionStr = (allowCorrections || isReadBackStage)
    ? `\n\n## Correction Handling\n\nSome pending items are marked "ALREADY CONFIRMED" with a currently collected value — these are correction candidates.\n\n- Only return one of these when the speaker EXPLICITLY states the existing value is wrong and gives a replacement — phrasing like "that's incorrect, it should be...", "change X from A to B", "actually, it's...", "no wait,...", "correction:". \n- Do NOT return one just because the value is mentioned again in passing, or because the caller re-confirms it ("yes, John Mabry, that's right") — that is confirmation, not a correction; omit it entirely.\n- When you do return a correction, extracted_value must be the NEW corrected value (not the old one), with confidence reflecting how clearly the new value was stated.\n- If a correction is ambiguous or only partially heard, omit it rather than guessing — a missed correction is safer than a wrong overwrite of a confirmed value.\n- Apply the same phonetic-mishearing tolerance used for normal collection: a facility/organization name can come through as a different-sounding phrase or a person's name (e.g. "John Meyer" / "John Miller" said for "John Mabry Hospital") — if the correction context makes the intended facility clear, use the intended value rather than the literal mis-transcription.${correctionContinuationStr}`
    : "";

  // The final "confirm all information is correct" item must only complete on
  // the CUSTOMER's own affirmative — not the agent reciting the read-back
  // script (which itself contains "is that correct?" and would otherwise
  // self-complete this item the moment it's read), and not a correction
  // request being mistaken for confirmation.
  const readBackConfirmationStr = isReadBackStage
    ? `\n\n## Final Confirmation Requires a Customer Affirmative\n\nOne pending item asks the caller to confirm the read-back is correct (e.g. "Confirm all information is correct"). Mark it completed ONLY when the CUSTOMER gives a clear, unambiguous affirmative to the read-back — e.g. "yes, that's correct", "yes", "that's right", "sounds good", "confirmed", "looks good". Do NOT mark it completed for:\n- The agent's own recitation of the read-back script (it is not the customer speaking, and its own wording ("is that correct?") must not be mistaken for the customer's answer)\n- A correction request ("that's wrong", "change X to Y", "no, actually...")\n- An ambiguous, partial, or non-committal response\n- Silence, filler, or an unrelated remark`
    : "";
...
${filledSlotsStr}${currentFocusStr}${stageBoundaryStr}${correctionStr}${readBackConfirmationStr}${bleedGuardStr}
...
- **One utterance can fill multiple slots.** When a single statement contains values for several pending slots, return a SEPARATE completed_items entry for each. Example: "ICU, room 412, bed B" -> department="ICU", room="412", bed="B". Example: "Jordan Rivera, born March 12 1968" -> patient name AND date of birth.
- **Names:** When SEPARATE first-name and last-name slots both exist and the caller gives a full name, split it — the final word is the last name and the remaining word(s) are the first name (e.g., "Sarah Thompson" -> first="Sarah", last="Thompson"; "Mary Jo Cline" -> first="Mary Jo", last="Cline"). If only a single combined name slot exists, use the whole name. NEVER put the SAME word in both the first-name and last-name slots. The first-name slot must hold the caller's GIVEN name; if you only hear a single word and it is the family name (or already appears in the last-name slot), put it in the LAST-name slot only and leave the first-name slot empty for the given name — do NOT fill the first-name slot with a surname just because the agent asked for "your name".
...
- **Don't infer a boolean value from shared wording alone — require an actual yes/no or a clear factual statement.** A slot label sharing a word root with something the customer said is not the same as the customer answering that slot. Concretely: "Air service declined for weather" (did another service **refuse/turn down** the flight because of weather?) is answered by a refusal statement ("they said the weather is too bad", "another crew turned it down") or an explicit yes/no — it is NOT answered by an unrelated remark that the weather is "declining" (i.e. deteriorating/getting worse), which describes current conditions, not a refusal, even though both use the word "declin-". If the only textual overlap is a shared word root and the customer never actually answered that specific question, omit the slot (or use low confidence) instead of guessing true.
