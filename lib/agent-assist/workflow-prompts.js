...
export function buildWorkflowAnalysisSystemPrompt({
  ...
  const correctionContinuationStr = correctionInProgress
    ? `\n\n- **A correction is already in progress**: the caller or agent flagged moments ago that a value is wrong. Speech-to-text often mangles the corrected value differently on each retry (e.g. "Twin Hill" / "Stonemere" / "John Miller" said in place of "John Muir Hospital"), so the speaker may just restate a plain new value on this turn WITHOUT repeating "that's wrong" again. Treat that bare restatement as continuing the same correction and return the new value — but only when it plausibly matches what that slot holds (e.g. a facility name for a facility slot), not for an unrelated slot.`
    : "";
  const correctionStr = isReadBackStage
    ? `\n\n## Correction Handling (Read-Back Stage)\n\nThis is the final read-back/confirmation step. Some pending items are marked "ALREADY CONFIRMED" with a currently collected value — these are correction candidates.\n\n- Only return one of these when the speaker EXPLICITLY states the existing value is wrong and gives a replacement — phrasing like "that's incorrect, it should be...", "change X from A to B", "actually, it's...", "no wait,...", "correction:". \n- Do NOT return one just because the value is mentioned again in passing, or because the caller re-confirms it ("yes, John Muir, that's right") — that is confirmation, not a correction; omit it entirely.\n- When you do return a correction, extracted_value must be the NEW corrected value (not the old one), with confidence reflecting how clearly the new value was stated.\n- If a correction is ambiguous or only partially heard, omit it rather than guessing — a missed correction is safer than a wrong overwrite of a confirmed value.\n- Apply the same phonetic-mishearing tolerance used for normal collection: a facility/organization name can come through as a different-sounding phrase or a person's name (e.g. "John Meyer" / "John Miller" said for "John Muir Hospital") — if the correction context makes the intended facility clear, use the intended value rather than the literal mis-transcription.${correctionContinuationStr}`
    : "";
