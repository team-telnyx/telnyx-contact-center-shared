 ...
 */
export function buildWorkflowAnalysisSystemPrompt({ pendingItems, slotsFilled }) {
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  "detected_intent": "brief intent description (2-3 words)",
  "sentiment": "positive|neutral|negative",
  "sentiment_score": 50
}

If no items were completed, return an empty array for completed_items.
Always include detected_intent, sentiment, and sentiment_score.`;
}
 ...
 */
export function buildBatchAnalysisPrompt({ transcripts, pendingItems, slotsFilled }) {
  const systemPrompt = buildWorkflowAnalysisSystemPrompt({ pendingItems, slotsFilled });
