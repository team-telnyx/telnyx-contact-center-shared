...
export function setupSessionMonitor() {
  ...
  // Set status to offline using sendBeacon for intentional disconnects
  // (logout/network loss). Page unloads are handled by the SSE grace timer so
  // a normal refresh does not force an otherwise available agent Offline.
