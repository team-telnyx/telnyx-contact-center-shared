...
class AzureSpeechSession {
  ...
  _onTurnEnd() {
    ...
    // Close the current WS (Azure s2s won't produce new turns on an existing connection)
    if (this.ws) {
      try { this.ws.close(1000, "Turn complete — reconnecting"); } catch (_) {}
      this.ws = null;
    }
    this.connected = false;
    // Fresh requestId for the new session
    this._requestId = randomUUID().replace(/-/g, "");
    // Reconnect after a short delay (let in-flight events settle)
    setTimeout(() => {
      if (!this.closed) {
