...
class AzureSpeechSession {
  ...
  connect() {
    ...
    console.log(`[AzureSpeech] URL: wss://${this.region}.stt.speech.microsoft.com/speech/universal/v2?language=auto&...`);
    console.log(`[AzureSpeech] API key present: ${!!this.apiKey}, region: ${this.region}`);
    ...
    this.ws.on("open", () => {
      console.log(`[AzureSpeech] ✅ Connected to Azure (track=${this.track}, callControlId=${this.callControlId})`);
      this._sendSpeechConfig();
    });

    this.ws.on("message", (data) => {
      try {
        // Azure Speech WebSocket messages have headers + body separated by \r\n\r\n
        // Format: "Path: turn.start\r\nContent-Type: ...\r\n\r\n{...json...}"
        const text = Buffer.isBuffer(data) ? data.toString("utf-8") : data.toString();

        // Parse Azure protocol: split on first \r\n\r\n to separate headers from body
        const separatorIdx = text.indexOf("\r\n\r\n");
        let path = null;
        let bodyText = text;

        if (separatorIdx !== -1) {
          const headerSection = text.substring(0, separatorIdx);
          bodyText = text.substring(separatorIdx + 4);
          // Extract Path header
        ...
        if (path === "turn.start" || path === "speech.startDetected") {
          console.log(`[AzureSpeech] ← Azure: ${path} (track=${this.track})`);
          return;
        }
        if (path === "turn.end") {
          console.log(`[AzureSpeech] ← Azure: turn.end (track=${this.track})`);
          return;
        }

        if (path === "speech.hypothesis") {
          // Interim result — log lightly
          console.log(`[AzureSpeech] ← hypothesis (track=${this.track}): "${msg.Text ?? ""}"`);
          return;
        }

        if (path === "speech.phrase" || path == null) {
          console.log(`[AzureSpeech] ← phrase (track=${this.track}): RecognitionStatus=${msg.RecognitionStatus ?? "N/A"} Text="${msg.DisplayText ?? ""}" Lang=${msg.PrimaryLanguage?.Language ?? "N/A"}`);
          this._onAzureMessage(msg);
        } else {
          console.log(`[AzureSpeech] ← unknown path "${path}" (track=${this.track})`);
        }
      } catch (e) {
        console.warn(`[AzureSpeech] Error processing Azure message (track=${this.track}):`, e.message);
      }
    });
  ...
  /**
   * Build an Azure Speech WebSocket protocol message.
   * Azure v2 requires text messages in the format:
   *   Path: <path>\r\nContent-Type: application/json; charset=utf-8\r\nX-RequestId: <id>\r\nX-Timestamp: <ts>\r\n\r\n<JSON body>
   */
  _buildAzureMessage(path, bodyObj) {
  ...
  _sendSpeechConfig() {
    ...
    const message = this._buildAzureMessage("speech.config", speechConfig);
    console.log(`[AzureSpeech] → Sending speech.config (track=${this.track}, translationEnabled=${!!this.config.translationEnabled}, targetLanguage="${this.config.targetLanguage || "none"}")`);
    ...
    this.ws.send(message, (err) => {
      ...
      console.log(`[AzureSpeech] speech.config sent (track=${this.track}) — ready for audio`);
      this.connected = true;
      ...
      if (this.pendingAudio.length > 0) {
        ...
        console.log(`[AzureSpeech] No buffered audio to flush (track=${this.track}) — waiting for live audio`);
      }
    });
  }
  ...
  sendAudio(pcmuBase64) {
    ...
    if (!this.connected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
      ...
      // Log every 50 buffered chunks to show audio IS arriving before Azure connects
      if (this.pendingAudio.length % 50 === 0) {
        console.log(`[AzureSpeech] Buffering audio (track=${this.track}, buffered=${this.pendingAudio.length}, connected=${this.connected})`);
      }
      return;
    }

    this._audioChunkCount = (this._audioChunkCount || 0) + 1;
    // Log first chunk and then every 200 to confirm audio is flowing
    if (this._audioChunkCount === 1 || this._audioChunkCount % 200 === 0) {
      console.log(`[AzureSpeech] → Sending audio to Azure (track=${this.track}, chunk #${this._audioChunkCount}, bytes=${pcm16Buffer.length})`);
    }
...
export async function startAzureTranscription(callControlId, config, interactionId, agentUsername) {
  ...
  // Env vars: AZURE_SERVICE_REGION and AZURE_SUBSCRIPTION_KEY (config fields removed from UI)
  ...
  console.log(`[AzureSpeech] Starting Azure transcription (callControlId=${callControlId}, interactionId=${interactionId}, agent=${agentUsername}, hasTelnyxSession=${!!telnyxSession}, region=${region}, apiKeyPresent=${!!apiKey})`);
