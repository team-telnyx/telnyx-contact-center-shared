...
class AzureSpeechSession {
  ...
  constructor({ region, apiKey, track, config, callControlId, interactionId, agentUsername }) {
    ...
    this.closed = false;
    this._reconnecting = false; // True while intentionally closing for reconnect (suppress closed=true)
    this.pendingAudio = [];
    this._audioChunkCount = 0;
    // Single requestId for ALL messages in this session (speech.config + every audio frame).
    // Using a new UUID per frame causes Azure to treat each frame as a separate request → TooManyRequests.
    this._requestId = randomUUID().replace(/-/g, "");
  }
  ...
  connect() {
    ...
    this.ws.on("close", (code, reason) => {
      ...
      this.connected = false;
      // Do NOT set closed=true when we intentionally closed the WS to reconnect after turn.end.
      // _reconnecting flag is cleared by _onTurnEnd's setTimeout before calling connect().
  ...
  _buildTextMsg(path, bodyObj) {
    // Use session-scoped requestId for all messages
    const timestamp = new Date().toISOString();
    const headers = [
      `Path: ${path}`,
      `Content-Type: application/json; charset=utf-8`,
      `X-RequestId: ${this._requestId}`,
  ...
  _buildAudioMsg(audioBuffer) {
    // Same requestId as speech.config — critical for continuous streaming
    const timestamp = new Date().toISOString();
  ...
  _sendSpeechConfig() {
    ...
    // speech.config MUST declare audio format for streaming PCM.
    // Without audio.source.format, Azure treats each binary frame as a new audio stream.
    const speechConfig = {
      context: {
  ...
  _handleMessage(data) {
    ...
    if (path === "turn.start") {
      console.log(`[AzureSpeech] ← turn.start (track=${this.track})`);
      return;
    }
    if (path === "speech.startDetected") {
      console.log(`[AzureSpeech] ← speech.startDetected (track=${this.track})`);
      return;
    }
    if (path === "speech.endDetected") {
      console.log(`[AzureSpeech] ← speech.endDetected (track=${this.track})`);
      return;
    }
    if (path === "turn.end") {
      // Azure s2s endpoint closes the recognition turn after each speech segment.
      // We must reconnect to process subsequent speech — the same WS connection
      // will NOT generate new turn.start events after turn.end.
      this._onTurnEnd();
      return;
    }
  ...
  _onTurnEnd() {
    if (this.closed) return;
    console.log(`[AzureSpeech] ← turn.end (track=${this.track}) — reconnecting for next speech segment`);
    // Set _reconnecting BEFORE closing WS so the close handler won't set this.closed = true
    this._reconnecting = true;
    this.connected = false;
    if (this.ws) {
      try { this.ws.close(1000, "Turn complete — reconnecting"); } catch (_) {}
      this.ws = null;
    }
    // Fresh requestId for the new Azure session
    this._requestId = randomUUID().replace(/-/g, "");
    // Reconnect after a short delay; clear _reconnecting flag just before connecting
    setTimeout(() => {
      if (this.closed) return; // stopAzureTranscription() was called while waiting
      this._reconnecting = false;
      console.log(`[AzureSpeech] Reconnecting (track=${this.track})`);
      this.connect();
    }, 200);
  }
  ...
  _sendPcmChunk(pcm16) {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) return;
    try {
      // Raw PCM16 only — speech.config declares the audio format.
      // No WAV headers in audio frames.
      const binaryMsg = this._buildAudioMsg(pcm16);
  ...
  _onPhraseMessage(msg, path) {
    ...
    if (this.config.translationEnabled) {
      // s2s endpoint returns Translation object in translation.phrase
      // Accept both TranslationStatus==="Success" and cases where field is absent
      const translations = msg.Translation?.Translations;
      const translationOk =
        !msg.Translation?.TranslationStatus ||
        msg.Translation.TranslationStatus === "Success";
      if (translationOk && translations?.length && translations[0].Text) {
        translation = {
    ...
    console.log(
      `[AzureSpeech] ✅ Phrase (track=${this.track}): original="${text}"` +
        (translation ? ` | translation [${translation.sourceLanguage}→${translation.targetLanguage}]="${translation.text}"` : " | no translation")
    );
  ...
  close() {
    ...
    if (this.ws) {
      try {
        // Signal end of audio stream with empty binary frame before closing
        if (this.ws.readyState === WebSocket.OPEN) {
          this.ws.send(this._buildAudioMsg(Buffer.alloc(0)), { binary: true });
        }
...
class TelnyxStreamSession {
  constructor(callControlId, clientState, interactionId = null) {
    this.callControlId = callControlId;
    this.clientState = clientState;
    this.interactionId = interactionId;
    // Support both Telnyx track name formats: "inbound_track"/"outbound_track" AND "inbound"/"outbound"
  ...
  // Get pre-buffered audio for a logical track (handles both name formats)
  getBuffer(logicalTrack) {
    // logicalTrack = "inbound" or "outbound"
    ...
    try {
      ...
      switch (msg.event) {
        ...
        case "media": {
          ...
          // Diagnostic: log first occurrence of each track type + stats every 500 chunks
          if (!session._trackCounts) session._trackCounts = {};
          session._trackCounts[track] = (session._trackCounts[track] || 0) + 1;
          const tc = session._trackCounts[track];
          if (tc === 1) {
            console.log(`[AzureSpeech] 📡 First media chunk (callControlId=${callControlId}, track=${track})`);
          } else if (tc % 500 === 0) {
            const summary = Object.entries(session._trackCounts).map(([t, c]) => `${t}=${c}`).join(", ");
            console.log(`[AzureSpeech] 📡 Track stats: ${summary}`);
          }

          const activeSessions = globalThis.__azureActiveSessions.get(callControlId);
          if (activeSessions) {
            // Telnyx may send track as "inbound_track"/"outbound_track" OR "inbound"/"outbound"
            const isInbound = track === "inbound_track" || track === "inbound";
            const azureTrack = isInbound ? "inbound" : "outbound";
            const azureSession = activeSessions[azureTrack];
            if (azureSession && !azureSession.closed) {
              azureSession.sendAudio(payload);
            } else {
              // Session closed/missing — log once
              if (tc === 1 || tc % 500 === 0) {
                console.log(`[AzureSpeech] ⚠️  No active Azure session for track=${azureTrack} (closed=${azureSession?.closed})`);
              }
            }
          } else {
...
export async function startAzureTranscription(callControlId, config, interactionId, agentUsername) {
  ...
  // Stamp interactionId on the TelnyxStreamSession so the webhook-handler fallback
  // scan can correctly match sessions to interactions in concurrent-call scenarios
  ...
  // inbound  = caller side  → speaks sourceLanguage, translate TO targetLanguage
  // outbound = agent side   → speaks targetLanguage, translate TO sourceLanguage (reversed!)
  const inboundSession = new AzureSpeechSession({
    ...sessionOpts,
    track: "inbound",
    config: {
      ...sessionOpts.config,
      sourceLanguage: config.sourceLanguage || "en-US",
      targetLanguage: config.targetLanguage || "pl-PL",
    },
  });
  const outboundSession = new AzureSpeechSession({
    ...sessionOpts,
    track: "outbound",
    config: {
      ...sessionOpts.config,
      sourceLanguage: config.targetLanguage || "pl-PL",
      targetLanguage: config.sourceLanguage || "en-US",
    },
  });
