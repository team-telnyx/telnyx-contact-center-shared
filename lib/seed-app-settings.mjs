...
export async function seedDefaultAppSettings() {
  try {
    ...
    if (!pool) {
      console.error("[Seed] Database connection failed");
      return false;
    }
    ...
    try {
      ...
      if (
        ...
        console.error(
          "[Seed] Failed to parse colors from globals.css - no colors found"
        );
        return false;
      }
      ...
      if (checkResult.rows.length > 0) {
        ...
        if (lightEmpty || darkEmpty || lightHexEmpty || darkHexEmpty) {
          ...
          console.log(
            `[Seed] Updated app settings with default colors from globals.css (light: ${
              lightEmpty ? "filled" : "kept"
            }, dark: ${darkEmpty ? "filled" : "kept"}, lightHex: ${
              lightHexEmpty ? "filled" : "kept"
            }, darkHex: ${darkHexEmpty ? "filled" : "kept"})`
          );
        } else {
          console.log(
            "[Seed] App settings already exist with colors, skipping seed"
          );
        }
        return true;
      }
      ...
      console.log(
        `[Seed] Extracted ${
          Object.keys(parsedColors.light).length
        } light theme colors and ${
          Object.keys(parsedColors.dark).length
        } dark theme colors from globals.css`
      );
      console.log(
        `[Seed] Hex values - Light: ${
          Object.keys(defaultThemeColorsHex.light).length
        }, Dark: ${Object.keys(defaultThemeColorsHex.dark).length}`
      );
      ...
      console.log("[Seed] Default app settings seeded successfully");
      return true;
    } finally {
    ...
    if (error.code === "57014") {
      console.log(
        "[Seed] Seeding skipped due to timeout (settings may already exist)"
      );
    } else {
      console.error("[Seed] Error seeding app settings:", error.message);
    }
    return false;
  }
}
