export const FORM_COMPONENT_TYPES = ["section", "row", "columns", "grid", "flex", "spacer", "divider", "codeblock", "hero", "stats", "card", "richtext", "accordion", "avatar", "label", "badge", "text", "textarea", "select", "radio", "checkbox", "switch", "slider", "datetime", "button", "image", "context_value", "hidden"];
...
export const FORM_COMPONENT_REGISTRY = {
  ...
  hero: { label: "Hero", data: false }, stats: { label: "Stats", data: false }, card: { label: "Card", data: false }, richtext: { label: "Rich text", data: false }, accordion: { label: "Accordion", data: false }, avatar: { label: "Avatar", data: false },
...
function makeAccordionItemId(fieldId = "field", index = 0) {
  const base = String(fieldId || "field").trim().replace(/[^A-Za-z0-9_:-]/g, "_") || "field";
  return `${base}_accordion_${index + 1}`;
}
function normalizeAccordionItems(items = [], fieldId = "field") {
  if (!Array.isArray(items)) return [];
  const usedIds = new Set();
  return items.map((item, index) => {
    const source = item && typeof item === "object" ? item : { title: String(item || `Item ${index + 1}`), content: "" };
    let internalId = String(source._id || source.id || makeAccordionItemId(fieldId, index)).trim() || makeAccordionItemId(fieldId, index);
    if (usedIds.has(internalId)) {
      const root = internalId;
      let suffix = 2;
      while (usedIds.has(`${root}_${suffix}`)) suffix += 1;
    ...
    usedIds.add(internalId);
    return { ...source, _id: internalId, title: source.title ?? `Item ${index + 1}`, content: source.content ?? source.body ?? source.description ?? "", defaultOpen: Boolean(source.defaultOpen || source.default_open) };
  });
}

function normalizeImageScale(value) {
  const number = Number(value);
  return Number.isFinite(number) ? Math.min(200, Math.max(50, number)) : undefined;
}

function normalizeFieldProps(type, props = {}, fieldId = "field") {
  const next = { ...props };
  ...
  if (["avatar", "hero", "card", "image"].includes(type)) {
    const imageScale = normalizeImageScale(next.imageScale);
    if (imageScale === undefined) delete next.imageScale; else next.imageScale = imageScale;
  }
  if (type === "accordion") {
    next.items = normalizeAccordionItems(next.items, fieldId);
    next.type = next.type === "multiple" ? "multiple" : "single";
    next.collapsible = next.type === "multiple" ? true : next.collapsible !== false;
  }
  return next;
}
