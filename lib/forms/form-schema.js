export const FORM_COMPONENT_TYPES = ["section", "row", "columns", "grid", "flex", "spacer", "hero", "stats", "card", "richtext", "label", "text", "textarea", "select", "radio", "checkbox", "button", "image", "context_value", "hidden"];
export const FORM_COMPONENT_REGISTRY = {
  section: { label: "Section", data: false }, row: { label: "Row", data: false }, columns: { label: "Columns", data: false }, grid: { label: "Grid", data: false }, flex: { label: "Flex", data: false }, spacer: { label: "Spacer", data: false },
...
export function normalizePage(page = {}, fallbackIndex = 0) {
  ...
  return { id, title: rawTitle, description: page.description || "", fields: Array.isArray(page.fields) ? page.fields.filter(Boolean) : Array.isArray(page.order) ? page.order.filter(Boolean) : [] };
}
...
function normalizePages({ schema = {}, layout = {}, fields = [] }) {
  ...
  let pages = pagesSource.map(normalizePage).map((page) => ({ ...page, fields: page.fields.filter((id) => knownIds.has(id)) }));
  const assigned = new Set(pages.flatMap((page) => page.fields));
  const legacyOrder = Array.isArray(layout.order) ? layout.order.filter((id) => knownIds.has(id)) : fields.map((f) => f.id);
  const unassigned = [...legacyOrder.filter((id) => !assigned.has(id)), ...fields.map((f) => f.id).filter((id) => !assigned.has(id) && !legacyOrder.includes(id))];
  if (!pages.length) pages = [{ id: "page_1", title: "Page 1", description: "", fields: legacyOrder.length ? legacyOrder : fields.map((f) => f.id) }];
  ...
  pages = pages.map((page, index) => {
    ...
    seenPageIds.add(id);
    return { ...page, id, title: page.title || `Page ${index + 1}`, fields: Array.from(new Set(page.fields.filter((fieldId) => knownIds.has(fieldId)))) };
  });
  if (!pages.length) pages = [{ id: "page_1", title: "Page 1", description: "", fields: [] }];
  return pages;
}

export function flattenPageOrder(pages = []) { return pages.flatMap((page) => Array.isArray(page.fields) ? page.fields : []); }
...
export function normalizeFormDefinition(form = {}) {
  ...
  const pageOrder = flattenPageOrder(pages);
