...
function getInstructionFacts(instruction) {
  ...
  const countryIsoByKeyword = [
    [/\b(polish|poland|polska|polski)\b/, "PL"],
    [/\b(german|germany|deutschland)\b/, "DE"],
    [/\b(united states|usa)\b/, "US"],
    [/\b(united kingdom|great britain|british)\b/, "GB"],
    [/\b(canadian|canada)\b/, "CA"],
    [/\b(french|france)\b/, "FR"],
    [/\b(spanish|spain)\b/, "ES"],
    [/\b(italian|italy)\b/, "IT"],
  ];
  const countryIsoAliases = { pl: "PL", de: "DE", us: "US", uk: "GB", gb: "GB", ca: "CA", fr: "FR", es: "ES", it: "IT" };
  const contextualCountryIsoMatch = text.match(/\b(?:country\s+code|country|iso|alpha2|alpha-2)\s*(?:is|=|:|code)?\s*(pl|de|us|uk|gb|ca|fr|es|it)\b/);
  const contextualCountryIso = countryIsoAliases[contextualCountryIsoMatch?.[1]];
  ...
  const pageSizeMatch = text.match(/(?:page\s*size|limit(?:\s+it\s+to)?|show(?:\s+only)?)\s*(\d{1,3})/);
  ...
  return {
    ...
    countryIso: countryIsoByKeyword.find(([pattern]) => pattern.test(text))?.[1] || contextualCountryIso,
