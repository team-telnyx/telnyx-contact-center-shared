...
import { getValidTelnyxMcpOAuthAccessToken } from "@/lib/mcp/mcp-oauth.js";
import {
...
export function isTelnyxMcpUrl(url) {
  try {
    ...
    return parsed.hostname === "api.telnyx.com" && pathname === "/v2/mcp";
  } catch {
    return false;
  }
}
...
async function resolveMcpAuthHeaders(server) {
  ...
  if (authType === "oauth_authorization_code") {
    if (!isTelnyxMcpUrl(server.url)) {
      throw new Error("Telnyx Portal OAuth can only be used with the Telnyx MCP URL https://api.telnyx.com/v2/mcp");
    }
