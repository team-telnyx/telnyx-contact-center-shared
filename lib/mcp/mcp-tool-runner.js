...
export function buildMcpToolArguments({ input, instruction, variables = {}, toolName } = {}) {
  ...
  if (advancedInput.request && typeof advancedInput.request === "object" && !Array.isArray(advancedInput.request)) {
    return {
      ...advancedInput,
      request: {
...
async function resolveMcpAuthHeaders(server) {
  ...
  const localSecret = telnyxApiKey ? null : await getSecretByName(apiKeyRef);
  const apiKey = telnyxApiKey || localSecret?.value || process.env[apiKeyRef];
  if (!apiKey) {
    throw new Error(
