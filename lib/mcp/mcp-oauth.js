...
import { createSecret, getSecretByName, updateSecret } from "@/lib/secrets.js";
...
export function getMcpOAuthRedirectUri(request) {
  ...
  return `${String(base).replace(/\/$/, "")}/api/admin/mcp-servers/oauth/callback`;
}
...
async function upsertSecretByName({ name, description, value, created_by }) {
  const existing = await getSecretByName(name);
  if (existing?.id) {
    return updateSecret(existing.id, { description, value });
  }
  return createSecret({ name, description, value, created_by });
}
