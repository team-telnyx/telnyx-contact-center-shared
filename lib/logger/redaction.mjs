const SECRET_KEY_RE = /(authorization|bearer|api[_-]?key|apikey|token|secret|password|passwd|client[_-]?state|ai[_-]?config)/i;
...
const SAFE_DEBUG_ID_KEYS = new Set([
  "interactionId",
  "callSessionId",
  "callControlId",
  "queueId",
...
function sanitizeString(value) {
  ...
  if (/^bearer\s+[A-Za-z0-9._-]{16,}$/i.test(compact) || /^(?=.*\d)[A-Za-z0-9_-]{24,}\.?[A-Za-z0-9._-]*$/i.test(compact)) {
...
function isSafeDebugIdKey(key) {
  return SAFE_DEBUG_ID_KEYS.has(key);
}

function sanitizeSafeDebugId(value) {
...
export function sanitizeLogPayload(value, seen = new WeakSet(), key = "", depth = 0) {
  ...
  if (isSafeDebugIdKey(key) && typeof value === "string") return sanitizeSafeDebugId(value);
  ...
  if (Array.isArray(value)) {
    return value.slice(0, MAX_ARRAY_ITEMS).map((item) => sanitizeLogPayload(item, seen, "", depth + 1));
  }
