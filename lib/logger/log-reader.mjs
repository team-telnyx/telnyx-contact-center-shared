...
function normalizeTopics(value, fallback) {
  ...
  const topics = raw.flatMap((item) => String(item || "").split(",")).map((item) => canonicalTopicFor(item.trim())).filter(Boolean);
  if (topics.length) return new Set(topics);
  const legacyTopic = canonicalTopicFor(normalizeString(fallback));
  return legacyTopic ? new Set([legacyTopic]) : new Set();
}
...
function matchesFilters(entry, filters) {
  ...
  if (filters.topics?.size && !filters.topics.has(canonicalTopicFor(normalizeString(entry.topic || entry.scope)))) return false;
  if (filters.nodeName) {
    const candidates = [entry.nodeName, entry.node_name, entry.nodeId, entry.node_id, entry.hostname]
      .map((value) => normalizeString(value).toLowerCase())
      .filter(Boolean);
    if (!candidates.includes(filters.nodeName)) return false;
  }
...
export async function queryLogEntries({
  ...
  const filters = {
    ...
    nodeName: normalizeString(nodeName).trim().toLowerCase(),
