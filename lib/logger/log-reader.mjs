...
function normalizeTopics(value, fallback) {
  ...
  const topics = raw.flatMap((item) => String(item || "").split(",")).map((item) => item.trim()).filter(Boolean);
  if (topics.length) return new Set(topics);
  const legacyTopic = normalizeString(fallback);
...
function matchesFilters(entry, filters) {
  ...
  if (filters.topics?.size && !filters.topics.has(normalizeString(entry.topic || entry.scope))) return false;
