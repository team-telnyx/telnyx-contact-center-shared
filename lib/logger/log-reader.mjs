...
async function readLogFile({ logDir, name }) {
  ...
  if (info.size > MAX_FILE_BYTES) throw new Error("Log file too large to read");
...
export async function queryLogEntries({
  ...
  for (const item of files) {
    const content = await readLogFile({ logDir, name: item.name });
  ...
  const truncated = entries.length > effectiveLimit;
  return {
