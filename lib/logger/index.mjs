...
function createFriendlyWriter(stdout) {
  return {
    write(line) {
      const entry = JSON.parse(line);
      const level = String(entry.level || "info").toUpperCase();
      const run = entry.runId || process.pid;
      const message = entry.msg || "";
      const text = `[${formatFriendlyTime(entry.time)}] ${level} (${run}): ${message}`;
...
function createLineWriter({ stdout, resolveConfig, getFileWriter }) {
  ...
  return {
    write(line) {
      ...
      if (consoleEnabled) {
        if (consolePretty && consoleFriendly && !friendlyWriter) friendlyWriter = createFriendlyWriter(stdout);
        if (consolePretty && consoleFriendly && friendlyWriter) friendlyWriter.write(line);
