...
export function managedArchiveBucketName({ env = process.env.APP_ENV || process.env.DEPLOY_GROUP || process.env.NODE_ENV, accountId = accountIdFromEnv() } = {}) {
...
export function normalizeRuntimeLoggingConfig(input = {}, { now = new Date() } = {}) {
  ...
  const config = {
    ...
    consolePretty: normalizeBoolean(readEnvBoolean("LOG_CONSOLE_PRETTY") ?? input.consolePretty ?? input.console_pretty, DEFAULT_LOGGING_CONFIG.consolePretty),
    consoleFriendly: normalizeBoolean(readEnvBoolean("LOG_CONSOLE_FRIENDLY") ?? input.consoleFriendly ?? input.console_friendly, DEFAULT_LOGGING_CONFIG.consoleFriendly),
