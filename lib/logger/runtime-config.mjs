...
function defaultArchiveBucketName({ env = process.env.DEPLOY_GROUP || process.env.APP_ENV || process.env.NODE_ENV, accountId = accountIdFromEnv() } = {}) {
  const configured = normalizeStorageText(process.env.LOG_ARCHIVE_BUCKET || process.env.FDE_RUNTIME_BUCKET || process.env.STORAGE_BUCKET || "");
  ...
  return `${cleanEnv}-${cleanAccount}`;
}

function defaultArchivePrefix({ env = process.env.DEPLOY_GROUP || process.env.APP_ENV || process.env.NODE_ENV } = {}) {
  const configured = normalizeStorageText(process.env.LOG_ARCHIVE_PREFIX || "").replace(/^\/+|\/+$/g, "");
  if (configured) return configured;
  const storagePrefix = normalizeStorageText(process.env.STORAGE_PREFIX || "").replace(/^\/+|\/+$/g, "");
  if (storagePrefix) return `${storagePrefix}/logs`;
  const cleanEnv = safeEnvSegment(env);
  return cleanEnv ? `${cleanEnv}/logs` : DEFAULT_LOGGING_CONFIG.archivePrefix;
}

export function managedArchiveBucketName(options = {}) {
  return defaultArchiveBucketName(options);
}
...
function normalizeArchivePrefixText(value, fallback = "") {
  return normalizeStorageText(value, fallback).replace(/^\/+|\/+$/g, "");
}

function resolveArchivePrefix(input = {}, { managedArchiveBucket = false } = {}) {
  if (managedArchiveBucket) {
    const envPrefix = normalizeArchivePrefixText(process.env.LOG_ARCHIVE_PREFIX || "");
    if (envPrefix) return envPrefix;

    const configuredPrefix = normalizeArchivePrefixText(input.archivePrefix ?? input.archive_prefix ?? "");
    if (configuredPrefix && configuredPrefix !== DEFAULT_LOGGING_CONFIG.archivePrefix) return configuredPrefix;

    return defaultArchivePrefix();
  }

  return normalizeArchivePrefixText(
    input.archivePrefix ?? input.archive_prefix ?? defaultArchivePrefix(),
    DEFAULT_LOGGING_CONFIG.archivePrefix,
  ) || DEFAULT_LOGGING_CONFIG.archivePrefix;
}
...
export function normalizeRuntimeLoggingConfig(input = {}, { now = new Date() } = {}) {
  ...
  const config = {
    ...
    archivePrefix: resolveArchivePrefix(input, { managedArchiveBucket: Boolean(managedArchiveBucket) }),
