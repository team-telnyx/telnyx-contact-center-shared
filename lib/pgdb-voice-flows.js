...
export const VoiceFlowDb = {
   ...
   * If username is null, returns flow by ID only (for public access like HTTP triggers)
   */
  async getFlowById(id, username) {
    ...
    if (username === null || username === undefined) {
      // Public access - no username filter
  ...
  async listFlows(username, filters = {}, pagination = {}) {
    ...
    // Get phone number counts for each flow
    const items = await Promise.all(
      (dataResult.rows || []).map(async (flow) => {
        const phoneNumbers = await this.getFlowPhoneNumbers(flow.id);
        return {
          ...flow,
  ...
  async deleteFlow(id, username) {
    ...
    const r = await pool.query(
      "DELETE FROM voice_flows WHERE id=$1 AND username=$2 RETURNING id, telnyx_voice_app_id",
      [id, username]
    );
    return r.rows?.[0] || null;
  },
