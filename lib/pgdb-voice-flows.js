...
async function withExecutionLock(pool, callControlId, run) {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))", [
      `voice-flow-execution:${callControlId}`,
    ]);
...
export const VoiceFlowDb = {
  ...
  /**
   * Get or create the single execution for this flow and provider call.
   * Serialize concurrent webhook transitions without rewriting historical rows.
   */
  async createFlowExecution(
    ...
    return withExecutionLock(pool, callControlId, async (client) => {
      const existing = await client.query(
        `SELECT * FROM voice_flow_executions WHERE flow_id=$1 AND call_control_id=$2
         ORDER BY started_at DESC, id DESC LIMIT 1`,
        [flowId, callControlId],
      );
  ...
  /** Finalize existing executions only after a verified call.hangup. */
  async completeFlowExecution(flowId, callControlId, completedAt = nowIso()) {
    const pool = getPostgresPool();
    if (!pool) throw new Error("Postgres not configured");
    return withExecutionLock(pool, callControlId, async (client) => {
      const result = await client.query(
        `UPDATE voice_flow_executions SET status='completed', completed_at=COALESCE(completed_at,$3::timestamptz,now())
         WHERE flow_id=$1 AND call_control_id=$2 AND status='active' RETURNING *`,
        [flowId, callControlId, completedAt],
      );
  ...
  async updateFlowExecution(callControlId, updates) {
    ...
    const q = `
      ...
      SET ${setClauses.join(", ")}
      WHERE call_control_id=$1 AND status='active'
      RETURNING *;
    `;
