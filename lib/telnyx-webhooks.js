...
// Public keys that may sign a webhook. The primary Telnyx account key is
// always tried; channels served by a second Telnyx account (the WhatsApp
// backup account) add that account's key through `extraPublicKeys`.
export function telnyxWebhookPublicKeys({ extraPublicKeys = [] } = {}) {
  return [...new Set([
    process.env.TELNYX_WEBHOOK_SECRET || process.env.TELNYX_WEBHOOK_PUBLIC_KEY || "",
    ...extraPublicKeys,
  ].map((value) => String(value || "").trim()).filter(Boolean))];
}

export async function verifyTelnyxSignature(request, rawBody, { extraPublicKeys = [] } = {}) {
  ...
  const publicKeys = telnyxWebhookPublicKeys({ extraPublicKeys });
  ...
  for (const publicKeyEnvRaw of publicKeys) {
    if (await verifyTelnyxSignatureWithKey(publicKeyEnvRaw, signatureHeader, timestampHeader, rawBody)) return true;
  }
  return false;
}

async function verifyTelnyxSignatureWithKey(publicKeyEnvRaw, signatureHeader, timestampHeader, rawBody) {
