...
//   - run agent_assist (assist_type workflows) so agent_assist_config is seeded, and
...
export function validateAiAgentTestFlow(flow, { expectAssistantId = null } = {}) {
  ...
  if (!analysis.agent_assist_active) reasons.push("missing_agent_assist");
...
export async function startAiAgentVoiceTest({
  ...
  if (!validation.ok) {
    return { ok: false, reason: "flow_invalid", details: validation.reasons };
  }
