...
export async function startAiAgentVoiceTest({
  ...
  // Only hard-block on issues that make the test impossible:
  //   - no ai_assistant_start node (nothing to test), or
  //   - the flow points at a DIFFERENT assistant than the workflow's.
  // Missing transcription is a soft warning (surfaced in the UI): the call still
  // runs, the user can hear it via the listener, and they can fix the flow.
  const blockingReasons = (validation.reasons || []).filter(
    (r) => r === "missing_incoming_call" || r === "missing_ai_assistant_start" || r === "assistant_mismatch",
  );
