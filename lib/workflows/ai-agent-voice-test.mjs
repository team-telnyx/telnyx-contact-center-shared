...
export async function startAiAgentVoiceTest({
  ...
  // Only hard-block on issues that make the test physically impossible:
  //   - no Incoming Call initiator (the SIP originate would not route), or
  //   - no ai_assistant_start node (there is no assistant to test).
  // Everything else is a soft warning surfaced in the UI:
  //   - assistant_mismatch: the flow intentionally starts a DIFFERENT assistant
  //     than the one pinned on the workflow. That is a legitimate choice — the
  //     thing under test is whatever assistant the chosen flow starts — so we
  //     must not block it. The simulated-caller context still comes from the
  ...
  const blockingReasons = (validation.reasons || []).filter(
    (r) => r === "missing_incoming_call" || r === "missing_ai_assistant_start",
  );
