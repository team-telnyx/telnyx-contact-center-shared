...
function isStaleInterruptedTranscriptionFinal({ interaction, callControlId, track, isMessageFinal }) {
  if (!isMessageFinal) return false;
  ...
  if (!current || current.track === track) return false;

  const liveKey = getTranscriptionLiveKey(callControlId, track);
  return !activeTranscriptionMessages.has(liveKey);
}
...
export async function routeAgentAssistTranscription(payload) {
  ...
  if (isStaleInterruptedTranscriptionFinal({ interaction, callControlId, track, isMessageFinal })) {
    return { routed: false, reason: "stale_interrupted_final" };
  }
