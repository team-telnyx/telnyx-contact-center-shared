...
function buildTelnyxV2Url(path = "") {
  let base = "https://api.telnyx.com";
  try {
    const configuredBase = process.env.TELNYX_BASE_PATH;
    if (configuredBase) {
  ...
  const baseV2 = `${base.replace(/\/$/, "")}/v2`;
  const suffix = String(path || "");
  if (!suffix) return baseV2;
  return `${baseV2}${suffix.startsWith("/") ? "" : "/"}${suffix}`;
}

async function speakTranslatedResponse(callControlId, text, language) {
  if (!callControlId || !text) return;
  const apiKey = process.env.TELNYX_API_KEY;
  if (!apiKey) {
    console.warn("[AgentAssist] TELNYX_API_KEY missing; cannot auto-send translated response");
    return;
  }

  const url = buildTelnyxV2Url(`/calls/${encodeURIComponent(callControlId)}/actions/speak`);
  const body = {
    payload: text,
  ...
  if (!response.ok) {
    ...
    const errorMsg = data?.errors?.[0]?.detail || data?.message || "speak failed";
    console.warn("[AgentAssist] Auto-send translated response failed:", errorMsg);
  }
}
        ...
        if (translation?.text) {
          ...
          if (assistConfig.auto_send_response === true) {
            ...
            if (targetCallControlId) {
              await speakTranslatedResponse(targetCallControlId, translation.text, targetLanguage);
            }
          }
        }
      }
