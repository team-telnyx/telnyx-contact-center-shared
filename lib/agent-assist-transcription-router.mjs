...
import { updateWorkItemAgentLanguage } from "./acd/artifacts.mjs";
...
import { persistAgentAssistTranscriptions } from "./agent-assist/transcription-persistence.mjs";
import {
  getTranscriptionLiveKey,
  getTranscriptionMessageKey,
  isUtteranceFinal,
  normalizeTranscriptionLeg,
  normalizeTranscriptForDedup,
...
} from "./acd/work-item-repository.mjs";
...
async function withCoreView(loader, value) {
  const pool = getPostgresPool();
  if (!pool) return null;
  return loader(pool, value);
}

async function findInteractionByCallControlId(callControlId) {
  if (!callControlId) return null;
  return withCoreView(findInteractionViewByCallControlId, callControlId);
}

async function findInteractionByCallSessionId(callSessionId) {
  if (!callSessionId) return null;
  return withCoreView(findInteractionViewByCallSessionId, callSessionId);
}

async function findInteractionById(id) {
  if (!id) return null;
  return withCoreView(findInteractionViewByReference, id);
}
...
async function updateInteractionAgentLanguage(interactionId, language) {
  const pool = getPostgresPool();
  if (!pool || !interactionId) return;
  await updateWorkItemAgentLanguage(pool, interactionId, language);
}
...
async function processFinalTranscriptionEnhancements({
  ...
  if (analysisEnabled && transcriptionData.transcript) {
    ...
    workflowLogger.info("auxiliary_analysis_completed", agentAssistRuntimePayload({
      interactionId: interaction.id,
      workflowId: assistConfig.workflow_id,
      provider: "telnyx",
      model: analysisModel,
      reasoningEnabled: false,
      includeIntent: assistConfig.enable_intent_recognition === true,
      includeSentiment: assistConfig.enable_sentiment_analysis === true,
      durationMs: Date.now() - auxiliaryStartedAt,
    }));
  ...
  if (translationEnabled && transcriptionData.transcript) {
    try {
      ...
      if (metadataChanged) {
        await updateInteractionAgentLanguage(interaction.id, metadataBase.agent_language);
      }
...
export async function routeAgentAssistTranscription(payload) {
  ...
  // Shared with the call.transcription webhook path — see isUtteranceFinal.
  const isMessageFinal = isUtteranceFinal(transcriptionData);
  ...
  if (isMessageFinal) {
    ...
    // callControlId) finds the entry and is dropped by this canonical router.
  ...
  if (isMessageFinal && transcriptionData.transcript) {
    ...
    // Keys embed the SHARED normalized text (an earlier fix): the webhook side has
    // always looked these up normalized, but this side seeded them RAW —
    // capitalization or a trailing period meant the cross-source dedup never
    // matched real speech, and any webhook-bypass miss rendered the same
    // utterance twice.
    const itrDedup = normalizeTranscriptForDedup(itrText);
    ...
    recentTranscriptionCache.set(itrKeyROOwn, { ts: nowMs });
    // Seed the tail-suppression anchor for the webhook path (an earlier fix). The
    // webhook's rfinal-itr check exists to suppress a late suffix-only
    // Telnyx final ("Thompson.") after the full utterance already went out
    // ("Sara Thompson.") — but only the webhook itself ever wrote this key,
    // so when THIS path broadcast first the suppression could never fire.
  ...
  if (isMessageFinal) {
    ...
    if (pool) {
      try {
        await persistAgentAssistTranscriptions(pool, {
          ...
          transcriptions: [
            {
              ...
              transcript: routedTranscriptionData.transcript,
              isFinal: true,
              track,
              callControlId,
              confidence: transcriptionData.confidence ?? null,
              source: transcriptionData.source || payload?.source || null,
              ...
              intent: transcriptionData.intent || null,
              sentiment: transcriptionData.sentiment || null,
              sentimentScore: transcriptionData.sentimentScore ?? null,
              tags: transcriptionData.tags || [],
            },
          ],
        });
      } catch (error) {
        // Live delivery must continue even when durable history is temporarily
        // unavailable; the browser sync remains an idempotent fallback.
        workflowLogger.warn(
          "transcription_history_persist_failed",
          agentAssistRuntimePayload({ error, interactionId: interaction.id }),
        );
      }
    }
