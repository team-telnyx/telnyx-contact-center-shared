...
export async function routeAgentAssistTranscription(payload) {
  ...
  // AI Agent voice test: the test flow runs `answer → ai_assistant_start` WITHOUT
  // an agent_assist node, so there is no agent_assist_config and the interaction
  // is not flagged is_contact_center. Correlate via the generator client_state →
  // an active ai_agent_voice_test ledger and drive the simulated-caller reply
  // directly. Only the assistant's spoken (outbound) track should trigger a reply.
  if (isMessageFinal && (track === "outbound" || track === "agent")) {
    try {
      const { resolveAiAgentVoiceTestAssist, handleWorkflowTestingFinalTranscription: handleAiTestReply } =
        await import("./call-generator/workflow-testing.mjs").then(async (wt) => {
          const vt = await import("./workflows/ai-agent-voice-test.mjs");
          return { resolveAiAgentVoiceTestAssist: vt.resolveAiAgentVoiceTestAssist, handleWorkflowTestingFinalTranscription: wt.handleWorkflowTestingFinalTranscription };
        });
      const pool = getPostgresPool();
      ...
      if (aiTest?.assistConfig) {
        handleAiTestReply({
          ...
          transcriptionData: {
            ...transcriptionData,
            transcript: compactTranscriptText(transcriptionData.transcript),
            is_final: true,
            provider_is_final: transcriptionData.is_final,
          },
          assistConfig: aiTest.assistConfig,
        }).catch((error) => {
          workflowLogger.warn("ai_agent_voice_test_reply_failed", agentAssistRuntimePayload({ error }));
        });
        return { routed: true, aiAgentVoiceTest: true };
      }
    } catch (error) {
      workflowLogger.warn("ai_agent_voice_test_route_failed", agentAssistRuntimePayload({ error }));
    }
  }
