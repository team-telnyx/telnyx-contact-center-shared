...
import { validateTranscriptionOverride } from "./assistant-transcription.mjs";
...
export function validateConversationFlow(flow, options = {}) {
  ...
  for (const node of nodes) {
    ...
    for (const issue of validateTranscriptionOverride(node.transcription)) {
      errors.push(`Node ${node.name || node.id}: ${issue}`);
    }
...
export function validateWorkflowNode(node, flow) {
  ...
  issues.push(...validateTranscriptionOverride(node.transcription));
