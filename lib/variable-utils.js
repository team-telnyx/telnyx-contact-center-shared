...
export const DEFAULT_INCOMING_CALL_PAYLOAD_VARIABLE = "call_payload";
export const DEFAULT_HTTP_REQUEST_PAYLOAD_VARIABLE = "request_payload";
...
export function getValueByPath(obj, path) {
  ...
  let current = parseJsonLikeString(obj);
  ...
  return parseJsonLikeString(current);
}

export function resolveVariablePath(variables = {}, rawPath = "") {
  const trimmedPath = String(rawPath || "").trim();
  if (!trimmedPath) return undefined;

  if (trimmedPath.startsWith("global.")) {
    return getValueByPath(variables.global || {}, trimmedPath.substring(7));
  }
  if (trimmedPath.startsWith("payload.")) {
    return getValueByPath(variables.payload || {}, trimmedPath.substring(8));
  }
  if (trimmedPath.startsWith("response.")) {
    return getValueByPath(variables.response || {}, trimmedPath.substring(9));
  }
  if (trimmedPath.startsWith("client_state.")) {
    return getValueByPath(variables.client_state || {}, trimmedPath.substring(13));
  }

  // Backward-compatible alias used by outbound campaign flows and templates.
  // Older flows/documentation often refer to contact data as contact_data, while
  // the engine also exposes the same row_data as contact_record.
  if (trimmedPath === "contact_data" && variables.contact_data === undefined) {
    return variables.contact_record;
  }
  if (
    trimmedPath.startsWith("contact_data.") &&
    variables.contact_data === undefined &&
    variables.contact_record !== undefined
  ) {
    return getValueByPath(variables.contact_record, trimmedPath.substring(13));
  }
...
export function replaceTemplateVariables(value, variables = {}) {
  ...
  return value.replace(/\{\{([^}]+)\}\}/g, (match, varPath) => {
    const resolved = resolveVariablePath(variables, varPath);
    const stringValue = stringifyTemplateValue(resolved);
    return stringValue === undefined ? match : stringValue;
  });
}
 ...
 */
// Path segments that would reach an object's prototype rather than its own
// properties. Paths here come from variable mappings that an operator edits, so
// a path of "__proto__.polluted" would otherwise write onto Object.prototype
...
const DEFAULT_OUTBOUND_CONTACT_FIELD_PATHS = [
  "id",
  "first_name",
  "last_name",
  "name",
...
function addVariableName(names, name) {
  if (name && name.trim()) {
    names.push(name.trim());
  }
}

function addOutboundContactVariableNames(names, payloadVariable) {
  addVariableName(names, payloadVariable);
  addVariableName(names, "contact_data");
  addVariableName(names, "contact_record");

  const aliases = new Set([payloadVariable, "contact_data", "contact_record"]);
  aliases.forEach((alias) => {
    if (!alias) return;
    DEFAULT_OUTBOUND_CONTACT_FIELD_PATHS.forEach((field) => {
      addVariableName(names, `${alias}.${field}`);
    });
  });
}
...
export function getAllVariableNames(flow) {
  ...
  if (flow.nodes) {
    flow.nodes.forEach((node) => {
      ...
      // Incoming Call initiators expose the full Telnyx webhook payload as an alias.
      ...
      // HTTP Request initiators expose the full request payload as an alias.
      ...
      // Outbound Campaign initiator creates full contact record payload variable
      if (nodeType === "outbound_campaign") {
        const payloadVariable = node.data?.config?.payloadVariable || "contact_record";
        addOutboundContactVariableNames(names, payloadVariable);
      }
      ...
      // MCP Tool nodes also create response variables
      if (nodeType === "mcp_tool") {
        const responseVariable = node.data?.config?.responseVariable || "mcp_response";
        if (responseVariable && responseVariable.trim()) {
          names.push(responseVariable.trim());
        }
      }
    ...
    for (const node of flow.nodes) {
      ...
      if (nodeType === "incoming_call") {
        ...
        if (payloadVar && payloadVar.trim() === trimmedName) {
          if (exclude.type === "incoming_call" && exclude.id === node.id) {
            continue;
          }
          return {
            isDuplicate: true,
            source: "incoming_call",
            message: `An Incoming Call node "${
              node.data?.label || node.id
      ...
      if (nodeType === "outbound_campaign") {
        const payloadVar = node.data?.config?.payloadVariable || "contact_record";
        if (payloadVar && payloadVar.trim() === trimmedName) {
          if (exclude.type === "outbound_campaign" && exclude.id === node.id) {
            continue;
          }
          return {
            isDuplicate: true,
            source: "outbound_campaign",
            message: `An Outbound Campaign node "${
              node.data?.label || node.id
            }" already uses this payload variable name`,
          };
        }
      ...
      if (nodeType === "mcp_tool") {
        const responseVar = node.data?.config?.responseVariable || "mcp_response";
        if (responseVar && responseVar.trim() === trimmedName) {
          if (exclude.type === "mcp_tool" && exclude.id === node.id) {
            continue;
          }
          return {
            isDuplicate: true,
            source: "mcp_tool",
            message: `An MCP Tool node "${
              node.data?.label || node.id
            }" already uses this response variable name`,
          };
        }
