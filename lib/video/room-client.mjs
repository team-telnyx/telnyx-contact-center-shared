...
export async function joinVideoRoom({ roomId, token, context = {}, onChange = () => {}, onMessage = () => {}, onDisconnected = () => {}, shouldSubscribe = () => true, logLevel = "WARN", sdk = null }) {
  ...
  const snapshot = (error = null) => {
    ...
    for (const stream of state.streams.values()) {
      ...
      if (!state.subscriptions.get(stream.participantId)?.get(stream.key)) continue;
      if (!live(state, stream) || !wanted(stream.participantId, stream.key)) continue;
    ...
    onChange({ status: state.status, localParticipantId: state.localParticipantId, remote, participants, mixedAudioTrack: state.mixedAudioTrack || null, error });
  };
