...
function upsertParticipant(list, payload, patch) {
  ...
  const next = { participant_id: id, role: context.role || existing?.role || "customer", name: context.name || existing?.name || null, ...existing, ...patch, ...(context.handoffNonce ? {handoff_nonce:context.handoffNonce,endpoint_id:context.endpointId} : {}) };
...
export async function readVideoDetail(db, { workItemId, agentId }) {
  ...
  return {
    ...
    offer: (await db.query("SELECT id,deadline_at FROM acd_offers WHERE work_item_id=$1 AND agent_id=$2 AND state IN ('created','ringing') ORDER BY generation DESC LIMIT 1", [workItemId, agentId])).rows[0] || null,
    wrapupCodes: (await db.query(`SELECT id,name,to_jsonb(c)->>'icon' AS icon,to_jsonb(c)->>'color' AS color FROM cc_wrapup_codes c WHERE is_active=true AND
