...
export async function seedSampleWorkflows() {
  ...
  if (!pool) {
    console.warn("[Seed Workflows] Database pool not available");
  ...
  try {
    for (const workflowData of SAMPLE_WORKFLOWS) {
      ...
      createdCount++;
      console.log(`[Seed Workflows] Created: ${workflowData.name}`);
    }

    if (createdCount > 0 || skippedCount > 0) {
      console.log(`[Seed Workflows] Complete - Created: ${createdCount}, Skipped: ${skippedCount}`);
    }

    return { created: createdCount, skipped: skippedCount };
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("[Seed Workflows] Error:", error.message);
    throw error;
  } finally {
    client.release();
  }
