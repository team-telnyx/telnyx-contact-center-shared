...
export function buildExpressionTestPayloadOptions({ nodes = [], edges = [] } = {}) {
  ...
  nodes.forEach((node) => {
    ...
    if (nodeType === "mcp_tool") {
      ...
      const responsePayload = getStoredResponseBody(config.testResponse);
      if (responsePayload !== undefined) {
        const testResponse = config.testResponse?.response || config.testResponse || {};
        // Codex review (0557da1d54 / 0cbf7ffc, P2): executeMcpToolNode now
        // unwraps the reference integration's {result, error} envelope (mcp-tool-runner.js) before
        // storing EVERY response variable at runtime — the response variable
        // itself, `_text`, AND `_structured` are all derived from the SAME
        // unwrapped payload there. This helper must mirror that exactly, or
        // an expression tested against `_structured` here (e.g.
        // mcp_response_structured.result.contract_id) would succeed in the
        // editor but resolve to undefined on a live call, since live
        // execution's `_structured` is also unwrapped.
        const { payload: unwrappedResponsePayload } = unwrapMcpResultEnvelope(getMcpResponseVariablePayload(testResponse));
        const responseText = typeof unwrappedResponsePayload === "string" ? unwrappedResponsePayload : testResponse.text || "";
        const structuredPayload = unwrappedResponsePayload && typeof unwrappedResponsePayload === "object"
          ? unwrappedResponsePayload
          : testResponse.structuredContent || {};
        addOption({
          id: `${node.id}:mcp-tool-response`,
          group: "MCP Tool responses",
          label: `${nodeLabel} → ${responseVariable}`,
          description: "Saved response from Test MCP Tool",
          payload: {
            [responseVariable]: unwrappedResponsePayload,
            [`${responseVariable}_text`]: responseText,
            [`${responseVariable}_structured`]: structuredPayload,
          },
        });
      }
    }
...
function getNodeWebhookEvents(node, edges) {
  ...
  const outputEvents = node?.data?.dynamicOutputEvents || nodeDef?.outputEvents;
  if (!outputEvents) {
    return [...events];
  }
  ...
  edges
    ...
    .forEach((edge) => {
      ...
      const eventType = outputEvents[outputIndex];
