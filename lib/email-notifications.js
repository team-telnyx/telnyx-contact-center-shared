...
import { normalizeEmailFromValue } from "./email-config.mjs";
...
function logEmailDeliveryFailure(error, { status, reason } = {}) {
  const providerError =
    error instanceof Error
      ? error
      : new Error(String(error || "Failed to send email"));
...
export async function sendEmailNotification(
  ...
  try {
    ...
    const emailFrom = normalizeEmailFromValue(process.env.EMAIL_FROM);
    ...
    if (!emailApiKey || !emailDomain || !emailFrom) {
      ...
      logEmailDeliveryFailure(
        new Error(`Missing email configuration: ${missingFields.join(", ")}`),
    ...
    if (
      ...
      return new Promise((resolve) => {
        formData.submit(
          ...
          (err, res) => {
            if (err) {
              logEmailDeliveryFailure(err, { reason: "network_error" });
            ...
            res.on("end", () => {
              try {
                ...
                if (res.statusCode >= 200 && res.statusCode < 300) {
                  ...
                  logEmailDeliveryFailure(new Error(errorMessage), {
                ...
                logEmailDeliveryFailure(parseError, {
                  status: res.statusCode,
    ...
    logEmailDeliveryFailure(error, { reason: "unexpected_error" });
    return {
      success: false,
