...
export async function sendEmailNotification(
  ...
  try {
    ...
    if (!emailApiKey || !emailDomain || !emailFrom) {
      console.error("Email notification failed: Missing Mailgun configuration");
...
export async function generateRegistrationEmailHTML(visitorData, baseUrl) {
  ...
  console.log("[Email Generation] QR Code Data URL check:", {
    unique_id,
    has_qr_code: !!qrCodeDataUrl,
    qr_code_length: qrCodeDataUrl?.length,
    qr_code_preview: qrCodeDataUrl?.substring(0, 50),
    qr_code_type: qrCodeDataUrl?.split(",")[0],
  });

  // Verify the data URL is valid
  if (qrCodeDataUrl && !qrCodeDataUrl.startsWith("data:image/")) {
    console.error(
      "[Email Generation] Invalid QR code data URL format:",
      qrCodeDataUrl?.substring(0, 100)
    );
  }
  ...
  try {
    ...
    if (qr_code_data_url && qr_code_data_url.startsWith("data:image/")) {
      ...
      if (matches) {
        ...
        console.log("[Email] Adding QR code as inline attachment:", {
          size: buffer.length,
          contentType,
        });
      }
    ...
    console.error("Visitor registration email error:", error);
    return {
      success: false,
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Password Reset - Telnyx Contact Center</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="card">
            ...
            <p class="message">
                Hi ${userName},<br><br>
                We received a request to reset your password for your Telnyx Contact Center account.
 ...
 * @returns {string} HTML email content
 */
export function generateActivationEmailHTML(userName, activationUrl) {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
        <div class="card">
            ...
            <h2 class="title">Welcome to Telnyx Contact Center!</h2>
            <p class="message">
                Hi ${userName},<br><br>
                Thank you for signing up! To complete your registration and start using
                the Telnyx Contact Center, please verify your email address by clicking the button below:
            </p>
            ...
            <div class="footer">
                If you didn't create an account with Telnyx Contact Center, you can safely ignore this email.<br><br>
...
export async function sendActivationEmail(email, userName, activationToken) {
  try {
    ...
    const html = generateActivationEmailHTML(userName, activationUrl);
...
export async function sendVisitorActivationEmail(
  ...
  try {
    ...
    console.error("Visitor activation email error:", error);
    return {
      success: false,
 ...
 */
function generateUserInviteEmailHTML(user, inviteUrl) {
...
<head>
  ...
  <style>
    ...
    .header { background-color: #00BC8C; padding: 36px 40px; text-align: center; }
    ...
    .cta-button { display: inline-block; background-color: #00BC8C; color: #ffffff; text-decoration: none; font-size: 16px; font-weight: 600; padding: 14px 36px; border-radius: 8px; }
  ...
  <div class="wrapper">
    ...
    <div class="body">
      ...
      <p class="text" style="font-size:13px;color:#888">
        ...
        <a href="${inviteUrl}" style="color:#00BC8C;word-break:break-all;">${inviteUrl}</a>
      </p>
    </div>
    <div class="footer">
      <p class="footer-text">
