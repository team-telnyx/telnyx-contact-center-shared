...
import { providerApiLogger, telnyxErrorPayload, telnyxRequestPayload, telnyxResourcePayload } from "@/lib/telnyx-ai-logging.mjs";
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status >= 500 && attempt < retries) {
          providerApiLogger.warn("insights_request_retry", { ...telnyxRequestPayload({ path, method: options.method, attempt: attempt + 1, retries, status: response.status }) });
          lastError = error;
      ...
      if (attempt < retries && !fetchError.status) {
        providerApiLogger.warn("insights_request_retry", { ...telnyxRequestPayload({ path, method: options.method, attempt: attempt + 1, retries, reason: "network_error" }), ...telnyxErrorPayload(fetchError) });
...
export async function createInsightGroup(workflowName, webhookUrl) {
  providerApiLogger.info("insight_group_create_started", { ...telnyxResourcePayload({ workflowName }) });
  ...
  providerApiLogger.info("insight_group_created", { ...telnyxResourcePayload({ groupId: data?.data?.id }) });
...
export async function updateInsightGroup(groupId, updates) {
  providerApiLogger.info("insight_group_update_started", { ...telnyxResourcePayload({ groupId }) });
...
export async function deleteInsightGroup(groupId) {
  providerApiLogger.info("insight_group_delete_started", { ...telnyxResourcePayload({ groupId }) });
  ...
  try {
    ...
    if (err.status === 404) {
      providerApiLogger.info("insight_group_delete_already_absent", { ...telnyxResourcePayload({ groupId }) });
      return true;
    }
    throw err;
  }
...
export async function createInsight({ name, instructions, json_schema }) {
  providerApiLogger.info("insight_create_started", { insightName: name });
  ...
  providerApiLogger.info("insight_created", { ...telnyxResourcePayload({ insightId: data?.data?.id }) });
  return data?.data || data;
}
...
export async function updateInsight(insightId, updates) {
  providerApiLogger.info("insight_update_started", { ...telnyxResourcePayload({ insightId }) });
...
export async function deleteInsight(insightId) {
  providerApiLogger.info("insight_delete_started", { ...telnyxResourcePayload({ insightId }) });
  ...
  try {
    ...
    if (err.status === 404) {
      providerApiLogger.info("insight_delete_already_absent", { ...telnyxResourcePayload({ insightId }) });
      return true;
    }
    throw err;
  }
...
export async function assignInsightToGroup(insightId, groupId) {
  providerApiLogger.info("insight_group_assignment_started", { ...telnyxResourcePayload({ insightId, groupId }) });
...
export async function unassignInsightFromGroup(insightId, groupId) {
  providerApiLogger.info("insight_group_unassignment_started", { ...telnyxResourcePayload({ insightId, groupId }) });
...
async function ensureInsightInGroup(insightId, groupId) {
  try {
    ...
    if (!err.response?.includes("already assigned")) {
      providerApiLogger.warn("insight_group_assignment_warning", { ...telnyxResourcePayload({ insightId, groupId }), ...telnyxErrorPayload(err) });
    }
  }
}
...
export async function syncWorkflowInsights(workflow, webhookUrl) {
  ...
  providerApiLogger.info("workflow_insights_sync_started", { ...telnyxResourcePayload({ workflowId: workflow.id, workflowName: workflow.name }) });
  ...
  if (!groupId) {
    ...
    if (!existing) {
      providerApiLogger.info("insight_group_missing_recreate_started", { ...telnyxResourcePayload({ groupId, workflowId: workflow.id }) });
      const group = await createInsightGroup(workflow.name, webhookUrl);
      groupId = group.id;
  ...
  providerApiLogger.info("workflow_insights_sync_completed", { ...telnyxResourcePayload({ workflowId: workflow.id, workflowName: workflow.name, groupId: result.groupId }) });
  return result;
}
...
export async function getConversationInsights(conversationId) {
  providerApiLogger.info("conversation_insights_fetch_started", { conversationId: conversationId ? String(conversationId) : undefined });
...
export async function deleteWorkflowInsights(workflow) {
  providerApiLogger.info("workflow_insights_cleanup_started", { ...telnyxResourcePayload({ workflowId: workflow.id, workflowName: workflow.name }) });
  ...
  if (errors.length > 0) {
    providerApiLogger.warn("workflow_insights_cleanup_partial_failure", { ...telnyxResourcePayload({ workflowId: workflow.id, workflowName: workflow.name }), errorCount: errors.length });
  }

  providerApiLogger.info("workflow_insights_cleanup_completed", { ...telnyxResourcePayload({ workflowId: workflow.id, workflowName: workflow.name }) });
}
