...
import { credentialsLogger, securityErrorPayload } from "./security-logging.mjs";
...
if (SECRET_KEY) {
  try {
    ...
    const configuredKeyLength = SECRET_KEY.length;
    const keyBuffer = Buffer.from(SECRET_KEY, "hex");
    if (keyBuffer.length < 32) {
      credentialsLogger.error("secrets_encryption_key_too_short", {
        expectedCharacters: 64,
        expectedBytes: 32,
        actualCharacters: configuredKeyLength,
        actualBytes: keyBuffer.length,
      });
    } else {
      ENCRYPTION_KEY_BUFFER = keyBuffer.slice(0, 32);
    }
  } catch (err) {
    credentialsLogger.error("secrets_encryption_key_invalid", { ...securityErrorPayload(err) });
  }
}

// If no secret key, secrets functionality is disabled
const encryptionKeyConfigured = Boolean(SECRET_KEY);
const encryptionKeyValid = Boolean(ENCRYPTION_KEY_BUFFER);
if (!SECRET_KEY || !ENCRYPTION_KEY_BUFFER) {
  credentialsLogger.warn("secrets_encryption_disabled", { configured: encryptionKeyConfigured, valid: encryptionKeyValid });
}
...
function getEncryptionKey() {
  if (!ENCRYPTION_KEY_BUFFER) {
    throw new Error(
      "SECRETS_ENCRYPTION_KEY not configured or invalid. It must be a hex-encoded string of at least 64 characters (32 bytes)."
    );
  }
...
export async function getSecretByName(name) {
  ...
  try {
    ...
    credentialsLogger.error("secret_get_failed", { secretName: name, ...securityErrorPayload(error) });
    return null;
  }
}
  ...
  while ((match = secretPattern.exec(text)) !== null) {
    ...
    try {
      ...
      if (secret) {
        ...
        credentialsLogger.warn("secret_not_found", { secretName });
        // Keep the original reference if secret not found
      }
    } catch (error) {
      credentialsLogger.error("secret_reference_resolve_failed", { secretName, ...securityErrorPayload(error) });
