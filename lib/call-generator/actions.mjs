...
export function normalizeSteps(raw) {
  ...
  return list
    .map((step) => {
      ...
      if (type === "record") return { type };
      if (type === "play_audio") {
        try {
          const url = new URL(step.audio_url);
          if (url.protocol !== "https:" || url.username || url.password) return null;
          return { type, audio_url: url.href, loop: step.loop === "infinity" ? "infinity" : 1 };
        } catch { return null; }
      }
      if (type === "play_media") {
        const mediaName = String(step.media_name || "").trim();
        return mediaName ? { type, media_name: mediaName, ...(step.loop === 'infinity' ? { loop: 'infinity' } : {}) } : null;
      }
      if (type === "speak") {
...
export function describeStep(step) {
  if (!step) return "";
  if (step.type === "record") return "Record caller receive track";
  if (step.type === "play_audio") return "Play test audio URL";
