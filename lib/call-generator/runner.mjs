...
import { WORKFLOW_TESTING_ACTION_ID, workflowTestingVoiceFromAction } from "./workflow-testing.mjs";
...
export function effectiveRunLimits(settings = {}, runConfig = {}, scenarioConfig = {}) {
  ...
  return {
    ...
    maxDurationSecs: clamp(runConfig.maxDurationSecs, 10, 3600, scenarioMaxDuration),
  };
}
...
export function normalizeTargets(config = {}, fallbackFromNumbers = []) {
  ...
  const targets = rawTargets
    ...
    .filter((t) => t.flow_id && t.from_numbers.length > 0);
  ...
  if (config.target_type === "call_flow" && config.target) {
    ...
    return [{
      ...
      total_calls: Math.max(1, Math.min(1000, Number(config.total_calls) || 1)),
...
async function seedLedger(pool, run, scenario, fallbackFromNumbers = []) {
  ...
  for (let targetIndex = 0; targetIndex < targets.length; targetIndex += 1) {
    ...
    for (let i = 0; i < target.total_calls; i += 1) {
      const fromNumber = target.from_numbers[i % target.from_numbers.length]; // per-target CLI rotation
      await pool.query(
        `INSERT INTO cg_call_ledger (run_id, status, from_number, result) VALUES ($1, 'pending', $2, $3)`,
        [run.id, fromNumber, JSON.stringify({ target_index: targetIndex, flow_id: target.flow_id, action_steps: target.workflow_testing ? [] : steps, action_trigger: target.action_trigger, ...(workflowTesting ? { workflow_testing: workflowTesting } : {}) })],
      );
      seeded += 1;
    }
  }
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (!(await isCallGeneratorEnabled(pool))) {
        const { rows: activeRows } = await pool.query(
          `SELECT id, call_control_id
           FROM cg_call_ledger
           WHERE run_id = $1 AND status IN ('dialing','ringing','answered','talking') AND call_control_id IS NOT NULL`,
          [runId],
        );

        for (const row of activeRows) {
          const hungUp = await hangupGeneratedCall(row.call_control_id);
          if (hungUp) {
            await markLedgerStatus(pool, row.id, "failed", { reason: "call_generator_disabled" });
          }
        }
        ...
        await pool.query(
          `UPDATE cg_call_ledger
           SET status = 'failed',
               ...
               result = COALESCE(result, '{}'::jsonb) || '{"reason":"call_generator_disabled"}'::jsonb
           WHERE run_id = $1
             AND status IN ('pending','dialing','ringing','answered','talking')
      ...
      for (let i = 0; i < budget; i += 1) {
        ...
        if (!ledgerRow) break;
        const rowResult = ledgerRow.result && typeof ledgerRow.result === "object" ? ledgerRow.result : {};
        const flowId = rowResult.flow_id || targets[0]?.flow_id || null;
        const fromNumber = ledgerRow.from_number || pickFromNumber(targets[0]?.from_numbers || [], dialedCount);
        dialedCount += 1;
        const result = await originateGeneratedCall(pool, {
          ...
          ledgerRow,
          task: { target_type: "call_flow", target: flowId },
          fromNumber,
        });
