...
export function normalizeTargets(config = {}, fallbackFromNumbers = []) {
  ...
  const targets = rawTargets
    .map((t) => ({
      ...
      action_id: String(t?.action_id || "").trim() || null,
  ...
  for (let targetIndex = 0; targetIndex < targets.length; targetIndex += 1) {
    ...
    for (let i = 0; i < target.total_calls; i += 1) {
      ...
      await pool.query(
        ...
        [run.id, fromNumber, JSON.stringify({ target_index: targetIndex, flow_id: target.flow_id, action_steps: steps, action_trigger: target.action_trigger })],
      );
      seeded += 1;
    }
  }
