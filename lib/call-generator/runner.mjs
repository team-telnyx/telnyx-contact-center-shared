...
export function normalizeTargets(config = {}) {
  ...
  if (config.target_type === "call_flow" && config.target) {
    return [{
      ...
      from_numbers: Array.isArray(config.from_numbers) ? config.from_numbers.filter(Boolean) : [],
      action_id: null,
    }].filter((t) => t.flow_id);
  }
  return [];
...
async function seedLedger(pool, run, scenario) {
  const targets = normalizeTargets(scenario?.config || {});
...
export async function startRunLoop(pool, runId) {
  ...
  const targets = normalizeTargets(scenario.config || {});
  if (!targets.length) {
    await pool.query(`UPDATE cg_runs SET status = 'failed', stopped_at = NOW(), stats = stats || '{"error":"no_valid_targets"}' WHERE id = $1`, [runId]);
  ...
  if (invalidNumbers.length) {
    await pool.query(`UPDATE cg_runs SET status = 'failed', stopped_at = NOW(), stats = stats || '{"error":"from_numbers_not_enabled"}' WHERE id = $1`, [runId]);
