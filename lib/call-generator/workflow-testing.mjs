...
function nodeType(node) {
  return String(node?.type || node?.data?.type || node?.data?.nodeType || node?.data?.node_type || "").toLowerCase();
}
...
export async function ensureWorkflowTestingAction(pool) {
  if (!pool) return null;
  const step = { type: "workflow_testing", voice: DEFAULT_WORKFLOW_TESTING_VOICE };
  const { rows } = await pool.query(
    ...
    [
      ...
      "Protected call-generator action used by Test Workflow targets. Only the TTS voice is editable.",
      JSON.stringify([step]),
    ],
  );
  return rows[0] || null;
...
async function loadWorkflowContext(pool, workflowId) {
  ...
  const { rows } = await pool.query(
    `SELECT id, name, description, items, stages, llm_model FROM aa_workflows WHERE id = $1 LIMIT 1`,
    [workflowId],
  );
  ...
  return {
    ...workflow,
    items: safeJson(workflow.items, []),
    stages: safeJson(workflow.stages, []),
  };
}
...
export async function handleWorkflowTestingFinalTranscription({ pool, interaction, payload, transcriptionData, assistConfig }) {
  ...
  const callSessionId = payload?.call_session_id || interaction.call_session_id || null;
  if (!callSessionId) return { ok: false, reason: "missing_call_session" };
  ...
  if (testing.workflow_id && String(testing.workflow_id) !== String(assistConfig.workflow_id)) {
