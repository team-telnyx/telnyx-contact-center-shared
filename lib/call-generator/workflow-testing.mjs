...
// Telnyx Ultra voices (Telnyx.Ultra.*) support SSML emotion tags in Call
// Control speak commands: `<emotion value="excited" />Hello.` plus the
// nonverbal `[laughter]`. Non-Ultra voices would speak those tags literally,
// so expression is ONLY applied when the voice is an Ultra voice. See
...
// Map each caller persona to a sensible default Ultra emotion so the simulated
// caller actually SOUNDS like the persona (angry caller sounds angry, etc.).
// Telnyx's primary/high-quality emotions are angry, excited, content, sad,
// scared; the rest (frustrated, confident, hesitant, apologetic, ...) are also
// supported. neutral → no tag (most natural delivery per Telnyx guidance).
const PERSONA_EMOTION = {
  neutral: null,
  angry: "angry",
  in_a_hurry: "frustrated",
  confused: "hesitant",
  chatty: "excited",
  elderly: "content",
  suspicious: "hesitant",
...
export function personaEmotion(personaId) {
  return PERSONA_EMOTION[normalizePersona(personaId)] || null;
}

// Prepend an Ultra emotion tag to the spoken text for the given persona, but
// only for Ultra voices. If the text already starts with an <emotion ... /> tag
// (e.g. the LLM produced one), leave it untouched. Returns the text unchanged
// for non-Ultra voices or the neutral persona.
export function applyUltraExpression(text, { voice, persona } = {}) {
  const raw = String(text || "");
  if (!isUltraVoice(voice)) return raw;
  if (/^\s*<emotion\b/i.test(raw)) return raw;
  const emotion = personaEmotion(persona);
...
function buildCustomerReplyPrompt({ workflow, transcript, history, persona, slotCount, expressive }) {
  ...
  // Ultra voices render SSML emotion tags and nonverbal cues. Invite the model
  // to use them sparingly for more realistic delivery; only enabled for Ultra
  // voices so non-Ultra TTS never speaks the tags literally.
  if (expressive) {
    systemParts.push('You may add SSML emotion tags such as <emotion value="angry" />, <emotion value="excited" />, <emotion value="content" />, <emotion value="frustrated" /> or <emotion value="hesitant" /> before a sentence, and the nonverbal cue [laughter], to make the delivery match the persona. Use them sparingly and only when they fit. Output them inline as part of the utterance text.');
  }
...
async function generateWorkflowTestingReply({ workflow, transcript, history, apiKey, persona, maxSlotsPerTurn, randomizeSlots, voice }) {
  const model = workflow?.llm_model || "openai/gpt-4o";
  const slotCount = resolveSlotCountForTurn({ maxSlotsPerTurn, randomizeSlots });
  const expressive = isUltraVoice(voice);
  const response = await fetch(buildTelnyxV2Url("/ai/chat/completions"), {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify({ model, temperature: 0.7, max_tokens: 160, messages: buildCustomerReplyPrompt({ workflow, transcript, history, persona, slotCount, expressive }) }),
  });
...
async function handleWorkflowTestingLedgerReply({ pool, ledger, transcriptionData, assistConfig }) {
  ...
  try {
    ...
    // For Ultra voices, prepend the persona's emotion tag so the caller sounds
    // the part (angry/excited/...). Non-Ultra voices and the neutral persona
    // speak the plain reply. The expressive text is only used for TTS; history
    // and the returned reply stay clean (no tags) so the LLM context and logs
    // are not polluted with SSML.
    const spokenText = applyUltraExpression(reply, { voice: testing.voice, persona: testing.persona });
