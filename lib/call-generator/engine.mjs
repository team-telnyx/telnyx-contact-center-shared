...
async function shouldRunActionTrigger(pool, ledgerId, trigger) {
  const normalizedTrigger = trigger === "agent_bridge" ? "agent_bridge" : "call_answer";
  const { rows } = await pool.query(
    `SELECT result FROM cg_call_ledger WHERE id = $1`,
    [ledgerId],
  );
  const result = rows[0]?.result || {};
  const configuredTrigger = result.action_trigger === "agent_bridge" ? "agent_bridge" : "call_answer";
  if (configuredTrigger !== normalizedTrigger) return false;

  const { rowCount } = await pool.query(
    `UPDATE cg_call_ledger
     SET result = COALESCE(result, '{}'::jsonb) || $1::jsonb
     WHERE id = $2
       AND NOT (COALESCE(result, '{}'::jsonb) ? 'action_sequence_started_at')`,
    [JSON.stringify({ action_trigger_fired: normalizedTrigger, action_sequence_started_at: new Date().toISOString() }), ledgerId],
  );
  return rowCount > 0;
}

async function getLedgerStatusAndActionTrigger(pool, ledgerId) {
  const { rows } = await pool.query("SELECT status, result FROM cg_call_ledger WHERE id = $1", [ledgerId]);
  const row = rows[0] || {};
  const result = row.result || {};
  return {
    status: row.status || null,
    actionTrigger: result.action_trigger === "agent_bridge" ? "agent_bridge" : "call_answer",
  };
}
...
export async function handleGeneratorWebhookEvent(pool, eventType, payload) {
  ...
  switch (String(eventType || "")) {
    ...
    case "call.answered": {
      ...
      if (ok) {
        const executeSequence = await shouldRunActionTrigger(pool, ledgerId, "call_answer");
        await runPostAnswerActions(pool, ledgerId, payload, { executeSequence });
      }
      return ok ? "answered" : null;
    }
    case "call.bridged": {
      const { status, actionTrigger } = await getLedgerStatusAndActionTrigger(pool, ledgerId);
      if (!["answered", "talking"].includes(status) && actionTrigger !== "agent_bridge") {
        return null;
      }
      const ok = await markLedgerStatus(pool, ledgerId, "talking", { event: eventType });
      if (ok && await shouldRunActionTrigger(pool, ledgerId, "agent_bridge")) {
        await runPostAnswerActions(pool, ledgerId, payload);
      }
      return ok ? "talking" : null;
    }
...
export async function runPostAnswerActions(pool, ledgerId, payload, { executeSequence = true } = {}) {
  const apiKey = getApiKey();
  ...
  try {
    const { rows } = await pool.query(
      `SELECT r.config AS run_config, l.result, s.config AS scenario_config
         FROM cg_call_ledger l
         JOIN cg_runs r ON r.id = l.run_id
    ...
    scenarioConfig = rows[0]?.scenario_config || {};
  } catch {
    config = {};
  }
  ...
  const hasActionSteps = Object.prototype.hasOwnProperty.call(ledgerResult || {}, "action_steps");
  const steps = Array.isArray(ledgerResult?.action_steps) ? ledgerResult.action_steps : [];

  try {
    if (!executeSequence) {
      // Action sequence is configured to wait for a later trigger (for example
      // the caller being bridged to an agent). Still arm the max-duration guard
      // below so answered generated calls cannot linger indefinitely.
      ...
      for (const step of steps) {
        if (step.type === "play_media") {
          ...
          // Pause between surrounding steps. Telnyx queues speak/playback
          // server-side per call, so sleeping here before issuing the next
          // command inserts a real gap in the played sequence.
          const ms = Math.min(30000, Math.max(0, Math.round(Number(step.delay_ms)) || 0));
  ...
  // Resolve the max call duration guard. Priority: explicit run config
  // (config.postAnswer.maxDurationSecs) → per-scenario override
  // (cg_scenarios.config.maxCallDurationSecs) → the global Settings value
  // (cg_settings.max_call_duration_secs) → 120s default.
  //
  // cg_runs.config is persisted empty ({}) — the runner only mutates its
  // in-memory run object — so config.postAnswer.maxDurationSecs is undefined
  // here. The per-scenario override therefore MUST be read from the joined
  // scenario config; otherwise this app-side hangup guard falls back to the
  // global Settings value and cuts the call short (e.g. firing at 120s on a
  // scenario configured for 360s, even though Telnyx received the correct
  // time_limit_secs:360 on the Dial). This guard is a backstop to Telnyx's own
  // time_limit_secs, so it must use the same effective value.
  let maxDurationSecs = Number(config?.postAnswer?.maxDurationSecs);
  if (!(maxDurationSecs > 0)) {
    const scenarioValue = Number(scenarioConfig?.maxCallDurationSecs);
    if (scenarioValue > 0) maxDurationSecs = scenarioValue;
  }
  if (!(maxDurationSecs > 0)) {
    try {
      const { rows } = await pool.query(`SELECT settings FROM cg_settings WHERE id = 'default' LIMIT 1`);
      const settingsValue = Number(rows[0]?.settings?.max_call_duration_secs);
      if (settingsValue > 0) maxDurationSecs = settingsValue;
    } catch {}
  }
  if (!(maxDurationSecs > 0)) maxDurationSecs = 120;
  // Clamp to the same bounds the runner enforces (10s–1h).
  maxDurationSecs = Math.min(3600, Math.max(10, Math.round(maxDurationSecs)));
