...
//
// Flag-gated: CALL_GENERATOR=true must be set for any call to be originated.
...
export function isCallGeneratorEnabled() {
  return String(process.env.CALL_GENERATOR || "").toLowerCase() === "true";
}
...
export async function markLedgerStatus(pool, ledgerId, status, resultPatch = {}) {
  ...
  await pool.query(
    `UPDATE cg_call_ledger
     ...
     WHERE id = $3`,
    [status, JSON.stringify(resultPatch || {}), ledgerId],
  );
  return true;
}
...
// Post-answer actions per run config: tts_loop (speak), audio_loop (playback)
// or silence with timed hangup. All actions are fire-and-forget with errors
// recorded on the ledger so a stuck call can still be reaped by the watchdog.
export async function runPostAnswerActions(pool, ledgerId, payload) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT r.config FROM cg_call_ledger l JOIN cg_runs r ON r.id = l.run_id WHERE l.id = $1`,
      [ledgerId],
    );
  ...
  try {
    if (action === "audio_loop" && config?.postAnswer?.audioUrl) {
      await fetch(buildTelnyxV2Url(`/calls/${encodeURIComponent(callControlId)}/actions/playback_start`), {
        method: "POST",
        headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ audio_url: config.postAnswer.audioUrl, loop: "infinity", client_state: clientState }),
      });
    } else if (action !== "silence") {
      const text = String(config?.postAnswer?.ttsText || "This is an automated test call from the contact center call generator.");
      await fetch(buildTelnyxV2Url(`/calls/${encodeURIComponent(callControlId)}/actions/speak`), {
        method: "POST",
        headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ payload: text, voice: "female", language: "en-US", client_state: clientState }),
      });
    }
  } catch (err) {
    await pool.query(
