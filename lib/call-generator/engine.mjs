...
export async function runPostAnswerActions(pool, ledgerId, payload, { executeSequence = true } = {}) {
  ...
  // (config.postAnswer.maxDurationSecs) → the global Settings value
  // (cg_settings.max_call_duration_secs) → 120s default. Historically this
  // fell back to a hardcoded 60s because cg_runs.config is persisted empty
  // ({}) and the runner only mutates its in-memory run object — so the
  // configured Settings value (e.g. 120s) never reached this timer and every
  // generated call was capped at ~60s. Read the Settings value here so the
  // guard honours what the admin configured.
