 ...
 * stores results as typed Core transcript artifacts, so history and Quality
 * Management consume the same durable record.
 */
...
export async function transcribeInteractionRecording({
  ...
  if (!interaction.work_item_id) {
    throw new Error("Core work item is required for transcription");
  }
  ...
  await persistGeneratedTranscript(pool, {
    ...
    model,
    sourceEventId: `quality:${recordingId}:${model}`,
