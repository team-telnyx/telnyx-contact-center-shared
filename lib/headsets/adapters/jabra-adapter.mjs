  ...
  return {
    ...
    async setSoftphoneState(state) {
      ...
      if (next.ringing && !previousState.ringing) {
        const incomingCallResult = easyCallControl.signalIncomingCall?.(incomingRingTimeoutMs);
        if (incomingCallResult && typeof incomingCallResult.then === "function") {
          incomingCallResult.then(emitIncomingCallResult).catch(() => {});
        } else {
          emitIncomingCallResult(incomingCallResult);
        }
      }
