...
function hasDeviceIdentity(payload = {}) {
  return Boolean(payload.HeadsetPath || payload.SerialNumber || payload.ProductName || payload.HeadsetType || payload.DeviceName || payload.HeadsetName || payload.Model);
}
...
export function createEposAdapter({
  ...
  const handleMessage = (event) => {
    ...
    if (/HeadsetDisconnected|DeviceDisconnected/i.test(eventName) && isNotification(payload)) {
      ...
      if (currentDevice && payload.HeadsetPath && currentDevice.id !== String(payload.HeadsetPath)) {
        return;
      }
