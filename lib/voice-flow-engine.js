...
function transformTranscriptionOptions(body) {
  ...
  if (body.transcription_engine) {
    ...
    if (engine === "Google" || engine === "Azure") {
      ...
      ["Deepgram", "Telnyx", "AssemblyAI", "Speechmatics", "Soniox", "xAI"].includes(
        engine,
      )
    ) {
      // For model-based providers, build transcription_engine_config from top-level fields
      const engineConfig = {
        transcription_engine: engine,
      };

      // transcription_model is required for most model-based providers
      if (body.transcription_model) {
        engineConfig.transcription_model = body.transcription_model;
      } else {
        // Use defaults if missing
        if (engine === "Deepgram") {
          engineConfig.transcription_model = "deepgram/flux";
        } else if (engine === "Telnyx") {
          engineConfig.transcription_model = "openai/whisper-tiny";
        } else if (engine === "Speechmatics") {
          engineConfig.transcription_model = "speechmatics/standard";
        } else if (engine === "Soniox") {
          engineConfig.transcription_model = "soniox/stt-rt-v4";
        }
      }
