...
export async function executeFlowNode(
  ...
  try {
    ...
    if (nodeType === "agent_assist") {
      ...
      if (clientStateUpdateResult.success) {
        console.log(
          `[FlowEngine] Agent Assist configured: type=${agentAssistConfig.assist_type}, workflow=${agentAssistConfig.workflow_id || "none"}`,
        );
        return {
          success: true,
          output: 0,
    ...
    switch (action) {
      ...
      case "streaming_start":
        ...
        if (
          ...
          if (providerConfig) {
            // Build the streaming WebSocket server URL (runs on STREAMING_WS_PORT, default main+1=3001)
            const wsPort = parseInt(process.env.STREAMING_WS_PORT || "3001", 10);
            ...
            if (aiProvider === "azure-transcription") {
              ...
              // Use STREAMING_WS_URL if explicitly set, otherwise build from APP_BASE_URL/localhost
              let wsBase = process.env.STREAMING_WS_URL;
              if (!wsBase) {
                let baseUrl = process.env.APP_BASE_URL || process.env.NEXT_PUBLIC_BASE_URL || "localhost:3000";
                baseUrl = baseUrl.replace(/^https?:\/\//, "");
                const hostname = baseUrl.split(":")[0];
                const protocol = hostname === "localhost" || hostname.startsWith("127.") ? "ws" : "wss";
                wsBase = `${protocol}://${hostname}:${wsPort}`;
              }

              body.stream_url = `${wsBase}/streaming/azure${secretParam}`;
              ...
              existingClientState.azure_transcription_config = {
                enabled: true,
                region: body.azure_region || process.env.AZURE_SPEECH_REGION || "eastus",
                apiKey: body.azure_api_key || process.env.AZURE_SPEECH_API_KEY || "",
              ...
              body.stream_url = `${wsBase}/streaming/${wsProvider}${secretParam}`;
        ...
        delete body.azure_region;
        delete body.azure_api_key;
