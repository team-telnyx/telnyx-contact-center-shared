...
import { executeMcpToolNode } from "@/lib/mcp/mcp-tool-runner.js";
import { normalizeSttLanguageCode } from "@/lib/language-code-utils.js";
import {
...
import { voiceRuntimePayload, voiceFlowLogger } from "./voice/logging.mjs";
...
function decodeClientStateObject(encodedClientState) {
  if (!encodedClientState || typeof encodedClientState !== "string") return {};
  try {
    const decoded = Buffer.from(encodedClientState, "base64").toString("utf8");
...
function deepMergeClientState(base, patch) {
  const next = { ...(base || {}) };
  for (const [key, value] of Object.entries(patch || {})) {
    if (
      ...
      next[key] = deepMergeClientState(next[key], value);
...
function parseClientStateValue(rawValue, valueType = "text") {
  if (valueType === "number") {
...
function parseClientStateJsonObject(jsonText) {
  if (!jsonText || typeof jsonText !== "string") return {};
  const parsed = JSON.parse(jsonText);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("Client state JSON must be an object");
  }
  return parsed;
}

function buildClientStatePatch(config = {}) {
  const mode = config.update_mode || (config.client_state ? "legacy_base64" : "predefined");

  if (mode === "legacy_base64") {
    return decodeClientStateObject(config.client_state);
  }

  if (mode === "raw_json") {
    return parseClientStateJsonObject(config.raw_json || "{}");
  }

  if (mode === "custom") {
    const key = String(config.custom_key || "").trim();
    if (!key) return {};
    return {
      [key]: parseClientStateValue(config.custom_value, config.custom_value_type || "text"),
    };
  }

  const predefinedKey = config.predefined_key || "caller_language";
  if (predefinedKey === "caller_language") {
    const value =
      config.value_source === "variable"
        ? config.variable_value
        : config.caller_language || config.value || "en";
    return value ? { caller_language: value } : {};
  }

  return {};
}

function buildMergedClientStateBase64({ existingClientStateBase64, config }) {
  const existing = decodeClientStateObject(existingClientStateBase64);
  const patch = buildClientStatePatch(config);
  const merged = mergeAcdClientState(
    existing,
    deepMergeClientState(existing, patch),
  );
  return encodeAcdClientState(merged);
}

function resolveClientStateUpdateBase({ config, event, executionState }) {
  const webhookClientState =
    event?.data?.payload?.client_state || executionState?.client_state || null;
  const injectedClientState = config?.client_state || null;

  if (webhookClientState && injectedClientState) {
    return encodeAcdClientState(
      mergeAcdClientState(
        webhookClientState,
        deepMergeClientState(
          decodeClientStateObject(webhookClientState),
          decodeClientStateObject(injectedClientState),
        ),
      ),
    );
  }

  return webhookClientState || injectedClientState || null;
}

// End friendly client_state helpers
...
export async function executeFlowNode(
  ...
  try {
    ...
    if (nodeType === "mcp_tool") {
      return await executeMcpToolNode(
        node,
        mergedVariables,
        callControlId,
        executionState,
    ...
    if (nodeType === "set_queue_options") {
      ...
      const clientStateBase64 = encodeAcdClientState(
        mergeAcdClientState(existingClientState, newClientState),
      );
    ...
    // Friendly Update Client State: merge JSON/text builder values into existing
    // client_state and Base64-encode the final object for Telnyx.
    if (nodeType === "client_state_update") {
      try {
        const currentClientState = resolveClientStateUpdateBase({
          config: processedConfig,
          event,
          executionState,
        });
        const clientStateBase64 = buildMergedClientStateBase64({
          existingClientStateBase64: currentClientState,
          config: processedConfig,
        });
        ...
        return {
          success: false,
          error: err?.message || "Invalid client state update configuration",
          variables: mergedVariables,
        };
      }
    }
    ...
    if (nodeType === "agent_assist") {
      ...
      if (assistType === "kb_articles") {
        ...
        agentAssistConfig.show_expanded_stages = processedConfig.show_expanded_stages === true;
        ...
        agentAssistConfig.enable_stt_confidence = processedConfig.enable_stt_confidence !== false;
        agentAssistConfig.enable_llm_confidence = processedConfig.enable_llm_confidence !== false;
    ...
    if (
      ...
      processedConfig.client_state = event.data.payload.client_state;
    }
...
async function resolveStreamingProviderConfig(body) {
  const { getTelnyxStreamingConfig, getProviderConfig } =
    await import("@/config/ai-streaming-providers");
  const providerId = body.ai_streaming_provider === "telnyx-stt"
    ? body.telnyx_stt_model
    : body.ai_streaming_provider;
  return {
    aiProvider: body.ai_streaming_provider,
    providerId,
    providerConfig: getTelnyxStreamingConfig(providerId),
    fullProviderConfig: getProviderConfig(providerId),
  };
}

function getStreamingWebSocketBase() {
...
function decodeStreamingClientState(clientState) {
...
function resolveTelnyxSttLanguage(body = {}, providerConfig = {}, existingClientState = {}, variables = {}) {
  const normalizeLanguage = (language) => {
    const supportedLanguages = providerConfig?.telnyxStt?.supported_languages || [];
    const supportedCodes = supportedLanguages.map((option) => option.value);
    const defaultLanguage = normalizeSttLanguageCode(providerConfig?.telnyxStt?.language, {
      supportedCodes,
      fallback: supportedCodes.includes("en") ? "en" : supportedCodes[0] || "en",
    });
    return normalizeSttLanguageCode(language, {
      supportedCodes,
      fallback: defaultLanguage,
    });
  };

  if (body.telnyx_stt_use_caller_language === true) {
    return normalizeLanguage(existingClientState?.caller_language);
  }

  if (body.telnyx_stt_language_source === "variable") {
    const resolved = resolveFormDataVariable(body.telnyx_stt_language, {
      ...variables,
      client_state: existingClientState || {},
      caller_language: existingClientState?.caller_language,
    });
    if (resolved) return normalizeLanguage(resolved);
  }

  return normalizeLanguage(body.telnyx_stt_language);
}

async function applyStreamingProviderConfiguration(body, action) {
  if (!body.ai_streaming_provider || body.ai_streaming_provider === "custom") return body;

  const { aiProvider, providerConfig, fullProviderConfig } =
    await resolveStreamingProviderConfig(body);
  if (!providerConfig) return body;

  const streamWsBase = getStreamingWebSocketBase();
  ...
  if (fullProviderConfig?.type === "telnyx-stt") {
    body.stream_url = `${streamWsBase}/streaming/telnyx-stt${secretParam}`;
    Object.assign(body, providerConfig);

    const transcriptionTracks = normalizeTelnyxSttTracks(
      body.telnyx_stt_tracks ||
        fullProviderConfig.telnyxStt?.transcription_tracks,
    );
    // The call-flow stream carries only the customer. ACD Core starts a second,
    // codec-aware media fork on the transferred agent transport leg after its
    // call.answered webhook identifies the negotiated codec.
    body.stream_track = telnyxSttMediaStreamTrack(transcriptionTracks);

    // Telnyx standalone STT consumes raw media frames as PCMU/mulaw. Old Answer/
    // Streaming nodes may still carry bidirectional AI fields such as
    // stream_bidirectional_mode: "mp3" from earlier provider selections; leaving
    // them in the Telnyx Call Control payload can change the media mode while the
    // STT bridge still declares input_format=mulaw. Strip them for this direct STT
    // path so stream_codec=PCMU remains the only audio format contract.
    delete body.stream_bidirectional_mode;
    delete body.stream_bidirectional_codec;
    delete body.stream_bidirectional_target_legs;
    delete body.stream_bidirectional_sampling_rate;

    const existingClientState = decodeStreamingClientState(body.client_state);
    const telnyxSttLanguage = resolveTelnyxSttLanguage(body, fullProviderConfig, existingClientState);
    existingClientState.telnyx_stt_config = {
      ...(fullProviderConfig.telnyxStt || {}),
      language: telnyxSttLanguage,
      stream_track: body.stream_track,
      transcription_tracks: transcriptionTracks,
      interim_results: body.telnyx_stt_interim_results !== false,
      enabled: true,
      flowId: existingClientState.flowId || existingClientState.flow_id || null,
      flow_id: existingClientState.flow_id || existingClientState.flowId || null,
    };
    body.client_state = encodeAcdClientState(existingClientState);
  } else {
...
function cleanupStreamingProviderFields(body) {
  delete body.telnyx_stt_tracks;
  delete body.telnyx_stt_model;
  delete body.telnyx_stt_interim_results;
  delete body.telnyx_stt_language;
  delete body.telnyx_stt_language_source;
  delete body.telnyx_stt_use_caller_language;
  ...
  if (body.stream_bidirectional_sampling_rate && typeof body.stream_bidirectional_sampling_rate === "string") {
    body.stream_bidirectional_sampling_rate = parseInt(body.stream_bidirectional_sampling_rate, 10);
  }
  return body;
}
  ...
  try {
    ...
    switch (action) {
      ...
      case "answer":
        ...
        body = await applyStreamingProviderConfiguration(body, action);
        body = cleanupStreamingProviderFields(body);
        // Transform transcription options to correct API format
        body = transformTranscriptionOptions(body);
        // Clean up empty string values in recording options
        body = cleanupRecordingOptions(body);
      ...
      case "ai_assistant_start":
        ...
        // Webhook updates on conversation message history changes.
        // Telnyx defaults this to false, so only send the flag when explicitly
        // enabled. Coerce loosely since variable substitution may yield a string.
        if (
          send_message_history_updates !== undefined &&
          send_message_history_updates !== null &&
          send_message_history_updates !== ""
        ) {
          body.send_message_history_updates =
...
function replaceDataActionFieldValue(value, variables) {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => replaceDataActionFieldValue(item, variables));
  }
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, nestedValue]) => [
        key,
        replaceDataActionFieldValue(nestedValue, variables),
      ]),
    );
  }
 ...
 */
export function extractVariablesFromWebhook(payload, variableMappings = [], variablesScope = {}) {
  const variables = {};
  const rootScope = {
    payload,
    query: payload?.query || {},
  ...
  variableMappings.forEach((mapping) => {
    ...
    // Otherwise resolve against both webhook roots and existing execution variables
    // so immediate-node outputs (for example Data Action's data_response) can be mapped.
    if (sourcePath.startsWith("payload.")) {
      ...
      value = getValueByPath(rootScope, sourcePath);
    }

    if (value === undefined && sourcePath.includes("[]")) {
      const fallbackPath = sourcePath.endsWith("[]")
        ? sourcePath.slice(0, -2).replaceAll("[]", ".0")
        : sourcePath.replaceAll("[]", ".0");

      if (fallbackPath.startsWith("payload.")) {
        value = getValueByPath(payload, fallbackPath.substring(8));
      } else if (fallbackPath.startsWith("query.")) {
        value = getValueByPath(payload?.query || {}, fallbackPath.substring(6));
      } else {
        value = getValueByPath(rootScope, fallbackPath);
      }
    }

    if (value !== undefined) {
      variables[variableName] = value;
      voiceFlowLogger.debug("voice_flow_extractvariablesfromwebhook", voiceRuntimePayload({ eventType: typeof event !== "undefined" ? event : typeof eventType !== "undefined" ? eventType : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof payload !== "undefined" ? payload?.call_control_id : undefined, callSessionId: typeof callSessionId !== "undefined" ? callSessionId : typeof payload !== "undefined" ? payload?.call_session_id : undefined, flowId: typeof flowId !== "undefined" ? flowId : typeof flow !== "undefined" ? flow?.id : undefined, nodeId: typeof nodeId !== "undefined" ? nodeId : typeof node !== "undefined" ? node?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined, provider: typeof provider !== "undefined" ? provider : undefined }));
    } else {
      voiceFlowLogger.warn("voice_flow_extractvariablesfromwebhook", voiceRuntimePayload({ error: typeof err !== "undefined" ? err : typeof error !== "undefined" ? error : typeof e !== "undefined" ? e : undefined, eventType: typeof event !== "undefined" ? event : typeof eventType !== "undefined" ? eventType : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof payload !== "undefined" ? payload?.call_control_id : undefined, callSessionId: typeof callSessionId !== "undefined" ? callSessionId : typeof payload !== "undefined" ? payload?.call_session_id : undefined, flowId: typeof flowId !== "undefined" ? flowId : typeof flow !== "undefined" ? flow?.id : undefined, nodeId: typeof nodeId !== "undefined" ? nodeId : typeof node !== "undefined" ? node?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined, provider: typeof provider !== "undefined" ? provider : undefined }));
    }
  });
...
export function shouldContinueImmediately(eventType, currentNode) {
  ...
  voiceFlowLogger.debug("voice_flow_shouldcontinueimmediately", voiceRuntimePayload({ eventType: typeof event !== "undefined" ? event : typeof eventType !== "undefined" ? eventType : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof payload !== "undefined" ? payload?.call_control_id : undefined, callSessionId: typeof callSessionId !== "undefined" ? callSessionId : typeof payload !== "undefined" ? payload?.call_session_id : undefined, flowId: typeof flowId !== "undefined" ? flowId : typeof flow !== "undefined" ? flow?.id : undefined, nodeId: typeof nodeId !== "undefined" ? nodeId : typeof node !== "undefined" ? node?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined, provider: typeof provider !== "undefined" ? provider : undefined }));
  voiceFlowLogger.debug("voice_flow_shouldcontinueimmediately", voiceRuntimePayload({ eventType: typeof event !== "undefined" ? event : typeof eventType !== "undefined" ? eventType : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof payload !== "undefined" ? payload?.call_control_id : undefined, callSessionId: typeof callSessionId !== "undefined" ? callSessionId : typeof payload !== "undefined" ? payload?.call_session_id : undefined, flowId: typeof flowId !== "undefined" ? flowId : typeof flow !== "undefined" ? flow?.id : undefined, nodeId: typeof nodeId !== "undefined" ? nodeId : typeof node !== "undefined" ? node?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined, provider: typeof provider !== "undefined" ? provider : undefined }));
...
function executeSetVariableNode(node, variables, callControlId) {
  try {
    ...
    voiceFlowLogger.error("voice_flow_executesetvariablenode", voiceRuntimePayload({ error: typeof err !== "undefined" ? err : typeof error !== "undefined" ? error : typeof e !== "undefined" ? e : undefined, eventType: typeof event !== "undefined" ? event : typeof eventType !== "undefined" ? eventType : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof payload !== "undefined" ? payload?.call_control_id : undefined, callSessionId: typeof callSessionId !== "undefined" ? callSessionId : typeof payload !== "undefined" ? payload?.call_session_id : undefined, flowId: typeof flowId !== "undefined" ? flowId : typeof flow !== "undefined" ? flow?.id : undefined, nodeId: typeof nodeId !== "undefined" ? nodeId : typeof node !== "undefined" ? node?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined, provider: typeof provider !== "undefined" ? provider : undefined }));
    voiceFlowLogger.error("voice_flow_executesetvariablenode", voiceRuntimePayload({ error: typeof err !== "undefined" ? err : typeof error !== "undefined" ? error : typeof e !== "undefined" ? e : undefined, eventType: typeof event !== "undefined" ? event : typeof eventType !== "undefined" ? eventType : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof payload !== "undefined" ? payload?.call_control_id : undefined, callSessionId: typeof callSessionId !== "undefined" ? callSessionId : typeof payload !== "undefined" ? payload?.call_session_id : undefined, flowId: typeof flowId !== "undefined" ? flowId : typeof flow !== "undefined" ? flow?.id : undefined, nodeId: typeof nodeId !== "undefined" ? nodeId : typeof node !== "undefined" ? node?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined, provider: typeof provider !== "undefined" ? provider : undefined }));
    addNodeExecutionEvent(
      callControlId,
      "set_variable",
...
async function executeHttpRequestNode(node, variables, callControlId) {
  try {
    ...
    if (!url) {
      ...
      voiceFlowLogger.debug("voice_flow_executehttprequestnode", voiceRuntimePayload({ eventType: typeof event !== "undefined" ? event : typeof eventType !== "undefined" ? eventType : undefined, callControlId: typeof callControlId !== "undefined" ? callControlId : typeof payload !== "undefined" ? payload?.call_control_id : undefined, callSessionId: typeof callSessionId !== "undefined" ? callSessionId : typeof payload !== "undefined" ? payload?.call_session_id : undefined, flowId: typeof flowId !== "undefined" ? flowId : typeof flow !== "undefined" ? flow?.id : undefined, nodeId: typeof nodeId !== "undefined" ? nodeId : typeof node !== "undefined" ? node?.id : undefined, reason: typeof reason !== "undefined" ? reason : undefined, provider: typeof provider !== "undefined" ? provider : undefined }));
      addNodeExecutionEvent(
        callControlId,
        "http_request_action",
        node.id,
    ...
    // Replace variables and resolve secret references in path parameters and URL
    const processedPathParams = {};
    for (const [key, value] of Object.entries(pathParams)) {
      const processedValue = replaceVariables({ value }, variables).value;
      processedPathParams[key] = await resolveSecretReferences(processedValue);
    }

    // Replace variables and resolve secret references in URL
    let processedUrl = replaceVariables({ url }, variables).url;
    processedUrl = await resolveSecretReferences(processedUrl);
    ...
    // Replace variables and resolve secret references in query parameters
    const processedQueryParams = {};
    for (const [key, value] of Object.entries(queryParams)) {
      const processedValue = replaceVariables({ value }, variables).value;
      processedQueryParams[key] = await resolveSecretReferences(processedValue);
    }
