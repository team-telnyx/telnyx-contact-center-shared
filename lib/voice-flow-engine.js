...
export async function executeFlowNode(
  ...
  try {
    ...
    if (nodeType === "enqueue") {
      ...
      if (processedConfig.use_queue_options) {
        ...
        if (event?.data?.payload?.client_state) {
          ...
          console.log(
            "[FlowEngine] Enqueue with use_queue_options - found client_state in event payload",
          );
        }
        // Fallback to node config if not in event (for backward compatibility)
          ...
          console.log(
            "[FlowEngine] Enqueue with use_queue_options - using client_state from node config",
          );
        } else {
          console.error(
            "[FlowEngine] Enqueue with use_queue_options - no client_state found. Event payload:",
            event?.data?.payload
              ? Object.keys(event.data.payload)
              : "no payload",
            "Config keys:",
            Object.keys(processedConfig),
          );
        }
        ...
        if (clientStateSource) {
          try {
            ...
            if (clientStateObj.queue_name) {
              ...
              console.error(
                "[FlowEngine] Enqueue with use_queue_options - queue_name not found in client_state. Client state:",
                clientStateObj,
              );
              // Don't proceed without queue_name - enqueue will fail
              throw new Error(
            ...
            // Also remove max_size - Telnyx doesn't allow modifying max_size of existing queues
            ...
            console.log(
              "[FlowEngine] Enqueue with use_queue_options - keeping client_state in request for routing parameters, removed call_priority and max_size from body",
            );
          } catch (err) {
            console.error(
          ...
          const errorMsg =
            "[FlowEngine] Enqueue with use_queue_options - no client_state found in event or config. Make sure Set Queue Options node runs before Enqueue node.";
        ...
        if (!processedConfig.queue_name) {
          const errorMsg =
            "[FlowEngine] Enqueue with use_queue_options - queue_name is required but not set";
          console.error(errorMsg);
          throw new Error(errorMsg);
        }
    ...
    // Call the Telnyx API
    // Log enqueue parameters for debugging
    if (nodeType === "enqueue") {
      console.log(
        "[FlowEngine] Enqueue API call - parameters:",
        JSON.stringify(
          {
            queue_name: processedConfig.queue_name,
            max_wait_time_secs: processedConfig.max_wait_time_secs,
            max_size: processedConfig.max_size,
            keep_after_hangup: processedConfig.keep_after_hangup,
            use_queue_options: processedConfig.use_queue_options,
  ...
  try {
    ...
    switch (action) {
      ...
      case "enqueue":
        ...
        // Log enqueue request for debugging
        console.log(
          "[FlowEngine] Enqueue API request:",
          JSON.stringify(
            {
              url,
              method,
              queue_name: body.queue_name,
              max_wait_time_secs: body.max_wait_time_secs,
              max_size: body.max_size,
              keep_after_hangup: body.keep_after_hangup,
              has_client_state: !!body.client_state,
            },
            null,
            2,
          ),
        ...
        if (!body.queue_name) {
          console.error(
            "[FlowEngine] Enqueue API call failed: queue_name is required but missing",
          );
    ...
    // Log request body for enqueue to debug priority-based routing issues
    if (action === "enqueue") {
      console.log(
        "[FlowEngine] Enqueue API request body:",
    ...
    // Log response for enqueue to debug priority-based routing issues
    if (action === "enqueue") {
      console.log(
        "[FlowEngine] Enqueue API response status:",
        response.status,
        response.statusText,
      );
    }
