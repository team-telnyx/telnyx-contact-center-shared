...
async function applyStreamingProviderConfiguration(body, action) {
  ...
  if (fullProviderConfig?.type === "telnyx-stt") {
    ...
    // Telnyx standalone STT consumes raw media frames using the RTP L16 test contract:
    // Call Control requests stream_codec=L16 plus stream_bidirectional_mode=rtp,
    // stream_bidirectional_codec=L16, and stream_bidirectional_sampling_rate=16000
    // from Telnyx Media Streaming, while the provider WebSocket declares
    // input_format=linear16&sample_rate=16000. The provider config owns those fields
    // so stale node values such as stream_bidirectional_mode=mp3 are overwritten.
  ...
  try {
    ...
    switch (action) {
      ...
      case "answer":
        ...
        // Telnyx AnswerRequest does not accept this StartStreaming-only option.
