...
function buildMergedClientStateBase64({ existingClientStateBase64, config }) {
  ...
  const merged = deepMergeClientState(existing, patch);
  return Buffer.from(JSON.stringify(merged)).toString("base64");
}

function encodeClientStateObject(clientState) {
  return Buffer.from(JSON.stringify(clientState)).toString("base64");
}
...
function resolveClientStateUpdateBase({ config, event, executionState }) {
  ...
  if (webhookClientState && injectedClientState) {
    return encodeClientStateObject(
...
async function applyStreamingProviderConfiguration(body, action) {
  ...
  if (fullProviderConfig?.type === "telnyx-stt") {
    ...
    existingClientState.telnyx_stt_config = {
      ...
      stream_track: body.stream_track || providerConfig?.stream_track || null,
    ...
    body.client_state = Buffer.from(JSON.stringify(existingClientState)).toString("base64");
  } else {
