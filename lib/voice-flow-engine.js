    ...
    if (nodeType === "set_queue_options") {
      ...
      if (processedConfig.client_state) {
        try {
          ...
          existingClientState = { ...existingClientState, ...configClientState };
        } catch {
          // Ignore decode errors
        }
      }
      ...
      const priorityConfig = processedConfig.call_priority !== undefined 
      ...
      if (priorityConfig !== undefined) {
        ...
        if (typeof priorityValue === "number" && !isNaN(priorityValue) && priorityValue >= 1 && priorityValue <= 5) {
          newClientState.call_priority = priorityValue;
          console.log(`[FlowEngine] Set Queue Options - Setting call_priority to ${priorityValue} in client_state`);
        } else {
          console.warn(`[FlowEngine] Set Queue Options - Invalid call_priority value: ${priorityValue} (type: ${typeof priorityValue})`);
        }
      } else {
        console.warn(`[FlowEngine] Set Queue Options - call_priority not found in processedConfig. Available keys: ${Object.keys(processedConfig).join(", ")}`);
      }
