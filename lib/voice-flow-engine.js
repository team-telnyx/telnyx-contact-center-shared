  ...
  if (predefinedKey === "caller_language") {
    const value =
      config.value_source === "variable"
        ...
        : config.caller_language || config.value || "en-US";
...
function resolveTelnyxSttLanguage(body = {}, providerConfig = {}, existingClientState = {}, variables = {}) {
  const normalizeLanguage = (language) => {
    ...
    const defaultLanguage = providerConfig?.telnyxStt?.language || supportedCodes[0] || "en-US";
    const candidate = String(language || "").trim();
    if (!candidate) return defaultLanguage;
    ...
    const regionalLanguage = supportedCodes.find(
      (code) => String(code).split("-")[0] === baseLanguage,
    );
