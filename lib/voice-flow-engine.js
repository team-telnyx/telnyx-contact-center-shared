...
async function applyStreamingProviderConfiguration(body, action) {
  ...
  if (fullProviderConfig?.type === "telnyx-stt") {
    ...
    // Telnyx standalone STT consumes raw media frames using the L16 test contract:
    // Call Control requests stream_codec=L16 from Telnyx Media Streaming, while the
    // provider WebSocket declares input_format=linear16&sample_rate=16000. Old
    // Answer/Streaming nodes may still carry bidirectional AI fields such as
    // stream_bidirectional_mode: "mp3" from earlier provider selections; leaving
    // them in the Telnyx Call Control payload can change the media mode. Strip
    // them for this direct STT path so the L16 -> linear16 contract stays intact.
