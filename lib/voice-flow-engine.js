...
export async function executeFlowNode(
  ...
  try {
    ...
    // Special handling for enqueue node: log client_state for debugging
    if (nodeType === "enqueue") {
      ...
      if (processedConfig.client_state) {
        try {
          ...
          console.log(
            "[FlowEngine] Failed to decode enqueue client_state:",
            err
          );
        }
      }
    ...
    // Special handling for enqueue node: update client_state BEFORE enqueuing
    // This ensures the call.enqueued webhook includes routing parameters
    if (nodeType === "enqueue" && processedConfig.client_state) {
      console.log(
        "[FlowEngine] Updating client_state before enqueue to ensure routing params are in call.enqueued webhook"
      );
      ...
      if (!clientStateUpdateResult.success) {
        console.warn(
          "[FlowEngine] Failed to update client_state before enqueue:",
...
export async function callTelnyxAction(flowId, action, callControlId, params) {
  ...
  try {
      ...
      case "playback_start":
        ...
        if (body.audio_url) {
          ...
          if (!isUrl) {
            ...
            console.log(
              "[FlowEngine] Using media_name instead of audio_url:",
              body.media_name
            );
          }
        }
    ...
    // Log enqueue requests for debugging
    if (action === "enqueue") {
      console.log("[FlowEngine] ===== ENQUEUE REQUEST =====");
      console.log("[FlowEngine] URL:", url);
      console.log("[FlowEngine] Body client_state:", body.client_state);
      if (body.client_state) {
        try {
          const decoded = Buffer.from(body.client_state, "base64").toString();
          const clientStateObj = JSON.parse(decoded);
          console.log(
            "[FlowEngine] Body client_state decoded:",
            clientStateObj
          );
        } catch (err) {
          console.log("[FlowEngine] Failed to decode body client_state:", err);
        }
      }
      console.log("[FlowEngine] Body (full):", JSON.stringify(body, null, 2));
    }
