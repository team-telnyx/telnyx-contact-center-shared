...
export function extractVariablesFromWebhook(payload, variableMappings = [], variablesScope = {}) {
  ...
  variableMappings.forEach((mapping) => {
    ...
    if (value === undefined && sourcePath.includes("[]")) {
      const firstItemPath = sourcePath.replaceAll("[]", ".0");
      if (firstItemPath.startsWith("payload.")) {
        value = getValueByPath(payload, firstItemPath.substring(8));
      } else if (firstItemPath.startsWith("query.")) {
        value = getValueByPath(payload?.query || {}, firstItemPath.substring(6));
      } else {
        value = getValueByPath(rootScope, firstItemPath);
      }
    }
