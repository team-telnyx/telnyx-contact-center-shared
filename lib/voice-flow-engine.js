  ...
  try {
    ...
    if (nodeType === "agent_assist") {
      ...
      if (assistType === "kb_articles") {
        ...
        // Web Pages options only. Save selected page IDs; empty array means all active pages.
        const webPageIds = Array.isArray(processedConfig.web_page_ids)
          ? processedConfig.web_page_ids.filter(Boolean)
          : processedConfig.web_page_id
            ? [processedConfig.web_page_id]
            : [];
        agentAssistConfig.web_page_id = processedConfig.web_page_id || webPageIds[0] || null;
        agentAssistConfig.web_page_ids = webPageIds;
      }
      ...
      if (clientStateUpdateResult.success) {
        console.log(
          `[FlowEngine] Agent Assist configured: type=${agentAssistConfig.assist_type}, workflow=${agentAssistConfig.workflow_id || "none"}, forms=${agentAssistConfig.form_ids?.length || 0}, web_pages=${agentAssistConfig.web_page_ids?.length || 0}`,
        );
        return {
          success: true,
          output: 0,
