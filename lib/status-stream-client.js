...
const coreSnapshotSubscribers = new Set();

export function applyCoreStatusSnapshot(agent) {
  if (!agent) { coreStatus = null; return; }
  coreStatus = {
    ...agent,
    status: agent.status,
    version: agent.version,
  };
  emit("status_changed", coreStatus);
}

export function applyCoreSnapshot(snapshot) {
  if (!snapshot) return;
  coreSnapshot = snapshot;
  applyCoreStatusSnapshot(snapshot.agent);
  for (const handler of coreSnapshotSubscribers) {
    try { handler(snapshot); } catch (_) {}
  }
}

export function subscribeCoreSnapshot(handler) {
  if (typeof handler !== "function") return () => {};
  coreSnapshotSubscribers.add(handler);
  if (coreSnapshot) {
    try { handler(coreSnapshot); } catch (_) {}
  }
  return () => coreSnapshotSubscribers.delete(handler);
}
  ...
  try {
    ...
    for (const name of EVENT_NAMES) {
      ...
      eventSource.addEventListener(name, (event) => {
        emit(name, name === "status_changed" && coreStatus ? coreStatus : safeParse(event?.data));
      });
    }
