...
export async function signAccessToken(payload, expiresIn = "1d") {
  ...
  return await new SignJWT({ ...payload, purpose: "access" })
...
export async function verifyAccessToken(token) {
  ...
  try {
    ...
    return payload.purpose === "refresh" ? null : payload;
  } catch (_) {
    return null;
  }
}
...
export async function signRefreshToken(payload, expiresIn = "30d") {
  ...
  return await new SignJWT({ ...payload, purpose: "refresh" })
    .setProtectedHeader({ alg: "HS256" })
    .setJti(randomUUID())
    .setIssuedAt()
    .setIssuer(issuer)
...
export async function verifyRefreshToken(token) {
  ...
  try {
    ...
    return payload.purpose === "refresh" ? payload : null;
  } catch (_) {
    return null;
  }
}
