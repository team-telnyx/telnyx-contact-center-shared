...
export function validateFlow(flow, queues = []) {
  ...
  if (!hasEntryNode) {
    errors.push(
      "Flow must have at least one entry node (Answer, Dial, Incoming Call, or HTTP Request)"
    );
  }
  ...
  if (initiatorNodes.length > 1) {
    errors.push(
      "Only one initiator node is allowed per flow (Incoming Call or HTTP Request)"
    );
  }
  ...
  nodes.forEach((node) => {
    ...
    if (nodeType === "set_queue_options") {
      ...
      if (queueNameUseVariable) {
        if (!config.queue_name_variable || config.queue_name_variable.trim() === "") {
          errors.push(
      ...
      // Validate priority
      const priorityUseVariable = config.priority_use_variable;
      if (priorityUseVariable) {
        if (!config.priority_variable || config.priority_variable.trim() === "") {
          errors.push(
            `Node "${nodeData.label || nodeId}" (Set Queue Options) is missing required parameter "Priority variable"`
          );
        }
      } else {
        if (config.priority === undefined || config.priority === null || config.priority === "") {
          errors.push(
            `Node "${nodeData.label || nodeId}" (Set Queue Options) is missing required parameter "Priority"`
          );
        } else if (config.priority < 1 || config.priority > 100) {
          errors.push(
            `Node "${nodeData.label || nodeId}" (Set Queue Options) has invalid priority value. Priority must be between 1 and 100.`
          );
        }
      }
      ...
      if (queueName && queues.length > 0) {
        ...
        if (queue && queue.routing_strategy === "Skill-based") {
          ...
          const hasSkills =
            ...
            routingSkills.some((skill) => skill && skill.name && skill.proficiency);

          // Also check client_state for required_skills
          let hasSkillsInClientState = false;
          if (!hasSkills && config.client_state) {
            try {
              const decoded = Buffer.from(config.client_state, "base64").toString();
