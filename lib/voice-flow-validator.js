...
export function validateFlow(flow, queues = []) {
  ...
  if (!hasEntryNode) {
    errors.push(
      "Flow must have at least one entry node (Answer, Dial, Incoming Call, HTTP Request, Form Submit, or Outbound Campaign)",
    );
  }
  ...
  if (initiatorNodes.length > 1) {
    errors.push(
      "Only one initiator node is allowed per flow (Incoming Call, HTTP Request, Form Submit, or Outbound Campaign)",
    );
  }
  ...
  nodes.forEach((node) => {
    ...
    // Special validation for HTTP Request initiators with authentication.
    if (nodeType === "http_request") {
      const authRequired =
        config.auth_required === true || config.auth_required === "true";
      const authToken =
      ...
      if (authRequired && !authToken) {
        errors.push(
          ...
          }" (HTTP Request) requires an Authentication Token when authentication is enabled`,
        );
      }
    }
