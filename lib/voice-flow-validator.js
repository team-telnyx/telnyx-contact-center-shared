  ...
  nodes.forEach((node) => {
    ...
    Object.entries(config).forEach(([key, value]) => {
      if (typeof value === "string" && value.includes("{{")) {
        // Skip validation for legacy {{username}} in enqueue node's queue_name
        // This is a deprecated feature that's being phased out
        if (
          nodeType === "enqueue" &&
          key === "queue_name" &&
          value === "{{username}}"
        ) {
          return; // Skip validation for this legacy value
        }
