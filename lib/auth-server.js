...
export async function getAuthenticatedUser(requestUrl = null) {
  try {
    ...
    // If no bearer/query token is present, accept the custom app auth cookie
    // issued by /api/auth/signin and /api/auth/refresh. The web AuthProvider
    // calls /api/auth/me with credentials: "include", so this path is required
    // for browser logins that use the app's local credentials endpoint.
    if (!token) {
    ...
    if (token) {
      try {
        ...
        console.log("[AUTH] JWT verification failed, trying session auth");
      }
    }
