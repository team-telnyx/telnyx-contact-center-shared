import { hasAuthSession } from "./auth-refresh-sessions.mjs";
...
export async function getAuthenticatedUser(requestUrl = null) {
  try {
    ...
    if (token) {
      try {
        ...
        if (payload?.sub) {
          ...
          if (user && user.active !== false && (!payload.sid || hasAuthSession(user, payload.sid))) {
            return { ...user, authSessionId: payload.sid || null };
          }
        }
      } catch (jwtErr) {
    ...
    if (token) return null; // Explicit bearer credentials never fall through to another identity.
    ...
    return user?.active === false ? null : user ? { ...user, authSessionId: session?.authTrackingSessionId || null } : null;
  } catch (err) {
