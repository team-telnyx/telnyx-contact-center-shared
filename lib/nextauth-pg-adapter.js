...
export function PostgresNextAuthAdapter() {
  ...
  return {
    async createUser(data) {
      ...
      const r = await query(`SELECT * FROM auth_users WHERE id=$1`, [id]);
    ...
    async getUserByEmail(email) {
      const r = await query(`SELECT * FROM auth_users WHERE email=$1`, [
        email,
      ]);
      return r.rows?.[0] || null;
    },
    ...
    async updateUser(data) {
      ...
      const r = await query(`SELECT * FROM auth_users WHERE id=$1`, [
        data.id,
      ]);
      return r.rows?.[0] || null;
    },
    ...
    async createSession(data) {
      ...
      const r = await query(`SELECT * FROM auth_sessions WHERE id=$1`, [
        id,
      ]);
      return {
        id: r.rows?.[0]?.id,
