    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('message', (msg) => {
        ...
        if (firstLine.includes('200 OK')) {
          ...
          // Resolve with success=false for non-200 responses (e.g. 401 Unauthorized)
          // instead of rejecting, so UI can show the error details
          resolve({
            success: false,
            action,
            target: `${ip}:${port}`,
            error: `Phone rejected command: ${firstLine}`,
            response: firstLine,
      ...
      client.send(buffer, 0, buffer.length, port, ip, (err) => {
        ...
        // Don't close client here, wait for response
      });

      // Set timeout
      setTimeout(() => {
        client.close();
        // Resolve with timeout error instead of reject to prevent crash
        resolve({
          success: false,
          error: 'Timeout waiting for phone response (check phone IP allowlist configuration)',
          timestamp: new Date().toISOString()
        });
      }, 5000);
    });
