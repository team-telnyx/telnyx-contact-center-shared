...
export const PolycomAPI = {
  ...
  hangup: (config) => sendRequest(config, 'POST', '/api/v1/callctrl/endCall', { data: { Ref: "0" } }), // Ref 0 usually means current/active call
  hold: (config) => sendRequest(config, 'POST', '/api/v1/callctrl/hold', { data: { Ref: "0" } }), 
  mute: (config) => sendRequest(config, 'POST', '/api/v1/callctrl/mute', { data: { Mute: "1" } }), // 1=mute, 0=unmute
