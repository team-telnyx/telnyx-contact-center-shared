import { completeCampaignIfExhausted } from "./completion.js";
...
async function tick(campaignId) {
  ...
  try {
    ...
    if (!claim) {
      const completion = await completeCampaignIfExhausted(state.pool, campaign, state.username || "system");
    ...
    runnerLogger.error("runner_tick_failed", { campaignId, ...outboundErrorPayload(err) });
