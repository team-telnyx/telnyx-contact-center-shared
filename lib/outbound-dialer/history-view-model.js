...
const CONTACT_IDENTITY_FIELD_TYPES = ["first_name", "last_name", "display_name", "company"];
...
export function contactRecordLabel(record = {}, contactList = {}) {
  ...
  const name = firstValue([first, last].filter(Boolean).join(" "), displayName);
  const methodNumbers = valuesFromContactMethods(methods);
  const to = firstValue(record.to_number, ...methodNumbers, row.phone, row.mobile, row.number, row.phone_number, row.msisdn, "Unknown number");
  return {
