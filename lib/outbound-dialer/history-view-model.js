...
function runtimeShowsExhausted(runtime = {}) {
  const progress = runtime?.progress || {};
  const live = runtime?.live || {};
  const total = Number(progress.total || 0);
  const remaining = Number(progress.remaining || 0);
  const active = Number(live.active || 0);
  const ringing = Number(live.ringing || 0);
  return total > 0 && remaining === 0 && active === 0 && ringing === 0;
}

export function normalizeCampaignExecutionState(campaign = {}, runtime = {}) {
  ...
  if (status === "running" && runtimeShowsExhausted(runtime)) return "exhausted";
...
export function campaignControlState(campaign = {}, saving = false, runtime = {}) {
  const state = normalizeCampaignExecutionState(campaign, runtime);
...
function labelTitle(value) {
  return String(value || "").replace(/_/g, " ").replace(/\b\w/g, (m) => m.toUpperCase()).replace(/\bAi\b/g, "AI");
}

export function attemptStatusReasonLabel(attempt = {}) {
  const status = String(attempt?.status || "unknown").trim().toLowerCase();
  const reason = String(attemptReasonCode(attempt) || "").trim().toLowerCase();
  const statusLabel = labelTitle(status);
  if (status === "cancelled") return statusLabel;
  if (!reason || reason === status) return statusLabel;
  return `${statusLabel} - ${labelTitle(reason)}`;
}
...
function rowValueByHeaders(row = {}, headers = []) {
  if (!row || typeof row !== "object") return "";
  const normalized = new Map(Object.entries(row).map(([key, value]) => [normalizeRowKey(key), value]));
  for (const header of headers) {
    const value = normalized.get(normalizeRowKey(header));
...
export function contactRecordLabel(record = {}, contactList = {}) {
  ...
  const first = firstValue(row[semantic.first_name], row.first_name, row.firstname, row.firstName, row.first, rowValueByHeaders(row, ["first name", "firstname", "first"]));
  const last = firstValue(row[semantic.last_name], row.last_name, row.lastname, row.lastName, row.last, rowValueByHeaders(row, ["last name", "lastname", "last", "surname"]));
  const displayName = firstValue(row[semantic.display_name], row.display_name, row.displayName, row.name, row.full_name, row.fullName, rowValueByHeaders(row, ["display name", "displayname", "display", "name"]));
  const company = firstValue(row[semantic.company], row.company_name, row.company, row.organization, row.account, rowValueByHeaders(row, ["company", "company name", "organization", "organisation", "account"]));
  const fullName = firstValue(row[semantic.full_name], row.full_name, row.fullName, rowValueByHeaders(row, ["first & last name", "first and last name", "full name", "fullname"]));
  ...
  const to = firstValue(record.to_number, ...methodNumbers, row[semantic.phone], row.phone, row.mobile, row.number, row.phone_number, row.msisdn, rowValueByHeaders(row, ["phone", "phone number", "mobile", "number", "tel", "telephone"]), "Unknown number");
  return {
...
export function contactRecordHeaderLabel(label = {}) {
  return [label.to, label.name || label.displayName, label.company].map((value) => String(value || "").trim()).filter(Boolean).join(" | ");
}
