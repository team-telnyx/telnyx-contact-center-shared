...
export function campaignServedCount(campaign = {}) {
  ...
  const numeric = Number(campaign.run_served_count ?? campaign.runServedCount ?? assignmentMetadata.served_count ?? campaign.served_count ?? campaign.servedCount ?? 0);
  return Number.isFinite(numeric) ? Math.max(0, Math.floor(numeric)) : 0;
}

export function campaignPriorityBaseline(campaign = {}) {
  const runMetadata = parseJson(campaign.active_run_metadata ?? campaign.activeRunMetadata, {});
  const numeric = Number(campaign.priority_baseline ?? campaign.priorityBaseline ?? runMetadata.priority_baseline ?? 0);
  return Number.isFinite(numeric) ? Math.max(0, numeric) : 0;
}

export function campaignPriorityScore(campaign = {}) {
  return campaignPriorityBaseline(campaign) + (campaignServedCount(campaign) / campaignDistributionWeight(campaign));
}
...
export function priorityDistributionCursor(campaigns = []) {
  ...
  return [...candidates].sort((a, b) => {
    ...
    const aScore = campaignPriorityScore(a);
    const bScore = campaignPriorityScore(b);
