import { campaignAgentAssistConfig } from "./agent-assist.js";
export { campaignAgentAssistConfig } from "./agent-assist.js";
import { startOutboundConnect } from "../acd/sagas/outbound-connect.mjs";
import crypto from "crypto";
import { startCampaignRun, claimOneAgentlessRecord } from "./execution.js";
...
import { campaignsLogger, outboundErrorPayload } from "./logging.mjs";
...
async function loadActiveAgentCampaigns(pool, agentUsername) {
  ...
  const { rows } = await pool.query(
    `SELECT c.*, a.metadata AS assignment_metadata, a.updated_at AS assignment_updated_at,
       ...
       active_run.metadata AS active_run_metadata,
...
export async function claimNextAgentCampaignRecord(pool, agentUsername) {
  ...
  try {
    ...
    await tx.query(`SELECT pg_advisory_xact_lock(hashtext($1))`, [`agent-campaign:${agentUsername}`]);
    const campaigns = await loadActiveAgentCampaigns(tx, agentUsername);
    const agent = (await tx.query(`SELECT id FROM users WHERE username = $1`, [agentUsername])).rows[0];
    if (!agent || !campaigns.length) { await tx.query("COMMIT"); return null; }
    for (const campaign of campaigns) {
      const previous = await findExistingAgentClaim(tx, campaign.id, agentUsername);
      if (previous) {
        const ledger = (await tx.query(`SELECT * FROM outbound_attempt_ledger WHERE id = $1`, [previous])).rows[0];
        await startOutboundConnect(tx, { campaign, ledger, agentId: agent.id, agentUsername });
        const payload = await loadClaimPayload(tx, previous);
        await tx.query("COMMIT"); return payload;
      }
    }
    ...
    while (candidates.length) {
      ...
      if (!campaign) break;
      candidates = candidates.filter(c => c.id !== campaign.id);
      const current = (await tx.query(`SELECT status FROM outbound_campaigns WHERE id=$1 FOR UPDATE`, [campaign.id])).rows[0];
      if (current?.status !== 'running') continue;
      const run = (await tx.query(`SELECT * FROM outbound_campaign_runs WHERE campaign_id = $1 AND status = 'running' ORDER BY started_at DESC LIMIT 1`, [campaign.id])).rows[0]
        || await startCampaignRun(tx, campaign.id, agentUsername);
      const ledger = await claimOneAgentlessRecord(tx, campaign, run.id, { attemptReason: 'agent_campaign_claim' });
      if (!ledger) continue;
      ledger.metadata = { ...ledger.metadata, assigned_agent: agentUsername, assigned_at: new Date().toISOString(), assignment_mode: campaign.mode,
        auto_dial_at: campaign.mode === "progressive" ? new Date(Date.now() + PROGRESSIVE_COUNTDOWN_SECONDS * 1000).toISOString() : null };
      await tx.query(`UPDATE outbound_attempt_ledger SET metadata = metadata || $2::jsonb WHERE id = $1`, [ledger.id, JSON.stringify(ledger.metadata)]);
      await startOutboundConnect(tx, { campaign, ledger, agentId: agent.id, agentUsername });
      await tx.query(`UPDATE outbound_campaign_agent_assignments
        SET metadata = (metadata - 'served_count') || jsonb_build_object('last_served_run_id', $3::text, 'last_served_at', now()::text), updated_at = now()
        WHERE campaign_id = $1 AND agent_username = $2`, [campaign.id, agentUsername, run.id]);
      const payload = await loadClaimPayload(tx, ledger.id);
      await tx.query("COMMIT"); return payload;
    }
    await tx.query("COMMIT"); return null;
  } catch (error) {
    await tx.query("ROLLBACK");
    if (error.code === "ACD_NO_CAPACITY") return null;
    throw error;
  } finally { tx.release(); }
}
...
export async function broadcastCampaignActivationChanged(pool, campaign, reason = "campaign_status_changed") {
  ...
  try {
    ...
    campaignsLogger.error("agent_campaign_activation_broadcast_failed", { ...outboundErrorPayload(error) });
  }
}
