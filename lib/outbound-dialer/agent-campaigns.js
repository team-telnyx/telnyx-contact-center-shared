...
export async function listAgentCampaigns(pool, agentUsername) {
  ...
  const { rows } = await pool.query(
    `SELECT
     ...
     ORDER BY CASE WHEN c.status = 'running' THEN 0 WHEN c.status = 'ready' THEN 1 ELSE 2 END, c.name ASC`,
    [agentUsername],
  );
  return rows.map((row) => ({
    id: row.id,
...
export async function setAgentCampaignActivation(pool, agentUsername, campaignId) {
  await ensureAgentCampaignAssignmentsTable(pool);
  await pool.query(
  ...
  const { rows: campaignRows } = await pool.query(
    `SELECT id, name, mode, status FROM outbound_campaigns WHERE id = $1 AND status <> 'archived' AND mode IN ('preview','progressive') LIMIT 1`,
    [campaignId],
  );
  const campaign = campaignRows[0] || null;
  if (!campaign) throw new Error("Campaign not found or not available for agent activation");
  const { rows } = await pool.query(
...
async function loadActiveAgentCampaign(pool, agentUsername) {
  await ensureAgentCampaignAssignmentsTable(pool);
  const { rows } = await pool.query(
    `SELECT c.*
...
export async function claimNextAgentCampaignRecord(pool, agentUsername) {
  const campaign = await loadActiveAgentCampaign(pool, agentUsername);
  if (!campaign) return null;

  const existingClaimId = await findExistingAgentClaim(pool, campaign.id, agentUsername);
  if (existingClaimId) return loadClaimPayload(pool, existingClaimId);
