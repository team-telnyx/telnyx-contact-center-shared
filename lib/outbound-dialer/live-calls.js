import { attemptStatusReasonLabel } from "./history-view-model.js";
...
function isClaimLeaseActive(row, now) {
  if (!row?.lease_expires_at) return true;
  const leaseExpiresAt = parseTime(row.lease_expires_at);
  const nowTime = now instanceof Date ? now.getTime() : parseTime(now);
  if (!Number.isFinite(leaseExpiresAt) || !Number.isFinite(nowTime)) return true;
  return leaseExpiresAt > nowTime;
}

function isPreviewAssignmentClaimWithoutCall(row) {
  const status = String(row?.status || "").toLowerCase();
  if (status !== "claimed") return false;
  const mode = String(row?.campaign_mode || row?.mode || "").toLowerCase();
  if (!["preview", "progressive"].includes(mode)) return false;
  const metadata = normalizeMetadata(row?.metadata);
  return !firstNonEmpty(row?.call_control_id, row?.call_session_id, metadata.call_control_id, metadata.callControlId, metadata.dial_started_at);
}

export function shouldShowOutboundLiveCall(row, now = new Date()) {
  const status = String(row?.status || "").toLowerCase();
  if (status === "claimed") {
    return isClaimLeaseActive(row, now) && !isPreviewAssignmentClaimWithoutCall(row);
  }
...
function titleLiveCallReason(value) {
  return String(value || "")
    .replace(/^sip_/, "SIP ")
    .replace(/[_-]+/g, " ")
    .replace(/\s+/g, " ")
...
function buildLiveCall(row, now) {
  ...
  const startedAt = firstNonEmpty(metadata.dial_started_at, metadata.dialStartedAt, row.created_at, row.updated_at) || null;
  const endedAt = status === "hangup" || status === "failed"
    ? firstNonEmpty(metadata.completed_at, metadata.completedAt, row.updated_at, row.created_at) || null
    : null;
  ...
  const failureReason = status === "failed" ? rawReason : "";
  ...
  const hangupReason = status === "hangup" ? firstNonEmpty(row.reason_code, metadata.reason_code, metadata.reasonCode, hangupCause, sipHangupCause ? `sip_${sipHangupCause}` : "") : "";
  const statusReasonLabel = status === "failed" ? attemptStatusReasonLabel({ status, reason_code: row.reason_code || metadata.reason_code || metadata.reasonCode, failure_reason: failureReason }) : (hangupReason ? titleLiveCallReason(hangupReason) : null);
  ...
  return {
    ...
    status_reason_label: statusReasonLabel,
...
export const OUTBOUND_LIVE_CALLS_SQL = `
  ...
  WHERE (
    (
      l.status = 'claimed'
      AND COALESCE(l.lease_expires_at, NOW() + INTERVAL '1 second') > NOW()
      AND NOT (
        c.mode IN ('preview','progressive')
        AND l.call_control_id IS NULL
        AND l.call_session_id IS NULL
        AND COALESCE(l.metadata->>'dial_started_at', '') = ''
      )
    )
    OR l.status IN ('dialing','answered','running')
