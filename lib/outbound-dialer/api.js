...
export async function loadOutboundContactLists(pool, limit = 100) {
  const { rows: columnRows } = await pool.query(
    `SELECT 1 FROM information_schema.columns WHERE table_name = 'outbound_contact_records' AND column_name = 'row_data' LIMIT 1`
  );
  if (!columnRows.length) {
    return pool.query(
      `SELECT l.*, '[]'::jsonb AS row_data_columns FROM outbound_contact_lists l WHERE l.status <> 'archived' ORDER BY l.updated_at DESC LIMIT $1`,
      [limit]
    );
  }
  const { rows: metadataColumnRows } = await pool.query(
    `SELECT 1 FROM information_schema.columns WHERE table_name = 'outbound_contact_lists' AND column_name = 'metadata' LIMIT 1`
  );
  const metadataFieldSources = metadataColumnRows.length ? `
         UNION
...
export async function testOutboundContactFilter(pool, { contactListId, conditions = [], sampleLimit = 10, batchSize = 1000, scanLimit = OUTBOUND_FILTER_PREVIEW_SCAN_LIMIT }) {
  ...
  const { rows: listRows } = await pool.query(`SELECT id, name, custom_field_schema, metadata FROM outbound_contact_lists WHERE id=$1 AND status <> 'archived' LIMIT 1`, [contactListId]);
