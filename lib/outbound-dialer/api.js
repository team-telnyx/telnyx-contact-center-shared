...
export function mapContactList(row) {
  return row ? {
    ...row,
    // Backward-compatible response keys for older UI code; source-of-truth is now
    // custom_field_schema + metadata.csv_import_settings and record row_data JSONB.
...
async function outboundRecordDataExpression(pool, alias = "r") {
  const { rows } = await pool.query(
    `SELECT column_name FROM information_schema.columns WHERE table_name='outbound_contact_records' AND column_name = ANY($1::text[])`,
    [["phone_number", "standard_fields", "custom_fields", "row_data"]]
  );
  const columns = new Set(rows.map((row) => row.column_name));
  const parts = [];
  if (columns.has("phone_number")) parts.push(`jsonb_strip_nulls(jsonb_build_object('phone_number', ${alias}.phone_number))`);
  if (columns.has("standard_fields")) parts.push(`COALESCE(${alias}.standard_fields, '{}'::jsonb)`);
  if (columns.has("custom_fields")) parts.push(`COALESCE(${alias}.custom_fields, '{}'::jsonb)`);
  if (columns.has("row_data")) parts.push(`COALESCE(${alias}.row_data, '{}'::jsonb)`);
...
export async function loadSampledRowDataColumns(pool, contactListId, sampleLimit = CONTACT_LIST_FIELD_SAMPLE_LIMIT) {
  const dataExpression = await outboundRecordDataExpression(pool, "r");
  const { rows } = await pool.query(
    `SELECT COALESCE(jsonb_agg(DISTINCT keys.key ORDER BY keys.key), '[]'::jsonb) AS row_data_columns
     FROM (
       SELECT ${dataExpression} AS row_data
...
export async function loadOutboundContactLists(pool, limit = 100) {
  const recordDataExpression = await outboundRecordDataExpression(pool, "r");
  ...
  return pool.query(
    `SELECT l.*,
     ...
     LEFT JOIN LATERAL (
       ...
       FROM (
         SELECT ${recordDataExpression} AS row_data
         FROM outbound_contact_records r
       ...
       CROSS JOIN LATERAL jsonb_object_keys(sampled.row_data) AS keys(key)
       WHERE jsonb_array_length(COALESCE(metadata_fields.row_data_columns, '[]'::jsonb)) = 0
...
export async function testOutboundContactFilter(pool, { contactListId, conditions = [], sampleLimit = 10, batchSize = 1000, scanLimit = OUTBOUND_FILTER_PREVIEW_SCAN_LIMIT }) {
  ...
  const hasMetadataColumn = await tableHasColumn(pool, "outbound_contact_lists", "metadata");
  const recordDataExpression = await outboundRecordDataExpression(pool, "r");
  const { rows: listRows } = await pool.query(`SELECT id, name, custom_field_schema, ${hasMetadataColumn ? "metadata" : "'{}'::jsonb AS metadata"} FROM outbound_contact_lists WHERE id=$1 AND status <> 'archived' LIMIT 1`, [contactListId]);
  const contactList = listRows[0];
  if (!contactList) return { valid: false, validated: false, totalRecords: 0, matchingRecords: 0, sampleRows: [], columns: [], errors: ["Target contact list not found"], contactList: null, conditions: [] };
  ...
  let lastId = null;
  const effectiveScanLimit = Math.max(1, Number(scanLimit) || OUTBOUND_FILTER_PREVIEW_SCAN_LIMIT);
  while (totalRecords < effectiveScanLimit) {
    const remaining = effectiveScanLimit - totalRecords;
    const { rows } = await pool.query(
      `SELECT r.id, ${recordDataExpression} AS row_data, r.created_at
       FROM outbound_contact_records r
       WHERE r.contact_list_id=$1
  ...
  const { rows: moreRows } = await pool.query(
    `SELECT 1
     ...
     WHERE r.contact_list_id=$1
       AND ($2::timestamptz IS NULL OR (COALESCE(r.created_at, 'epoch'::timestamptz), r.id) > ($2::timestamptz, $3::uuid))
     LIMIT 1`,
    [contactListId, lastCreatedAt, lastId]
  );
  const limited = Boolean(moreRows.length);
  const errorsWithLimit = [...errors];
  if (limited) errorsWithLimit.push(`Preview scanned the first ${effectiveScanLimit.toLocaleString()} records only. Totals are limited estimates; narrow the list or run an offline/export job for exact counts.`);

  return { valid: errors.length === 0, validated: errors.length === 0, totalRecords, matchingRecords, scannedRecords: totalRecords, scanLimit: effectiveScanLimit, limited, sampleRows, columns, errors: errorsWithLimit, contactList: { id: contactList.id, name: contactList.name }, conditions: normalized.conditions };
}
