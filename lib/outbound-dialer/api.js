...
export async function findDncSuppression(pool, dncListId, value, preferredType = "phone") {
...
export async function annotateCampaignDncScaffold(pool, campaignRow, username = "system") {
  ...
  const { rows: contacts } = await pool.query(
    `SELECT id, row_data, contact_methods FROM outbound_contact_records WHERE contact_list_id=$1 ORDER BY created_at ASC LIMIT 5000`,
    [contactListId]
  );
  ...
  for (const contact of contacts) {
    ...
    const values = selectedNumbers.length
      ? selectedNumbers.map((column) => rowData[column]).filter(Boolean)
      : [
    ...
    for (const value of values) {
      hit = await findDncSuppression(pool, dncListId, value, "phone");
      if (hit) break;
    }
    if (!hit) continue;
    suppressedCount += 1;
    await pool.query(`UPDATE outbound_contact_records SET validation_status='suppressed', updated_at=NOW() WHERE id=$1`, [contact.id]);
    if (events.length < 25) events.push({ type: "dnc_suppressed", at: new Date().toISOString(), contact_record_id: contact.id, dnc_list_id: dncListId, value_type: hit.value_type, normalized_value: hit.normalized_value, annotation: "Number was on assigned DNC list and was not dialed." });
  }
  const metadata = {
    ...(campaignRow.metadata || {}),
    dnc_scaffold: { dnc_list_id: dncListId, checked_records: contacts.length, suppressed_records: suppressedCount, updated_at: new Date().toISOString(), note: "Scaffold check only; no outbound dialer worker was started." },
