...
export async function loadOutboundContactLists(pool, limit = 100) {
  ...
  return pool.query(
    `SELECT l.*, COALESCE(record_fields.row_data_columns, '[]'::jsonb) AS row_data_columns
     ...
     LEFT JOIN LATERAL (
       ...
       CROSS JOIN LATERAL jsonb_object_keys(r.row_data) AS keys(key)
       WHERE r.contact_list_id = l.id
     ) record_fields ON TRUE
...
export function outboundContactListFieldNames(list = {}, rowDataColumns = []) {
  ...
  const schemaFields = (Array.isArray(list?.custom_field_schema) ? list.custom_field_schema : []).map((field) => field?.name).filter(Boolean);
  const metadataSchemaFields = (importSettings.field_schema || importSettings.fieldSchema || []).map((field) => field?.name).filter(Boolean);
  const selectedColumns = importSettings.selected_columns || importSettings.selectedColumns || [];
  return [...new Set([...schemaFields, ...metadataSchemaFields, ...selectedColumns, ...(Array.isArray(rowDataColumns) ? rowDataColumns : [])].map((field) => String(field || "").trim()).filter(Boolean))];
}
...
export async function testOutboundContactFilter(pool, { contactListId, conditions = [], sampleLimit = 10, batchSize = 1000 }) {
  ...
  const rowDataColumns = fieldRows[0]?.row_data_columns || [];
  ...
  return { valid: errors.length === 0, validated: errors.length === 0, totalRecords, matchingRecords, sampleRows, columns, errors, contactList: { id: contactList.id, name: contactList.name }, conditions: normalized.conditions };
}
