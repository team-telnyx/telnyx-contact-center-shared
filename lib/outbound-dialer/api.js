...
async function clearInactiveDncScaffoldSuppressions(pool, dncListId, contactListId, selectedNumbers = [], matchStrategy = "phone", campaignChannel = "voice") {
  ...
  for (;;) {
    ...
    for (const contact of contacts) {
      ...
      if (!hit) continue;
      const { rowCount } = await pool.query(`UPDATE outbound_contact_records SET validation_status='valid', updated_at=NOW() WHERE id=$1 AND validation_status='suppressed'`, [contact.id]);
