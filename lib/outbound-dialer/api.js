...
export function mapOutboundSettings(row) {
  ...
  return {
    ...
    compliance_abandon_rate_subject: settings.compliance_abandon_rate_subject || settings.complianceAbandonRateSubject || "campaign",
    reschedule_timezone_skipped_contacts: settings.reschedule_timezone_skipped_contacts ?? settings.rescheduleTimezoneSkippedContacts ?? true,
    supported_countries: Array.isArray(settings.supported_countries || settings.supportedCountries) ? (settings.supported_countries || settings.supportedCountries) : ["United States", "Poland"],
