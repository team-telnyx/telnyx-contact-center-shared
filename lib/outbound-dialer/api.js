import { normalizeBlending } from "../acd/outbound-blending.mjs";
...
import { OUTBOUND_MESSAGING_CHANNELS, OUTBOUND_MESSAGING_MODES } from "@/lib/outbound-dialer/schema";
import { messagingSettingsFrom } from "@/lib/outbound-dialer/messaging/settings.mjs";

// Outbound routes are guarded by withPermission (campaigns:* and the dialer
// resource families; the system admin role holds them per decision D-13). The
// former owner-only session helper that lived here was removed in Phase 2.
...
export const outboundSchemaPayload = {
  ...
  messagingChannels: OUTBOUND_MESSAGING_CHANNELS,
  // Channels with a campaign template source, sender model and provider command.
  messagingChannelsAvailable: OUTBOUND_MESSAGING_CHANNELS,
  messagingModes: OUTBOUND_MESSAGING_MODES,
  campaignModes: ["preview", "progressive", "power", "predictive", "agentless_ai", "agentless_flow", "broadcast"],
...
export function mapOutboundSettings(row) {
  ...
  return {
    ...
    messaging: messagingSettingsFrom(settings),
...
const DNC_SCAFFOLD_PREVIOUS_STATUS_KEY = "__dnc_scaffold_previous_validation_status";

async function clearInactiveDncScaffoldSuppressions(pool, dncListId, contactListId, selectedNumbers = [], matchStrategy = "phone", campaignChannel = "voice") {
    ...
    const { rows: contacts } = await pool.query(
      `SELECT id, row_data, contact_methods, created_at
       ...
       WHERE contact_list_id=$1
         AND validation_status='suppressed'
      ...
      const { rowCount } = await pool.query(
        `UPDATE outbound_contact_records
         SET validation_status = CASE
               WHEN row_data->>$2 IN ('valid', 'needs_review') THEN row_data->>$2
               ELSE 'valid'
             END,
             row_data = row_data - $2,
             updated_at=NOW()
         WHERE id=$1
           AND validation_status='suppressed'
           AND (
             row_data->>$2 IN ('valid', 'needs_review')
             OR COALESCE(row_data ? $2, false) = false
           )`,
        [contact.id, DNC_SCAFFOLD_PREVIOUS_STATUS_KEY]
      );
      restoredCount += rowCount || 0;
    }
...
export async function annotateCampaignDncScaffold(pool, campaignRow, username = "system") {
  ...
  const { rows: dncLists } = await pool.query(`SELECT match_strategy, status FROM outbound_dnc_lists WHERE id=$1 LIMIT 1`, [dncListId]);
  if (!dncLists.length) return { campaign: campaignRow, suppressedCount: 0 };
  const matchStrategy = dncLists[0]?.match_strategy || "phone";
  const campaignChannel = campaignRow?.channel || "voice";
  if (dncLists[0]?.status !== "active") {
    const restoredCount = await clearInactiveDncScaffoldSuppressions(pool, dncListId, contactListId, selectedNumbers, matchStrategy, campaignChannel);
    return { campaign: campaignRow, suppressedCount: 0, restoredCount };
  }
  const batchSize = 1000;
  let lastCreatedAt = null;
  let lastId = null;
      ...
      const { rowCount } = await pool.query(
        `UPDATE outbound_contact_records
         SET row_data = jsonb_set(
               row_data,
               $2::text[],
               to_jsonb(COALESCE(row_data->>$3, validation_status)::text),
               true
             ),
             validation_status='suppressed',
             updated_at=NOW()
         WHERE id=$1
           AND validation_status IN ('valid', 'needs_review')`,
        [contact.id, [DNC_SCAFFOLD_PREVIOUS_STATUS_KEY], DNC_SCAFFOLD_PREVIOUS_STATUS_KEY]
      );
      suppressedCount += rowCount || 0;
      if (rowCount && events.length < 25) events.push({ type: "dnc_suppressed", at: new Date().toISOString(), contact_record_id: contact.id, dnc_list_id: dncListId, value_type: hit.value_type, normalized_value: hit.normalized_value, annotation: `${hit.value_type === "email" ? "Email" : "Number"} was on assigned DNC list and was not dialed.` });
    }
