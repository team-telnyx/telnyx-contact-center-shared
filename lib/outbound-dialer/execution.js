...
export async function executeAgentlessAttempt(pool, campaignRow, ledgerRow) {
  ...
  // WS5-T1: fine-grained lifecycle (additive; never blocks the legacy path)
  await applyDialStateForEvent(pool, ledgerRow, "call.initiated", {
    reason: "originate",
  });
  ...
  const telnyxPayload = {
    ...
    ...(amdEnabled
      ? {
          ...
          typeof amdConfig.detectionConfig === "object"
            ? { answering_machine_detection_config: amdConfig.detectionConfig }
            : {}),
        }
      : {}),
    metadata: {
...
export async function handleOutboundMachineDetection(
  ...
  if (isHuman) {
    // WS5-T3/WS6: Power & Predictive campaigns connect the human to a
    // reserved agent here. Flag-gated and mode-gated; agentless campaigns
    // keep the existing behaviour (the handler flow/AI assistant already
    // has the call).
    if (["power", "predictive"].includes(String(ledger.mode || ""))) {
      try {
        const { handleHumanAnswerForPowerCampaign } = await import("./pacing-power.js");
        const connect = await handleHumanAnswerForPowerCampaign(pool, ledger, callControlId);
        if (connect) {
          return { status: "human", ledgerId: ledger.id, powerConnect: connect };
        }
      } catch (err) {
        runnerLogger.warn("power_human_connect_failed", {
          ledgerId: ledger.id,
