 ...
 *            - "drop_message": speak amd_config.voicemailMessage, then the
 *              normal call.hangup flow finalizes the attempt
 *
...
export async function handleOutboundMachineDetection(
  pool,
  { callControlId, amdResult, eventId = null },
) {
  ...
  await applyDialStateForEvent(pool, ledger, "call.machine.detection.ended", {
  ...
  if (voicemailAction === "drop_message" && amdConfig.voicemailMessage && apiKey) {
    try {
      const speakResponse = await fetch(
        buildTelnyxV2Url(`/calls/${encodeURIComponent(callControlId)}/actions/speak`),
        {
          ...
          body: JSON.stringify({
            payload: String(amdConfig.voicemailMessage),
      ...
      if (speakResponse.ok) {
        ...
        // The Telnyx speak.ended → hangup is handled by the campaign flow /
        // subsequent call.hangup webhook, which finalizes the attempt.
        return { status: "machine", action: "drop_message", ledgerId: ledger.id };
      }
    } catch {
