...
export async function handleOutboundMachineDetection(
  ...
  if (isHuman) {
    // WS5-T3: Power campaigns connect the human to a reserved agent here.
    // Flag-gated and mode-gated; agentless campaigns keep the existing
    // behaviour (the handler flow/AI assistant already has the call).
    if (String(ledger.mode || "") === "power") {
      try {
  ...
  const voicemailAction = String(amdConfig.voicemailAction || "hangup");
