...
import { MESSAGING_REQUIREMENT_LABELS, messagingCampaignSaveRequirements } from "./messaging/campaign-config.mjs";
...
export function campaignReferenceKindForMode(mode) {
  if (mode === "broadcast") return "queue";
  if (mode === "agentless_ai") return "ai_assistant";
  if (mode === "agentless_flow") return "call_flow";
  if (mode === "preview" || mode === "progressive") return "agent_activation";
  return "queue";
}

export function campaignRequiresQueueTarget(mode) {
  return ["power", "predictive", "agentless_ai", "agentless_flow"].includes(mode);
}
...
export function campaignSaveRequirements(campaign = {}, options = {}) {
  // Messaging campaigns need a sender, a template and destination fields
  // instead of FROM numbers and callable phone columns.
  if (isOutboundMessagingChannel(campaign?.channel)) return messagingCampaignSaveRequirements(campaign, options);
  const metadata = campaign?.metadata || {};
  const missing = [];

  if (!hasValue(campaign.name)) missing.push("name");
  if (!hasValue(campaign.contact_list_id)) missing.push("contact_list");
  if (campaignRequiresQueueTarget(campaign.mode) && !hasValue(campaign.handler_ref)) missing.push("handler_ref");
...
export function campaignSaveRequirementsMessage(requirements = {}) {
  ...
  const labels = missing.map((key) => {
    if (MESSAGING_REQUIREMENT_LABELS[key]) return MESSAGING_REQUIREMENT_LABELS[key];
