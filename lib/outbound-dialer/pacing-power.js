 ...
 *
 * Returns null when this attempt is not a power-campaign human connect
 * (agentless campaigns keep their existing behaviour).
 */
export async function handleHumanAnswerForPowerCampaign(pool, ledger, callControlId) {
  if (!isPowerPacingEnabled()) return null;
  if (!pool || !ledger?.id || !callControlId) return null;
  if (String(ledger.mode || "") !== "power") return null;
