...
- **Templates** live in `form_templates` and are seeded by `ensurePostgresSchema()` from `lib/forms/form-templates.js`. The editor rail includes a Templates panel with 9 seeded examples; selecting one creates a new draft form from the template.
