/**
 * Seed All Sample Workflows Script
 * 
 ...
 * 4. Survey/Feedback
 * 
 * Usage: node scripts/seed-all-workflows.mjs
 */
...
async function seedAllWorkflows() {
  console.log("[Seed] Starting sample workflows creation...\n");
  ...
  try {
    for (const workflowData of SAMPLE_WORKFLOWS) {
      ...
      if (existing.length > 0) {
        console.log(`[Seed] ⏭️  Skipping "${workflowData.name}" - already exists`);
      ...
      console.log(`[Seed] ✅ Created workflow: "${workflow.name}" (${workflow.category})`);
      ...
      for (const stageData of workflowData.stages) {
        ...
        console.log(`[Seed]    └─ Stage "${stageData.name}": ${stageData.items.length} items`);
      }
    ...
    await client.query("ROLLBACK");
    console.error("[Seed] Error creating workflows:", error);
    throw error;
  } finally {
    client.release();
    await pool.end();
